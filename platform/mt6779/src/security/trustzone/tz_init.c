...
u32 tee_get_load_addr(u32 maddr)
{
    ...
    align_addr = ATF_MEM_ALIGNMENT;
#endif /* !CFG_TRUSTONIC_TEE_SUPPORT */
...
#if CFG_MICROTRUST_TEE_SUPPORT
    if (tee_secmem_start != PL_TEE_BASE) {
...
#if CFG_TEE_SUPPORT
    DBG_MSG("%s Jump to ATF, then 0x%x and 0x%llxx\n", MOD, tee_entry_addr, addr);
#else
    DBG_MSG("%s Jump to ATF, then 0x%llxx\n", MOD, addr);
#endif
