...
int mtk_get_vbus_adc(u32 *vbus)
{
	...
	if (ret < 0) {
		print("%s: enable vbus adc and adc en fail\n", __func__);
		goto err_adc_conv;
	}
	...
	for (i = 0; i < max_retry_cnt; i++) {
		...
		if (ret < 0) {
			print("%s: clear adc donei irq fail\n", __func__);
			goto err_adc_conv;
		}
		...
		if (ret < 0)
			goto err_adc_conv;
		if ((rpt[0] & 0x0f) != 0x01) {
			print("%s: not want channel report(%d)\n",
				__func__, rpt[0]);
			continue;
		}
