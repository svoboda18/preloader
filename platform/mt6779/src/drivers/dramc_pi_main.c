...
#if !defined(SLT) // ETT & Android Used
...
#else  // SLT Used
DRAM_DFS_FREQUENCY_TABLE_T gFreqTbl[DRAM_DFS_SHUFFLE_MAX] = {
   {LP4_DDR2400 /*0*/, DIV8_MODE, SRAM_SHU1, DUTY_LAST_K,  VREF_CALI_OFF, NORMAL_CLOSE_LOOP},  //francis : can't change this shuffle's k position due to duty K
   {LP4_DDR2400 /*1*/, DIV8_MODE, SRAM_SHU3, DUTY_LAST_K,  VREF_CALI_OFF, NORMAL_CLOSE_LOOP},  //francis : can't change this shuffle's k position due to duty K
   {LP4_DDR2400 /*2*/, DIV8_MODE, SRAM_SHU2, DUTY_LAST_K,  VREF_CALI_OFF, NORMAL_CLOSE_LOOP},  //francis : can't change this shuffle's k position due to duty K
   {LP4_DDR2400 /*3*/, DIV8_MODE, SRAM_SHU0, DUTY_NEED_K,  VREF_CALI_ON,  NORMAL_CLOSE_LOOP},  // highest freq of term group (3733) must k first.
   {LP4_DDR1600 /*4*/, DIV4_MODE, SRAM_SHU5, DUTY_DEFAULT, VREF_CALI_OFF, NORMAL_CLOSE_LOOP},
   {LP4_DDR1200 /*5*/, DIV4_MODE, SRAM_SHU7, DUTY_DEFAULT, VREF_CALI_OFF, NORMAL_CLOSE_LOOP},
   {LP4_DDR1200 /*6*/, DIV4_MODE, SRAM_SHU8, DUTY_DEFAULT, VREF_CALI_OFF, NORMAL_CLOSE_LOOP},
   {LP4_DDR800  /*7*/, DIV4_MODE, SRAM_SHU9, DUTY_DEFAULT, VREF_CALI_OFF, DDR800_SEMI_LOOP},
...
static void vCalibration_Flow_LP4(DRAMC_CTX_T *p)
{
    ...
    if (p->frequency != 400) // for Eiger E1 DDR800 no CBT calib
#endif  
    {
        #if ENABLE_CBT_DIV4_WA // for Eiger E1 1:4 mode CBT workaround
...
int Init_DRAM(DRAM_DRAM_TYPE_T dram_type, DRAM_CBT_MODE_EXTERN_T dram_cbt_mode_extern, DRAM_INFO_BY_MRR_T *DramInfo, U8 get_mdl_used)
{
    ...
    vSetDFSFreqSelByTable(p, &gFreqTbl[3]); // 9:DDR1600
#else
...
#endif
#if !(defined(LOOPBACK_TEST)||defined(DUMP_INIT_RG_LOG_TO_DE))
    if (p->dram_type == TYPE_LPDDR4X) // LP4/LP4P need confirm
    {
            ...
            DramcSwImpedanceCal(p, 1, LOW_FREQ); // for DRVN/P and ODTN
            #if FSP1_CLKCA_TERM
            DramcSwImpedanceCal(p, 1, HIGH_FREQ); // for DRVN/P and ODTN
            #endif
    }
    else
    {
...
#if (!defined(FIRST_BRING_UP)) && (!__FLASH_TOOL_DA__)
...
// ETT_NO_DRAM #endif
...
#endif
    {
        ...
        {               
#if defined(SLT)
            final_shu = SRAM_SHU4;  // DDR2400 for SLT
#else
