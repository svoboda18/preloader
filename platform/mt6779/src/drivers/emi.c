...
#if 0 //sagy: wait for EMI porting and disable for bringup
#if !__ETT__
	// Enable MPU violation interrupt to MD for D1 and D7
	*((volatile unsigned int *)EMI_MPU_CTRL_D(1)) |= 0x10;
	*((volatile unsigned int *)EMI_MPU_CTRL_D(7)) |= 0x10;
...
unsigned int mt_get_dram_type_from_hw_trap(void)
{
	...
	switch (trap) {
		...
		//	return TYPE_LPDDR4;
		default:
			dramc_debug("[dramc] Wrond HW Trapping.\n");
			ASSERT(0);
			break;
	}
	return 0;
...
void setup_dramc_voltage_by_pmic(void)
{
	...
	if (ret)
		dramc_debug("mtk_regulator_get vdram fail\n");

	ret = mtk_regulator_get("VDRAM2", &reg_vddq);
	if (ret)
		dramc_debug("mtk_regulator_get vddq fail\n");
...
static int mt_get_mdl_number(void)
{
	...
	if (emi_setting_index == 0)
		return emi_setting_index;
                    ...
                    if ((emi_settings[i].type & 0xF00) != 0x000) {
                        ...
                        if (result == 0
                            ...
                            if ((emi_settings[i].type & 0xF00) == 0x200) {
                                /* eMMC */
#if __FLASH_TOOL_DA__
								if (memcmp(sysob_mmc.cid, emi_settings[i].ID, emi_settings[i].id_length) == 0) {
#else
...
void update_voltage_select_info(void)
{
	...
	voltage_sel_info_ptr.vmddr_cust = doe_get_config("vmddr_cust");
...
{
	if (voltage_sel_info_ptr.vmddr_cust)
		return voltage_sel_info_ptr.vmddr_cust;
...
void update_last_dramc_info(void)
{
	...
	// init checksum, magic pattern and SPM dump buffer
	if(last_dramc_info_ptr->ta2_result_magic != LAST_DRAMC_MAGIC_PATTERN) {
		...
		for (temp = 0; temp <= 512 - PLAT_DBG_INFO_SIZE; temp++) {
			curr = (unsigned int *)(PLAT_DBG_INFO_BASE + PLAT_DBG_INFO_SIZE) + temp;
			DRV_WriteReg32(curr, 0);
		}
	} else {
