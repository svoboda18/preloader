...
int usb_cable_in(void)
{
	...
	if ((USBPHY_READ32(0x6c) & (0x1 << 16)) > 0) {
		pal_log_info("\n[USB] In UART Mode no USB cable detection: %x\n", (USBPHY_READ32(0x6c) & (0x1 << 16)));
		return 0;
	}
...
#if MTK_INFRA_REG_SET_CLR
	DRV_SetReg32(CFG_INFRA_RSVD3, 0x0300);	// Bit 9:8
#endif
...
static void peribus_init(void)
{
     ...
     */
    /* bypass_mode=1 to bypass X2H ahb_write_adapter so that transfer axi burst to ahb side */
    writel(readl(PERI_CON_BASE + 0x208) | (0x1 << 13), PERI_CON_BASE + 0x208);
    /*
     * allow ahb side max incr burst length up to 16 base on axi side burst length
     * and partial read data ready transfer
     */
    writel(readl(PERI_CON_BASE + 0x220) | 0xff, PERI_CON_BASE + 0x220);
}

static void mcusys_early_cke(void)
{
    u32 rank_ratio = 0;
    u64 rank_sizes[2];
    u32 cona = readl(EMI_CEN_BASE + 0x38);
    u32 val = 0;

    get_dram_rank_size(rank_sizes);
    rank_ratio = rank_sizes[0]/1024/1024/256;
    val = 1 | (((cona & (0x1<<8))>>8)<<2)  |
	  (((cona & (0x3<<2))>>2)<<4)  |
	  (((cona & (0x1<<10))>>10)<<6)|
	  (((cona & (0x1<<17))>>17)<<7)|
	  (((cona & (0x3<<18))>>18)<<8 |
    ...
    writel(val, MCUSYS_CFGREG_BASE + 0x80fc);
}
...
void platform_init(void)
{
    ...
    BOOTING_TIME_PROFILING_LOG("clkbuf");
...
#if CFG_FUNCTION_EEM_SUPPORT
	eem_probe();
	BOOTING_TIME_PROFILING_LOG("EEM");
#endif
    ...
    show_emi_test_last_word();

    /* init watch dog, will enable AP watch dog */
    mtk_wdt_init();
    BOOTING_TIME_PROFILING_LOG("wdt");

    /*init kpd PMIC mode support*/
    set_kpd_pmic_mode();
    BOOTING_TIME_PROFILING_LOG("kpd");
    ...
    BOOTING_TIME_PROFILING_LOG("battery");
#endif

    enable_PMIC_kpd_clock();
    BOOTING_TIME_PROFILING_LOG("kpd clk");
...
#else
    pal_log_info("KPD_DL_KEY1 not define\n");
#endif
...
#if !CFG_BYPASS_EMI
#if !CFG_FPGA_PLATFORM
    //EMI Workaround: disable hypervisor mode
    *(volatile unsigned int *)0x1021a060 |= 0x1;
    mt_mem_init();
...
void platform_post_init(void)
{
    ...
    if (g_boot_mode == NORMAL_BOOT && !hw_check_battery() && usb_accessory_in()) {
        pal_log_info("%s Wait for battery inserted...\n", MOD);
...
hw_ver_t drdi_get_hw_ver(void)
{
    ...
    if(!mt_get_gpio_in(GPIO175)) { // detect GPIO pull LOW
        pal_log_err("Detect GPIO175 Low: EVB\n");
        return HW_VER_V0;
    }
    else {
        pal_log_err("Detect GPIO175 High: Phone\n");
        return HW_VER_V1;
    }
