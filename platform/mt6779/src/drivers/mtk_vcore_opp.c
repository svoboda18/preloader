...
static void soc_ptp_check(void)
{
	...
	print("[CPE]: Det_Count : %d, %d, %d, %d\n", fptp0,
		fptp1, fptp2, fptp3);
	print("[CPE]: vcore efuse: 0x%x\n", ptpod);

	if ((fptp0 == 0) || (fptp1 == 0)
		|| (fptp2 == 0) || (fptp3 == 0)) {
		print("[CPE]: PTP Counter invalid\n");
	} else {
		for(i = 0; i < VCORE_OPP_NUM; i++) {
			...
			print("[CPE]: SOC_OPP=%d, SOC_BIN=%d, CPE_VBIN=%d, PASS_Criteria=%d, Result=%d\n",
...
void dvfsrc_opp_level_mapping(void)
{
	...
	print("%s: CT=%d, AGING=%d, QEA=%d, VMODE=%d\n",
			__func__,
			is_vcore_ct,
			is_vcore_aging,
			is_vcore_qea,
	...
	print("%s: FINAL vcore_opp_uv: %d, %d, %d\n",
			__func__,
			vcore_opp_0_uv,
			vcore_opp_1_uv,
			vcore_opp_2_uv);
