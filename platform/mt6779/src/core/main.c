...
static int bldr_load_sspm_part(blkdev_t *bdev, u32 *addr, u32 *size)
{
    ...
    struct sspm_info_t *sspm_info = (struct sspm_info_t *) *addr;

#if defined(MTK_AB_OTA_UPDATER)
    tmp_addr = *addr + ROUNDUP(sizeof(*sspm_info), 4);
    ...
    sspm_dbg("pmimg: 0x%x(%d bytes), dmimg: 0x%x(%d bytes), cpmimg: 0x%x(%d bytes), ximg: 0x%x(%d bytes)\n",
        (u32)pmimg, pmsize, (u32)dmimg, dmsize, (u32)cpmimg, cpmsize, (u32)ximg, xsize);

    if (!pmimg || !dmimg || !ximg) {
        pal_log_err("%s SSPM partition missing - PM:0x%x, DM:0x%x, XFILE:0x%x (@0x%x)\n",
            MOD, (u32)pmimg, (u32)dmimg, (u32)ximg, tmp_addr);
        return -1;
    }
    ...
    /* Setup sspm_info  */
    memset(sspm_info, 0, ROUNDUP(sizeof(*sspm_info), 4));

    sspm_info->dm_ofs = (unsigned int)dmimg - (unsigned int)sspm_info;
    sspm_info->dm_sz = dmsize;

    sspm_info->rd_ofs = ROUNDUP(SSPM_IMG_SZ, SSPM_MEM_ALIGN);
    sspm_info->rd_sz = SSPM_TCM_SZ + 0x800;

    sspm_info->xfile_addr = (unsigned int)ximg - sizeof(ptimg_hdr_t);
    sspm_info->xfile_sz = sizeof(ptimg_hdr_t) + xsize + sizeof(unsigned int); // with postfix 'END'

    sspm_dbg("sspm_info @0x%x: dm offset 0x%x (%d bytes), rd offset 0x%x(%d bytes), xfile addr 0x%x(%d bytes)\n",
        (u32)sspm_info, sspm_info->dm_ofs, sspm_info->dm_sz, sspm_info->rd_ofs, sspm_info->rd_sz, sspm_info->xfile_addr, sspm_info->xfile_sz);
    ...
    DRV_SetReg32(0x10005310, 0x00000050);
    DRV_SetReg32(0x10005410, 0x00550055);
    //Setup GPIO pinmux for SSPM UART0
    DRV_SetReg32(0x10005300, 0x00330000);
#endif
...
static int bldr_load_images(u32 *jump_addr)
{
    ...
    addr = CFG_MD_DSP_MEMADDR;
    bldr_load_part("TDS_ONLY_MD_DSP",bootdev, &addr, &size);
    ...
    pass_size_slt_md = addr - 4;
    *pass_size_slt_md = size;
    pal_log_info("assign 0x%X value %d\n", pass_size_slt_md, size);

    addr = CFG_FDD_MD_ROM_MEMADDR;    
    bldr_load_part("TDS_ONLY_MD_ROM", bootdev, &addr, &size);
    pass_size_slt_md = addr - 4;
    *pass_size_slt_md = size;
    pal_log_info("assign 0x%X value %d\n", pass_size_slt_md, size);

    addr = CFG_FDD_MD_ROM_MEMADDR;
    bldr_load_part("C2K_ONLY_MD_ROM", bootdev, &addr, &size);
    pass_size_slt_md = addr - 4;
    *pass_size_slt_md = size;
    pal_log_info("assign 0x%X value %d\n", pass_size_slt_md, size);
...
void main(u32 *arg)
{
...
#if !CFG_FPGA_PLATFORM
    wk_vcore_check();
    BOOTING_TIME_PROFILING_LOG("vcore chk");
#endif
    #ifdef HW_INIT_ONLY
    bldr_wait_forever();
    #endif
    ...
    if (0 != bldr_load_images(&jump_addr)) {
		...
		int result = -1;
		/*vIO32Write4B(0x10007000, 0x22000064);
		while (1) {
			result = complex_mem_test(0x56000000, 0x2000);
			if (result == 0)
				pal_log_info("[PK] complex_mem_test pass (from 0x56000000)\n");
			else
				pal_log_info("[PK] complex_mem_test fail (%d) (from 0x56000000)\n", result);
		}*/
#if !CFG_BYPASS_EMI
        goto error;
#endif
