...
#define CFG_ATF_ADDR    ATF_ROM_MEMADDR
...
static void usbdl_send_image(void)
{
	...
	if(id == IMAGE_ATF_ID) {
		...
		// move atf_dram to dram
#if CFG_ATF_DRAM_EXTENSION
		...
		print("%s MOVE ATF_DRAM dst=0x%x, src=0x%x, size=0x%x\n", MOD, maddr, cur_part, dsize);
...
static void usbdl_stay_still(void)
{
    ...
    usb_disconnect();
    print("%s USB Disconnect and Enter Dead Loop.\n", MOD);
