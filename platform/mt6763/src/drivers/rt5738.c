...
/* DD2 */
#define RT5738_I2C_CHANNEL_REG0	I2C1
...
#define RT5738_I2C_CHANNEL_REG1	I2C1
...
/**********************************************************
  ...
  *
  *********************************************************/
kal_uint32 rt5738_write_byte(struct rt5738_chip *chip, kal_uint8 addr, kal_uint8 value)
{
	int ret_code = I2C_OK;
...
kal_uint32 rt5738_read_byte(struct rt5738_chip *chip, kal_uint8 addr, kal_uint8 *dataBuffer)
{
	int ret_code = I2C_OK;
	kal_uint16 len;
	*dataBuffer = addr;
...
/**********************************************************
  ...
  *
  *********************************************************/
kal_uint32 rt5738_read_interface(struct rt5738_chip *chip, kal_uint8 RegNum, kal_uint8 *val,
...
kal_uint32 rt5738_config_interface(struct rt5738_chip *chip, kal_uint8 RegNum, kal_uint8 val,
				   kal_uint8 MASK, kal_uint8 SHIFT)
{
	kal_uint8 rt5738_reg = 0;
	kal_uint32 ret = 0;
...
int rt5738_enable(int id, unsigned char en)
{
	...
	ret = rt5738_config_interface(chip, RT5738_CTRL2, en, 0x1, chip->en_shift);
...
int rt5738_is_enabled(int id)
{
	...
	ret = rt5738_read_interface(chip, RT5738_CTRL2, &en, 0x1, chip->en_shift);
	if (ret == 0)
		return -1;
	return en;
}
...
int rt5738_set_voltage(int id, unsigned long val)
{
	...
	ret = rt5738_config_interface(chip, chip->buck_ctrl, reg_val, 0xFF, 0);
...
unsigned long rt5738_get_voltage(int id)
{
	...
	ret = rt5738_read_interface(chip, chip->buck_ctrl, &reg_val, 0xFF, 0);
	if (ret == 0)
		return 0;
...
int rt5738_set_mode(int id, unsigned char mode)
{
	...
	ret = rt5738_config_interface(chip, RT5738_CONTROL, mode, 0x1, chip->mode_shift);
	return ret;
}
...
unsigned char rt5738_get_mode(int id)
{
    ...
    ret = rt5738_read_interface(chip, RT5738_CONTROL, &reg_val, 0x01, chip->mode_shift);
    if (ret < 0)
		return 0;
...
void rt5738_driver_probe(void)
{
	...
	for (id = 0; id < RT5738_MAX; id++) {
		...
		if (g_rt5738_hw_exist[id] == 1) {
			...
			__func__, id, g_rt5738_hw_exist[id], g_rt5738_driver_ready[id]);
		} else {
			rt5738_print("[%s] PL %s is not exist\n", __func__, chip->name);
		}
	}
	/* -------------------------------------------------------- */
	/* init VDDQ at normal mode */
