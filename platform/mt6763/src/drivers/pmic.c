...
#define EXTERNAL_BUCK_MT6311 1
#define EXTERNAL_BUCK_RT5738

/* External BUCK Config */
#if EXTERNAL_BUCK_MT6311
#include <mt6311.h>
#endif
...
void pmic_check_rst(unsigned int poff_sts)
{
	...
	if (val != 0x7F) {
		...
		val = upmu_set_reg_value(MT6356_TOP_RST_STATUS, 0x78 | val);
	}
	if ((poff_sts >> 8) & 1)
		print("[%s] PMIC BWDT\n", __func__);
	if ((poff_sts >> 6) & 1)
...
void pmic_default_voltage(void)
{
	...
	if (ret)
		print("PMIC_GET reg_vsram_proc failed\n");

	ret = mtk_regulator_get("vsram_gpu", &reg_vsram_gpu);
	if (ret)
		print("PMIC_GET reg_vsram_gpu failed\n");

	ret = mtk_regulator_get("vsram_others", &reg_vsram_others);
	if (ret)
		print("PMIC_GET reg_vsram_others failed\n");

	ret = mtk_regulator_get("vproc", &reg_vproc);
	if (ret)
		print("PMIC_GET reg_vproc failed\n");

	ret = mtk_regulator_get("vcore", &reg_vcore);
	if (ret)
		print("PMIC_GET reg_vcore failed\n");

	ret = mtk_regulator_get("vmodem", &reg_vmodem);
	if (ret)
		print("PMIC_GET reg_vmodem failed\n");
	...
	/* MT6311 = 0.9V -> vsram_proc = 1.0V -> MT6311 = 0.8V -> vsram_proc = 0.9V */
		...
		if (ret)
			print("PMIC_SET reg_vsram_proc failed\n");
	} else {
	/* vsram_proc = 1.0V -> vproc = 0.8V -> vsram_proc = 0.9V */
		...
		udelay(20);
		if (ret)
			print("PMIC_SET reg_vproc failed\n");
		ret = mtk_regulator_set_voltage(&reg_vsram_proc, vsram_proc_vol, vsram_proc_max_uV);
		if (ret)
			print("PMIC_SET reg_vsram_proc failed\n");
	}
...
int pmic_get_auxadc_value(AUXADC_LIST list)
{
	...
	if (list < AUXADC_LIST_BATADC || list >= AUXADC_LIST_MAX) {
...
static void pmic_efuse_sw_load(void)
{
	...
	if ((efuse_data >> 15) & 0x1) {
		/* VS1 & VS2 PFM_RIP[1] TRIM enable */
		pmic_config_interface(0x0D3A, (efuse_data >> 13) & 0x1, 0x1, 10);
		pmic_config_interface(0x0D46, (efuse_data >> 14) & 0x1, 0x1, 10);
	}
}
