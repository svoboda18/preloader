...
void _i2c_write_reg(mt_i2c *i2c)
{
  ...
  /*Set ioconfig*/
  if (i2c->pushpull) {
      i2c_writel(i2c, OFFSET_IO_CONFIG, 0x01a0); /*set 0x1ax for AED, width-like, div 2*/
  } else {
      i2c_writel(i2c, OFFSET_IO_CONFIG, 0x01f3); /*set 0x1fx for AED, width-like, div 7*/
  }
...
/*choose one define for SCL6*/
#define SCL6_PAD_EINT11
//#define SCL6_PAD_EINT2
//#define SCL6_PAD_EINT8
//#define SCL6_PAD_F2W_CLK

/*choose one define for SDA6*/
#define SDA6_PAD_EINT12
//#define SDA6_PAD_EINT3
//#define SDA6_PAD_PWM_A
//#define SDA6_PAD_F2W_DATA

int i2c_hw_init(void)
{
	/*enable i2c arbitration*/
	DRV_WriteReg8(PERICFG_I2C_ARBITRATION, DRV_Reg8(PERICFG_I2C_ARBITRATION) | 0x7);

	/*swtich clock to PLL*/
	DRV_WriteReg32(INFRA_MODULE_CLK_SEL, DRV_Reg32(INFRA_MODULE_CLK_SEL) | (1 << I2C_CLK_SEL_OFST));

#if 0 // for external pull up resistor
	I2C_SET_REG32(0x11D300F0, (0xfffff << 5), 0); //i2c0 & i2c1
	I2C_SET_REG32(0x11D200F0, (0xfffff << 10), 0); //i2c2 & i2c4
	I2C_SET_REG32(0x11E900F0, (0x3ff << 5), 0); //i2c3
	I2C_SET_REG32(0x11E700F0, (0x3ff << 13), 0); //i2c_appm
#else // for internal pull up resistor
	//i2c0
	I2C_SET_REG32(0x11D300F0, (0x7 << 9), (0x3 << 9)); //sda0_eh
	I2C_SET_REG32(0x11D300F0, (0x7 << 19), (0x3 << 19)); //scl0_eh

	//i2c1
	I2C_SET_REG32(0x11D300F0, (0x7 << 12), (0x3 << 12)); //sda1_eh
	I2C_SET_REG32(0x11D300F0, (0x7 << 22), (0x3 << 22)); //scl1_eh

	//i2c2
	I2C_SET_REG32(0x11D200F0, (0x7 << 14), (0x3 << 14)); //sda2_eh
	I2C_SET_REG32(0x11D200F0, (0x7 << 24), (0x3 << 24)); //scl2_eh

	//i2c3
	I2C_SET_REG32(0x11E900F0, (0x7 << 7), (0x3 << 7)); //sda3_eh
	I2C_SET_REG32(0x11E900F0, (0x7 << 12), (0x3 << 12)); //scl3_eh

	//i2c4
	I2C_SET_REG32(0x11D200F0, (0x7 << 17), (0x3 << 17)); //sda4_eh
	I2C_SET_REG32(0x11D200F0, (0x7 << 27), (0x3 << 27)); //scl4_eh

	//appm
	I2C_SET_REG32(0x11E700F0, (0x7 << 15), (0x3 << 15)); //sda_appm
	I2C_SET_REG32(0x11E700F0, (0x7 << 20), (0x3 << 20)); //scl_appm

	/* REL: internal pull up resistor selection */
	//0x0:75k
	//0x1:5k
	//0x2:10k
	//0x3:1k
#define RSEL_SETTING 0x3
	//i2c0
	I2C_SET_REG32(0x11D300F0, (0x3 << 5), (RSEL_SETTING << 5)); //sda0_rel
	I2C_SET_REG32(0x11D300F0, (0x3 << 15), (RSEL_SETTING << 15)); //scl0_rel

	//i2c1
	I2C_SET_REG32(0x11D300F0, (0x3 << 7), (RSEL_SETTING << 7)); //sda1_rel
	I2C_SET_REG32(0x11D300F0, (0x3 << 17), (RSEL_SETTING << 17)); //scl1_rel

	//i2c2
	I2C_SET_REG32(0x11D200F0, (0x3 << 10), (RSEL_SETTING << 10)); //sda2_rel
	I2C_SET_REG32(0x11D200F0, (0x3 << 20), (RSEL_SETTING << 20)); //scl2_rel

	//i2c3
	I2C_SET_REG32(0x11E900F0, (0x3 << 5), (RSEL_SETTING << 5)); //sda3_rel
	I2C_SET_REG32(0x11E900F0, (0x3 << 10), (RSEL_SETTING << 10)); //scl3_rel

	//i2c4
	I2C_SET_REG32(0x11D200F0, (0x3 << 12), (RSEL_SETTING << 12)); //sda4_rel
	I2C_SET_REG32(0x11D200F0, (0x3 << 22), (RSEL_SETTING << 22)); //scl4_rel

	//appm
	I2C_SET_REG32(0x11E700F0, (0x3 << 13), (RSEL_SETTING << 13)); //sda_appm_rel
	I2C_SET_REG32(0x11E700F0, (0x3 << 18), (RSEL_SETTING << 18)); //scl_appm_rel
#endif

#if defined(SCL6_PAD_EINT11)
	/* gpio11 to scl6 */
	DRV_WriteReg32(0x10005310, (DRV_Reg32(0x10005310) & ~(0x7 << 12)) | (0x3 << 12)); //mode3 i2c scl6
	DRV_WriteReg32(0x11F20060, DRV_Reg32(0x11F20060) | (0x1 << 10)); //pullen
	DRV_WriteReg32(0x11F20080, DRV_Reg32(0x11F20080) | (0x1 << 10)); //pullsel high
#elif defined(SCL6_PAD_EINT2)
	/* gpio2 to scl6 */
	DRV_WriteReg32(0x10005300, (DRV_Reg32(0x10005300) & ~(0x7 << 8)) | (0x4 << 8)); //mode4 i2c scl6
	DRV_WriteReg32(0x11D20060, DRV_Reg32(0x11D20060) | (0x1 << 8)); //pullen
	DRV_WriteReg32(0x11D20080, DRV_Reg32(0x11D20080) | (0x1 << 8)); //pullsel high
#elif defined(SCL6_PAD_EINT8)
	/* gpio8 to scl6 */
	DRV_WriteReg32(0x10005310, (DRV_Reg32(0x10005310) & ~(0x7 << 0)) | (0x3 << 0)); //mode3 i2c scl6
	DRV_WriteReg32(0x11D20060, DRV_Reg32(0x11D20060) | (0x1 << 0)); //pullen
	DRV_WriteReg32(0x11D20080, DRV_Reg32(0x11D20080) | (0x1 << 0)); //pullsel high
#elif defined(SCL6_PAD_F2W_CLK)
	/* gpio111 to scl6 */
	DRV_WriteReg32(0x100053D0, (DRV_Reg32(0x100053D0) & ~(0x7 << 28)) | (0x4 << 28)); //mode4 i2c scl6
	DRV_WriteReg32(0x11C50060, DRV_Reg32(0x11C50060) | (0x1 << 4)); //pullen
	DRV_WriteReg32(0x11C50080, DRV_Reg32(0x11C50080) | (0x1 << 4)); //pullsel high
#endif

#if defined(SDA6_PAD_EINT12)
	/* gpio12 to sda6 */
	DRV_WriteReg32(0x10005310, (DRV_Reg32(0x10005310) & ~(0x7 << 16)) | (0x3 << 16)); //mode3 i2c sda6
	DRV_WriteReg32(0x11F20060, DRV_Reg32(0x11F20060) | (0x1 << 17)); //pullen
	DRV_WriteReg32(0x11F20080, DRV_Reg32(0x11F20080) | (0x1 << 17)); //pullsel high
#elif defined(SDA6_PAD_EINT3)
	/* gpio3 to sda6 */
	DRV_WriteReg32(0x10005300, (DRV_Reg32(0x10005300) & ~(0x7 << 12)) | (0x4 << 12)); //mode4 i2c sda6
	DRV_WriteReg32(0x11D20060, DRV_Reg32(0x11D20060) | (0x1 << 9)); //pullen
	DRV_WriteReg32(0x11D20080, DRV_Reg32(0x11D20080) | (0x1 << 9)); //pullsel high
#elif defined(SDA6_PAD_PWM_A)
	/* gpio90 to sda6 */
	DRV_WriteReg32(0x100053B0, (DRV_Reg32(0x100053B0) & ~(0x7 << 8)) | (0x3 << 8)); //mode3 i2c sda6
	DRV_WriteReg32(0x11D20060, DRV_Reg32(0x11D20060) | (0x1 << 1)); //pullen
	DRV_WriteReg32(0x11D20080, DRV_Reg32(0x11D20080) | (0x1 << 1)); //pullsel high
#elif defined(SDA6_PAD_F2W_DATA)
	/* gpio110 to sda6 */
	DRV_WriteReg32(0x100053D0, (DRV_Reg32(0x100053D0) & ~(0x7 << 24)) | (0x4 << 24)); //mode4 i2c sda6
	DRV_WriteReg32(0x11C50060, DRV_Reg32(0x11C50060) | (0x1 << 3)); //pullen
	DRV_WriteReg32(0x11C50080, DRV_Reg32(0x11C50080) | (0x1 << 3)); //pullsel high
#endif
