...
#define PMIC_CW11_INIT_VAL			0xB800
...
static void clk_buf_dump_clkbuf_log(void)
{
	...
	pmic_read_interface(PMIC_XO_EXTBUF1_ISET_M_ADDR, &pmic_cw16,
			    PMIC_REG_MASK, PMIC_REG_SHIFT);
	pmic_read_interface(MT6356_DCXO_CW20, &pmic_cw20,
			    PMIC_REG_MASK, PMIC_REG_SHIFT);
	pmic_read_interface(PMIC_RG_SRCLKEN_IN3_EN_ADDR, &top_spi_con1,
			    PMIC_REG_MASK, PMIC_REG_SHIFT);
	clk_buf_warn("%s DCXO_CW00=0x%x, CW02=0x%x, CW11=0x%x, CW14=0x%x, CW16=0x%x, CW20=0x%x, top_spi_con1=0x%x\n",
		     __func__, pmic_cw00, pmic_cw02, pmic_cw11, pmic_cw14,
		     pmic_cw16, pmic_cw20, top_spi_con1);
#endif
}
...
static void clk_buf_init_pmic_clkbuf(void)
{
	...
	pmic_config_interface(MT6356_DCXO_CW20, 0x4, 0x7, 9);

	if (clk_buf_is_auto_calc()) {
#if 0 /* FIXME: stop auto-K before it is verified */
		clk_buf_ctrl_vcn18(1);
		clk_buf_set_auto_calc(1);
		clk_buf_ctrl_vcn18(0);
#else
		clk_buf_set_auto_calc(0);
		clk_buf_set_manual_drv_curr(1);
#endif
	} else {
		clk_buf_set_auto_calc(0);
