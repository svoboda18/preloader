...
void set_kpd_pmic_mode(void)
{
	...
	/* Enable pull-up for HOMEKEY pin */
	pmic_config_interface(PMIC_RG_FCHR_PU_EN_ADDR, 0x1, PMIC_RG_FCHR_PU_EN_MASK, PMIC_RG_FCHR_PU_EN_SHIFT);
#endif
...
bool mtk_detect_key(unsigned short key)  /* key: HW keycode */
{
	...
	{	/* Power key */
		...
		pmic_read_interface(STRUP_CON12, &just_rst, PMIC_JUST_PWRKEY_RST_MASK, PMIC_JUST_PWRKEY_RST_SHIFT);
		if(just_rst)
		{
			pmic_config_interface(STRUP_CON12, 0x01, PMIC_CLR_JUST_RST_MASK,  PMIC_CLR_JUST_RST_SHIFT);
