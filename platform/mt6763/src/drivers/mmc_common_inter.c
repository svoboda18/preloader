...
void msdc_apply_timing(unsigned int id)
{
	...
	if (priv->tuning_mode == MSDC_PATH_USE_DELAY_LINE) {
		...
		if ((id == 0) && (mode == 3) && (host->src_clk==364000000)) {
			/*eMMC HS400*/
			msdc_pr_err("[SD%d] %s %d\n", id,  __func__, __LINE__);

			MSDC_SET_FIELD(MSDC_IOCON, MSDC_IOCON_RSPL, 0);    /*MSDC_IOCON[MSDC_IOCON_RSPL] */
			MSDC_SET_FIELD(MSDC_IOCON, MSDC_IOCON_R_D_SMPL, 0);     /* MSDC_IOCON[MSDC_IOCON_R_D_SMPL] */
			MSDC_SET_FIELD(MSDC_PATCH_BIT1 , MSDC_PB1_STOP_DLY_SEL, 3);    /* MSDC_PB1_STOP_DLY_SEL */
			...
			MSDC_SET_FIELD(EMMC50_CFG0, MSDC_EMMC50_CFG_CRC_STS_EDGE,0);
			/*top reg*/
			MSDC_WRITE32(EMMC_TOP_CONTROL, 0x0);  /* EMMC_TOP_CONTROL */
			MSDC_WRITE32(EMMC_TOP_CMD, 0xb00);  /* EMMC_TOP_CMD */
			MSDC_WRITE32(TOP_EMMC50_PAD_CTL0, 0x3);  /* TOP_EMMC50_PAD_CTL0 */
			MSDC_WRITE32(TOP_EMMC50_PAD_DS_TUNE, 0x13814);  /* TOP_EMMC50_PAD_DS_TUNE */
			...
			MSDC_SET_FIELD(MSDC_IOCON, MSDC_IOCON_RSPL, 0);    /*MSDC_IOCON[MSDC_IOCON_RSPL] */
			MSDC_SET_FIELD(MSDC_IOCON, MSDC_IOCON_R_D_SMPL, 0);     /* MSDC_IOCON[MSDC_IOCON_R_D_SMPL] */
			MSDC_SET_FIELD(MSDC_PATCH_BIT0, MSDC_PB0_INT_DAT_LATCH_CK_SEL, 0x0);      /* PATCH_BIT0[MSDC_PB0_INT_DAT_LATCH_CK_SEL] */
			MSDC_SET_FIELD(MSDC_PATCH_BIT0, MSDC_PB0_RD_DAT_SEL, 0x0);
			MSDC_SET_FIELD(MSDC_PATCH_BIT1, MSDC_PB1_STOP_DLY_SEL, 3);    /* MSDC_PB1_STOP_DLY_SEL */
			...
			MSDC_SET_FIELD(MSDC_PATCH_BIT2, MSDC_PB2_CFGCRCSTSEDGE, 0);      /* MSDC_PB2_CFGCRCSTSEDGE, write crc stage edge */
			/*top reg*/
			MSDC_WRITE32(EMMC_TOP_CONTROL, 0x2a80);  /* EMMC_TOP_CONTROL */
			MSDC_WRITE32(EMMC_TOP_CMD, 0xb00);  /* EMMC_TOP_CMD */
			MSDC_WRITE32(TOP_EMMC50_PAD_CTL0, 0x3);  /* TOP_EMMC50_PAD_CTL0 */
			MSDC_WRITE32(TOP_EMMC50_PAD_DS_TUNE, 0x0);  /* TOP_EMMC50_PAD_DS_TUNE */
		}
	}
}
#endif
