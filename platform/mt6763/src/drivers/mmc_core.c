...
//	struct mmc_host *host = mmc_get_host(dev_num);
	...
	if (type == WP_POWER_ON && partition == EMMC_PART_USER) {
		...
		if ((blknr % card->wp_size) != 0) {
			msdc_pr_err("[%s] wp start addr (0x%x) is not wp_size (0x%x) aligned\n",
		...
		if (blknr + blkcnt > card->nblks) {
			msdc_pr_err("[%s] wp end addr (0x%x) exceed device capacity (0x%x) aligned\n",
...
int mmc_set_boot_wp(struct mmc_card *card, STORAGE_WP_TYPE type)
{
	int err = MMC_ERR_FAILED;
	u8 value;
	...
	if (err) {
		msdc_pr_err(INFO, "[%s]: mmc_switch err %d\n", __func__,err);
		goto out;
	}
	...
	err = mmc_read_ext_csd(card->host, card);
	if (err) {
		msdc_pr_err(INFO, "[%s]: read ext_csd err %d\n", __func__,err);
	}
