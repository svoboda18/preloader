...
static int mt_get_dram_type_for_dis(void)
{
	...
	mt_set_gpio_mode(DRAM_TYPE_GPIO_NUM, GPIO_MODE_GPIO);
	mt_set_gpio_dir(DRAM_TYPE_GPIO_NUM, GPIO_DIR_IN);
	mt_set_gpio_pull_enable(DRAM_TYPE_GPIO_NUM, GPIO_PULL_DISABLE);

	gpio_val= mt_get_gpio_in(DRAM_TYPE_GPIO_NUM); // GPIO trapping - LP2: 1, LP3: 0

	if (gpio_val == GPIO_VALUE_FOR_LPDDR3) {
		type = TYPE_LPDDR3;
	} else if (gpio_val == GPIO_VALUE_FOR_LPDDR2) {
		type = TYPE_LPDDR2;
	} else {
		printf("[EMI] Get Discrete DRAM Type Fail. GPIO%d is %d.\n",
				DRAM_TYPE_GPIO_NUM, gpio_val);
		ASSERT(0);
	}
	printf("[EMI] Get DRAM type by GPIO%d is %d\n", DRAM_TYPE_GPIO_NUM, type);
#else
...
return index;
    ...
    if (!(found))
    {
        ...
        if ((discrete_dram_num > 0) && (num_of_emi_records >= 2))
        {
            ...
            for (i = 0 ; i < num_of_emi_records; i++)
            {
#ifndef DRAM_TYPE_DECIDED_BY_GPIO
        ...
        if ((0 == found) && (1 == enable_combo_dis))
        {
			...
			{
				print("[Error] It's not allow unknown dram type\r\n");
				ASSERT(0);
			}
...
{
    // follow vSetChannelNumber()
    if (mt_get_dram_type() == TYPE_LPDDR4)
        return 2;
    else
        return 1;
}
...
void setup_dramc_voltage_by_pmic(void)
{
    ...
    if (dram_type == LPDDR2) {
        // only LP2 need config Vcore (Vcore default 1.15v, suitable for LP3)
        mtk_regulator_set_voltage(&reg_vcore, LP2_VCORE, LP2_VCORE);
    }
