...
#define OFFSET(block)       (((u64)block) * BLOCK_SIZE_BMT)           //((block) << (mtd->erasesize_shift) + (page) << (mtd->writesize_shift))
...
#define bmt_buf nfi_buf
#define bmt_dat_buf bmt_info

//static u8 *dat_buf = bmt_buf;		//(0x80000);
//static u8 *oob_buf = bmt_buf + 4096;		//(0x80000 + 4096);

static u8 *dat_buf = 0;		//(0x80000);
static u8 *oob_buf = 0;		//(0x80000 + 4096);
...
{
	/* BMT read data from sector 0 ~ 3 of one page , size is 4 sectors */
    return mtk_nand_read_sector_hw(page, 0, dat, oob, 4);
}
...
{
    phys_bmt_struct *phys_bmt = bmt_dat_buf;
...
static int load_bmt_data(int start, int pool_size)
{
    ...
    phys_bmt_struct *phys_table = bmt_dat_buf;
    int i;
...
#endif
        memcpy(phys_table, bmt_buf + MAIN_SIGNATURE_OFFSET, sizeof(phys_bmt_struct));
...
bmt_struct *init_bmt(struct nand_chip *chip, int size)
{
    ...
    oob_buf = bmt_buf + MAX_DAT_SIZE;		//(0x80000 + 4096);
    ...
    page_per_block = (devinfo.blocksize * 1024) / chip->page_size;
