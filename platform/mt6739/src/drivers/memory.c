...
mt_mem_init (void)
{
  unsigned int emi_cona,emi_conc_ch0;
  int region;
  int cmp_err;

#if 0//#ifdef LAST_DRAMC
  ...
  if((g_ddr_reserve_enable==1) && (g_ddr_reserve_success==1))
  {
    ...
    emi_conc_ch0  = *(volatile unsigned *)(EMI_CONC_CH0);
    print("[DDR Reserve mode] EMI dummy read CONA = 0x%x  EMI_CONC_CH0=0x%x\n", emi_cona ,emi_conc_ch0);
...
#if 0
	do {
	        cmp_err = ta2_enable_write_read(0xF0000);        
		if (cmp_err == 0xFFFFFFFF) {
			print ("[%s] 1st complex R/W mem test agent fail\n", MOD);
			while(1);
		} else
			print ("[%s] 1st complex R/W mem test agent pass\n", MOD);
	} while(0);
#endif
