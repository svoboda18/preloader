...
static bool rtc_spar_alarm_clear_wait(void)
{
	...
	while (RTC_Read(RTC_BBPU) & RTC_BBPU_CLR)
	{
		...
		{
			print("[RTC] rtc spar/alarm clear time out!!!!!\n");
			return false;
		}
	}
...
static void rtc_bbpu_power_down(void)
{
	...
	if (is_pmic_rtc_alarm() || is_pmic_spar()) {
		print("[RTC] clearing alarm/spar, %d, %d\n", is_pmic_rtc_alarm(), is_pmic_spar());
		bbpu |= RTC_BBPU_CLR;
	}
	RTC_Write(RTC_BBPU, bbpu);
