...
/* TT: we use log_to_pl feature to switch sram buffer to dram buffer */
extern int g_log_drambuf;

extern bool g_USE_AHB_MODE;

extern char *nfi_buf;
extern char *bmt_info;
...
static inline u64 BLKDEV_BK_SIZE(u64 logical_size)
{
	return ((unsigned long long) (logical_size) / (page_size / 4));
}
...
extern int mtk_nand_read_devblk_hwecc(u64 logical_addr, char *buf);
extern int mtk_nand_read_sector_hwecc(u64 logical_addr, char *buf, u8 subpageno);
...
void pmt_init(void)
{
	...
	{
		g_USE_AHB_MODE = TRUE;
		print("[NFI]:change nfi sram buffer to dram buffer\n");
		print("[NFI]:sram => nfi_buf 0x%x bmt_info 0x%x\n", nfi_buf, bmt_info);
		memcpy(g_dram_buf->bmt_dat_buf, bmt_info, NFI_BMT_SRAM_BUFFER_SIZE);
		nfi_buf = (unsigned char *)g_dram_buf->nand_nfi_buf;
		bmt_info = (unsigned char *)g_dram_buf->bmt_dat_buf;
		print("[NFI]:dram => nfi_buf 0x%x bmt_info 0x%x\n", nfi_buf, bmt_info);
	}
#endif
    ...
    if (ret == ERR_NO_EXIST) { //first run preloader before dowload
        ...
        for (i = 0; i < PART_MAX_COUNT; i++,part++) {  
			...
			part->blks = lastest_part[i].size / (page_size / 4);
			...
			part->startblk =lastest_part[i].offset / (page_size/4);
...
part_t *pmt_get_part(part_t *part, int index)
{
    ...
    if (new_part[0].size != 0) {
        tempart.startblk = BLKDEV_BK_SIZE(new_part[index].offset);
        tempart.blks=BLKDEV_BK_SIZE(new_part[index].size);
    } else {
        tempart.startblk = BLKDEV_BK_SIZE(lastest_part[index].offset);
        tempart.blks=BLKDEV_BK_SIZE(lastest_part[index].size);
    }
...
int find_empty_page_from_top(u64 start_addr)
{
	...
	for(page_offset=0;page_offset<(block_size/page_size);page_offset++)
	{
		...
		if(!mtk_nand_read_sector_hwecc(current_add, page_readbuf, 4))
		{
...
bool find_mirror_pt_from_bottom(u64 *start_addr)
{
	...
	for(mpt_locate=(block_size/page_size);mpt_locate>0;mpt_locate--)
	{
		...
		if(!mtk_nand_read_sector_hwecc(current_start_addr, page_readbuf, 4))
		{
...
int load_exist_part_tab(u8 *buf)
{
	...
	print("load_pt from %llx lastest_part size 0x%x\n", pt_start_addr, sizeof(lastest_part));
	...
	for(pt_locate=0;pt_locate<(block_size/page_size);pt_locate++)
	{
		...
		memset(pmt_spare,0xFF,PT_SIG_SIZE);
		memset(g_nand_spare,0xFF,64);
		if(!mtk_nand_read_sector_hwecc(pt_cur_addr, buf, 4))
		{
		...
		if(is_valid_pt(buf))
		#else
		if(is_valid_pt(buf)&&is_valid_pt(pmt_spare))
		#endif
		{
		    #if defined(MTK_TLC_NAND_SUPPORT)
            slc_ratio = *((u32 *)buf + 1);//slc ratio
        	sys_slc_ratio = (slc_ratio >> 16)&0xFF;
        	usr_slc_ratio = (slc_ratio)&0xFF;
        	MSG (INIT, "[pl] slc ratio %d\n",slc_ratio);
        	#endif
	...
	if(pt_locate==(block_size/page_size))
	{
		...
		else
		{
			...
			mtk_nand_read_sector_hwecc(mirror_address, buf, 4);
		}
	}
	memcpy(buf,&buf[PT_SIG_SIZE],sizeof(lastest_part));
		...
		if(pageoffset!=0xFFFF)                                                      
		{
			...
			for(i=0;i<2;i++)
			{
				...
				else
				{
					...
					//read back verify
					if((!mtk_nand_read_sector_hwecc(current_addr, page_readbuf, 4))||memcmp(page_buf,page_readbuf,1042*4))
					{
...
int update_part_tab(void)
{
	...
	{	
		...
		for(retry_r=0;retry_r<RETRY_TIMES;retry_r++)
		{
			...
			current_addr = (start_addr+(((retry_w)+retry_r*RETRY_TIMES)*page_size));
			if(!mtk_nand_read_sector_hwecc(current_addr, page_readbuf, 4)||memcmp(page_buf,page_readbuf,4*1024))
			{
