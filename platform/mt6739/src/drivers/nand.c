...
bool g_USE_AHB_MODE = FALSE;

#if CFG_DRAM_CALIB_OPTIMIZATION
unsigned char __NOBITS_SECTION__(.gpt_sram_buf) gpt_sram_buf[NFI_TMP_SRAM_BUFFER_SIZE];
unsigned char __NOBITS_SECTION__(.gpt_sram_part_meta_info) sram_bmt_info[NFI_BMT_SRAM_BUFFER_SIZE];
...
/* TT: use nfi_sram_buf for nfi internal use */
#define NFI_SRAM_BUFFER_SIZE 1024
#define NFI_SPARE_BUFFER_SIZE 128

#if MTK_EFUSE_WRITER_SUPPORT
unsigned char nfi_sram_buf[NFI_SRAM_BUFFER_SIZE];
#endif
unsigned char g_nand_spare[NFI_SPARE_BUFFER_SIZE];
...
static inline u32 BLKDEV_BK_SIZE(u64 logical_size)
{
	return ((unsigned long)(logical_size) / (PAGE_SIZE / 4));
}
	...
	if(devinfo.iowidth == IO_ONFI || devinfo.iowidth ==IO_TOGGLEDDR || devinfo.iowidth ==IO_TOGGLESDR)
	{
		if(DDR_INTERFACE == FALSE)
		{
			if(mtk_nand_interface_config())
			{
				...
				DDR_INTERFACE = TRUE;
				print("[NAND_DBG] %s NFI_NAND_TYPE_CNFG_REG32:0x%x\n", __func__, DRV_Reg32(NFI_NAND_TYPE_CNFG_REG32));
				print("[NAND_DBG] %s NFI_ACCCON1_REG32:0x%x\n", __func__, DRV_Reg32(NFI_ACCCON1_REG32));
				print("[NAND_DBG] %s NFI_DLYCTRL_REG32:0x%x\n", __func__, DRV_Reg32(NFI_DLYCTRL_REG32));
			}
			else
			{
...
static void mtk_nand_turn_on_randomizer(u32 page)
{
	...
	u32 page_mask = (1UL << devinfo.tlcControl.block_bit) - 1;
	if(gVendor != VEND_NONE)
	{
...
#if defined(MTK_TLC_NAND_SUPPORT)
		if(devinfo.NAND_FLASH_TYPE == NAND_FLASH_TLC) {
			seed = randomizer_seed[(page&page_mask)%128];
		}
#else
		if(page_per_blk ==256)
...
static void ECC_Config(u32 ecc_bit)
{
	...
	u4DECODESize = ((g_nand_chip.sector_size + g_nand_chip.nand_fdm_size) << 3) + ecc_bit * ECC_PARITY_BIT;
...
static void mtk_nand_toshiba_rrtry(flashdev_info *pdeviceinfo, u32 retryCount, bool defValue)
{
	u32 acccon;
...
static void mtk_nand_toshiba_tlc_1y_rrtry(flashdev_info *pdeviceinfo, u32 retryCount, bool defValue)
{
	u32 acccon;
...
static void mtk_nand_toshiba_slc_1y_rrtry(flashdev_info *pdeviceinfo, u32 retryCount, bool defValue)
{
	u32 acccon;
...
static void mtk_nand_toshiba_tlc_rrtry(flashdev_info *pdeviceinfo, u32 retryCount, bool defValue)
{
	if(TRUE == devinfo.tlcControl.slcopmodeEn)
		mtk_nand_toshiba_slc_1y_rrtry(pdeviceinfo, retryCount, defValue);
	else
		mtk_nand_toshiba_tlc_1y_rrtry(pdeviceinfo, retryCount, defValue);
}

#endif
static void mtk_nand_micron_rrtry(flashdev_info *pdeviceinfo, u32 feature, bool defValue)
{
	//u32 feature = deviceinfo.feature_set.FeatureSet.readRetryStart+retryCount;
	mtk_nand_SetFeature(pdeviceinfo->feature_set.FeatureSet.sfeatureCmd,\
								pdeviceinfo->feature_set.FeatureSet.readRetryAddress,\
								(u8*)&feature,4);
}

static int g_sandisk_retry_case = 0;
static void mtk_nand_sandisk_rrtry(flashdev_info *pdeviceinfo, u32 feature, bool defValue)
{
	//u32 feature = deviceinfo.feature_set.FeatureSet.readRetryStart+retryCount;
	if(FALSE == defValue)
		mtk_nand_reset();
	...
	mtk_nand_SetFeature(pdeviceinfo->feature_set.FeatureSet.sfeatureCmd,\
								...
								(u8 *)&feature,4);
	if(FALSE == defValue)
		mtk_nand_set_command(pdeviceinfo->feature_set.FeatureSet.readRetryPreCmd);
}
...
static void mtk_nand_sandisk_19nm_rrtry(flashdev_info *pdeviceinfo, u32 retryCount, bool defValue)
{
...
static void mtk_nand_hynix_rrtry(flashdev_info *pdeviceinfo, u32 retryCount, bool defValue)
{
    if(defValue == FALSE)
    {
    	...
    	HYNIX_Set_RR_Para(g_hynix_retry_count, pdeviceinfo);
...
static void mtk_nand_hynix_16nm_rrtry(flashdev_info *pdeviceinfo, u32 retryCount, bool defValue)
{
    if(defValue == FALSE)
    {
		if(g_hynix_retry_count == READ_RETRY_STEP)
    	...
    	HYNIX_Set_RR_Para(g_hynix_retry_count, pdeviceinfo);
		//mb();
    	//HYNIX_Get_RR_Para(g_hynix_retry_count, pdeviceinfo);
    	g_hynix_retry_count ++;

	}
	//HYNIX_Set_RR_Para(retryCount, pdeviceinfo);
}
...
static void mtk_nand_sandisk_tlc_1ynm_rrtry(flashdev_info *pdeviceinfo, u32 feature, bool defValue)
{
	u16 reg_val = 0;
	...
	if(devinfo.tlcControl.slcopmodeEn) //slc block
	{
		...
		mtk_nand_SetFeature(pdeviceinfo->feature_set.FeatureSet.sfeatureCmd, 0x14, (u8 *)&value3, 4);
	}
	else //tlc block
	{
		value1 = sandisk_tlc_rrtbl_12h[feature];
		value2 = sandisk_tlc_rrtbl_13h[feature];
		mtk_nand_SetFeature(pdeviceinfo->feature_set.FeatureSet.sfeatureCmd, 0x12, (u8 *)&value1, 4);
		mtk_nand_SetFeature(pdeviceinfo->feature_set.FeatureSet.sfeatureCmd, 0x13, (u8 *)&value2, 4);
	}
...
typedef void (*rrtryFunctionType)(flashdev_info *pdeviceinfo, u32 feature, bool defValue);
...
static void mtk_nand_rrtry_func(flashdev_info *pdeviceinfo, u32 feature, bool defValue)
{
	if(gVendor != VEND_NONE)
	{
		rtyFuncArray[pdeviceinfo->feature_set.FeatureSet.rtype](pdeviceinfo, feature, defValue);
	}
}
...
static void mtk_nand_set_async()
{
	...
	while ((val != 0x01) && retry--)
	{
		if ((val != 0x01) && (val != 0x00) && (DRV_Reg16(NFI_NAND_TYPE_CNFG_REG32) == 0x01))
		{
			...
			NFI_CLN_REG32(MT_CLKMUX_NFI_INFRA_SEL, (CLK_NFI_CLK_MASK << CLK_NFI2X_SEL_SHIFT) | (CLK_NFI_CLK_MASK << CLK_NFIECC_SEL_SHIFT));
			// Set NFI2X & ECC mux sel
			NFI_SET_REG32(MT_CLKMUX_NFI_INFRA_SEL, (CLK_NFI2X_CLK_91M << CLK_NFI2X_SEL_SHIFT) | (CLK_NFIECC_CLK_218M << CLK_NFIECC_SEL_SHIFT));
			// Enable NFI2X & ECC Clock
			NFI_CLN_REG32(MT_CLKMUX_NFI_INFRA_SEL, (CLK_NFI_CLOCK_OFF << PDN_NFI2X_SHIFT) | (CLK_NFI_CLOCK_OFF << PDN_NFIECC_SHIFT));
			// Update NFI INFRA
			DRV_WriteReg32(MT_CLKMUX_NFI_INFRA_UPD, (1 << HF_FNFI2X_CK_UPDATE_SHIFT) | (1 << HF_FNFIECC_CK_UPDATE_SHIFT));
...
#define GPIO_MODE5                  (GPIO_BASE + 0x350)
...
#define TRAP_BOOT  (3U<<13)
#define BOOT_TLC   (2U<<13)
#define BOOT_SLC   (3U<<13)

static void mtk_nand_gpio_init(void)
{
	unsigned int reg_val;

	reg_val = DRV_Reg32(GPIO_BANK);
	if(BOOT_TLC == (reg_val & TRAP_BOOT))
	{
		DRV_WriteReg32(GPIO_MODE4, 0x12000000); //GPIO 38:    default is 0x11000000
		DRV_WriteReg32(GPIO_MODE5, 0x22222222); //GPIO 40~47: default is 0x11111111
		DRV_WriteReg32(GPIO_MODE6, 0x12222222); //GPIO 48~54: default is 0x10000011
		DRV_WriteReg32(GPIO_MODE0, 0x00000002); //GPIO 0:     default is 0x00000000

		DRV_WriteReg32(IOCFG_DRV_CFG0, 0x09240001); //Drv_CFG0: default is 0x09240000
		DRV_WriteReg32(IOCFG_DRV_CFG1, 0x09249249); //Drv_CFG1: default is 0x00249249
		DRV_WriteReg32(IOCFG_DRV_CFG2, 0x00000049); //Drv_CFG2: default is 0x00000000

		DRV_WriteReg32(IOCFG_PUPD_CFG0, 0x0025FF7E); //PUPD_CFG0:default is 0x0025007E
		DRV_WriteReg32(IOCFG_R0_CFG0, 0x005A0081);   //R0_CFG0:  default is 0x0002FF80
		DRV_WriteReg32(IOCFG_R1_CFG0, 0x0025FF7E);   //R1_CFG0:  default is 0x007D007F
	}
	else
	{
		// Set GPIO mode for SLC (Aux Func 3)
		DRV_WriteReg32(GPIO_MODE4, 0x33000000); //GPIO 38,39: default is 0x11000000
		DRV_WriteReg32(GPIO_MODE5, 0x33333333); //GPIO 40~47: default is 0x11111111
		DRV_WriteReg32(GPIO_MODE6, 0x13333333); //GPIO 48~54: default is 0x10000011
		DRV_WriteReg32(GPIO_MODE0, 0x00333333); //GPIO 0~5:   default is 0x00000000

		DRV_WriteReg32(IOCFG_DRV_CFG0, 0x09249249); //Drv_CFG0: default is 0x09240000
		DRV_WriteReg32(IOCFG_DRV_CFG1, 0x09249249); //Drv_CFG1: default is 0x00249249
		DRV_WriteReg32(IOCFG_DRV_CFG2, 0x00000049); //Drv_CFG2: default is 0x00000000

		DRV_WriteReg32(IOCFG_PUPD_CFG0, 0x007E7F7A); //PUPD_CFG0:default is 0x0025007E
		DRV_WriteReg32(IOCFG_R0_CFG0, 0x00018085);   //R0_CFG0:  default is 0x0002FF80
		DRV_WriteReg32(IOCFG_R1_CFG0, 0x007E7F7A);   //R1_CFG0:  default is 0x007D007F
	}
}
...
int mtk_nand_init(void)
{
...
#if CFG_DRAM_CALIB_OPTIMIZATION
    nfi_buf = (unsigned char *)gpt_sram_buf;
    bmt_info = (unsigned char *)sram_bmt_info;
#else
    nfi_buf = (unsigned char *)g_dram_buf->nand_nfi_buf;
    bmt_info = (unsigned char *)g_dram_buf->bmt_dat_buf;
#endif
	...
	/* set non-zero value after nand init done */
    g_nand_chip.ChipID = 0x56;
    return 0;
}
...
//-----------------------------------------------------------------------------
static bool mtk_nand_read_page_data(u32 * buf, u32 u4Size)
{
    u32 timeout = 0xFFFF;
    u32 i;
    u32 *pBuf32;
    ...
    for (i = 0; (i < (u4Size >> 2)) && (timeout > 0);)
    {
        if (DRV_Reg16(NFI_PIO_DIRDY_REG16) & 1)
        {
			if (i<1024)
				...
				DRV_WriteReg32(NFI_DATAW_REG32, Fake);
            i++;
        } else
        {
            timeout--;
...
//---------------------------------------------------------------------------
static bool mtk_nand_ready_for_read(u32 page_addr, u32 sector_addr, u32 sec_num, u8 * buf)
{
    u32 u4RowAddr = page_addr;
    u32 u4ColAddr = sector_addr;
    u32 colnob = 2;
...
int mtk_nand_read_devblk_hwecc(u64 logical_addr, char *buf)
{
	int i, start, len, offset = 0;
	u32 page_no;
	u32 blk_page_offset;
	u8  blk_page_index;
	u32 block = 0, mapped_block = 0;
	int rtn = ERR_RTN_SUCCESS;
	u8 *oob = buf + NAND_PAGE_SIZE;

	blk_page_offset = BLKDEV_BK_SIZE(logical_addr);
	blk_page_index = blk_page_offset % 4;
	page_no = mtk_nand_page_transform(logical_addr,&block,&mapped_block);

	MSG(INIT, "[%s] logical_addr 0x%llx page_no %d block %d mapped_block %d, %d %d\n",
		__func__, logical_addr, page_no, block, mapped_block, blk_page_offset, blk_page_index);

	rtn = mtk_nand_read_page_by_sectors(page_no, buf, g_nand_spare, (blk_page_index*4));
...
int mtk_nand_read_page_by_sectors(u32 page, u8 *dat, u8 *oob, u8 blk_sector_index)
{
	int i, start, len, offset = 0;
	int rtn = ERR_RTN_SUCCESS;

	u32 sector_size = g_nand_chip.sector_size;
	u32 fdm_size = g_nand_chip.nand_fdm_size;

	u8 *pPageBuf, *pSpareBuf;
	pPageBuf = (u8 *) dat;
	pSpareBuf = (u8 *) oob;

	MSG(INIT, "[%s] page %d sector_size %d fdm_size %d %d\n",
		__func__, page, sector_size, fdm_size, blk_sector_index);

	for ( i = blk_sector_index; i < (blk_sector_index+4); i++)
	{
		/* read one page by sector 0 ~ 15 , size is 15 sectors */
		rtn = mtk_nand_read_sector_hw(page, i, pPageBuf, pSpareBuf, 1);
		pPageBuf += sector_size;
		pSpareBuf += fdm_size;
...
int mtk_nand_read_sector_hwecc(u64 logical_addr, char *buf, u8 subpageno)
{
	int i, start, len, offset = 0;
	u32 page_no;
	u32 block, mapped_block;
	...
	/* read data from sector 0 ~ 3 of one page , size is 4 sectors */
	rtn = mtk_nand_read_sector_hw(page_no, 0, buf, g_nand_spare, subpageno);
	if (rtn != ERR_RTN_SUCCESS)  // g_nand_spare
		return FALSE;
...
int mtk_nand_read_page_hw(u32 page, u8 * dat, u8 * oob)
{
	...
	do{
		...
		spare_ptr = oob;
		logical_plane_num = 1;
		tlc_wl_info.word_line_idx = page;
...
//		else
//			mtk_nand_interface_async();
	    if (mtk_nand_ready_for_read(real_row_addr, 0, data_sector_num, pPageBuf))
	    {
				...
				if (g_bHwEcc)
	    {
		        ...
		        if (!mtk_nand_read_page_data((u32 *) temp_byte_ptr, g_nand_chip.oobblock))
	        {
		            //MSG(INIT, "[xl] fail1\n");
	            bRet = ERR_RTN_FAIL;
	        }
...
int mtk_nand_read_sector_hw(u32 page, u32 sector, u8 * dat, u8 * oob, u8 subpageno)
{
	int bRet = ERR_RTN_SUCCESS;
	u8 *pPageBuf;
	...
	bool   tlc_left_plane = TRUE;
	u32 reg_val = 0;
	int spare_per_sector = g_nand_chip.oobsize/(g_nand_chip.oobblock >> g_nand_chip.sector_shift);
#endif
	u32   real_row_addr = 0;
	u32   logical_plane_num = 1;
	u32   col_addr = 0;
	...
	MSG(INIT, "[NAND_DBG] %s: 0x%x:%d read sectors %d (dat 0x%x, oob 0x%x)\n",
		__func__, page, sector*(spare_per_sector+g_nand_chip.sector_size), subpageno,(u32)dat, (u32)oob);
	...
	do{
		mtk_nand_interface_switch();
		col_addr = sector*(spare_per_sector+g_nand_chip.sector_size);
		data_sector_num = u4SecNum;
		temp_byte_ptr = dat;
		spare_ptr = oob;
		logical_plane_num = 1;
		...
		if (mtk_nand_ready_for_read(real_row_addr, col_addr, data_sector_num, pPageBuf))
	    {
			while(logical_plane_num)
			{
#if defined(MTK_TLC_NAND_SUPPORT)
				if(devinfo.NAND_FLASH_TYPE == NAND_FLASH_TLC)
				{
					if(devinfo.tlcControl.needchangecolumn)
					{
						...
						mtk_nand_set_command(CHANGE_COLUNM_ADDR_1ST_CMD);
						mtk_nand_set_address(col_addr, real_row_addr, 2, devinfo.addr_cycle - 2);
						mtk_nand_set_command(CHANGE_COLUNM_ADDR_2ND_CMD);
				...
				if (!mtk_nand_read_page_data((u32 *) temp_byte_ptr, data_sector_num*(g_nand_chip.sector_size)))
				{
					bRet = ERR_RTN_FAIL;
				}
				...
				if(0 != (DRV_Reg32(NFI_STA_REG32) & STA_READ_EMPTY))
				{
					...
					{
						memset(pPageBuf, 0xFF,g_nand_chip.sector_size);
						memset(oob, 0xFF,g_nand_chip.nand_fdm_size * u4SecNum);
						readRetry = FALSE;
						bRet = ERR_RTN_SUCCESS;
					}
				...
				if(bRet == ERR_RTN_BCH_FAIL)
				break;
			} /* end of while(logical_plane_num) */
		} /* end of  if (mtk_nand_ready_for_read) */
...
int mtk_nand_write_devblk_hwecc(u64 logical_addr, char *buf)
{
    u32 block,mapped_block;
    u32 page_no;
    u8 *oob = buf + NAND_PAGE_SIZE;
...
int mtk_nand_write_page(u32 page, u8 * dat, u8 * oob)
{
	...
	MSG(INIT, "mtk_nand_write_page: page %d\n",page);
...
int mtk_nand_write_page_hw(u32 page, u8 * dat, u8 * oob)
{
	...
	if(devinfo.NAND_FLASH_TYPE == NAND_FLASH_TLC)	 {
		if((devinfo.tlcControl.normaltlc) && (devinfo.tlcControl.pPlaneEn))
		{
		    ...
		    temp_page_buf += (g_nand_chip.page_size / 2);
			temp_fdm_buf += ((u4SecNum / 2) * g_nand_chip.nand_fdm_size);

		    bRet = mtk_nand_write_page(page, temp_page_buf, temp_fdm_buf);
		}
		else
		{
		    bRet = mtk_nand_write_page(page, dat, oob);
		}
	}
	else
	#endif
...
bool nand_block_bad_hw(u64 logical_addr)
{
...
#if MTK_EFUSE_WRITER_SUPPORT
    unsigned char *tmp = (unsigned char *)nfi_sram_buf;
#else
    unsigned char *tmp = (unsigned char *)nfi_buf;
#endif

    memset(tmp, 0x0, NFI_SRAM_BUFFER_SIZE);
    memset(g_nand_spare, 0x0, NFI_SPARE_BUFFER_SIZE);
	...
	MSG(INIT, "nand_block_bad_hw: type %d vendor %d logical_addr %d %d\n",
		devinfo.NAND_FLASH_TYPE,devinfo.vendor, logical_addr, page);
	#if defined(MTK_TLC_NAND_SUPPORT)
	#if 1
	if((devinfo.NAND_FLASH_TYPE == NAND_FLASH_TLC)
	...
	#endif
	{
		#if 0 /* use sector read for SRAM buffer */
		mtk_nand_read_page_hw(page, tmp, g_nand_spare);
		#else
		/* Check bad block only read sector 0 of one page , size is 1 sector */
		mtk_nand_read_sector_hw(page, 0, tmp, g_nand_spare, 1);
		#endif
...
//			mtk_nand_interface_async();
    ...
    {
        if (!mtk_nand_read_page_data((u32 *) tmp), NFI_SRAM_BUFFER_SIZE)
        {
            bRet = FALSE;
        }
...
bool mark_block_bad_hw(u64 offset)
{
    ...
    int i, page_num = (BLOCK_SIZE/ g_nand_chip.oobblock);
#if MTK_EFUSE_WRITER_SUPPORT
    unsigned char* buf = (unsigned char *)nfi_sram_buf;
#else
    unsigned char* buf = (unsigned char *)nfi_buf;
#endif

    memset(buf, 0x00, NFI_SRAM_BUFFER_SIZE);
    memset(g_nand_spare, 0x0, NFI_SPARE_BUFFER_SIZE);
...
u32 nand_init_device(void)
{
    if (!blkdev_get(BOOTDEV_NAND))
    {
        ...
        g_nand_bdev.blksz = g_nand_chip.page_size/4;
...
u32 nand_get_device_id(u8 * id, u32 len)
{
	...
	/* NAND device already enabled */
	if (g_nand_chip.ChipID != 0)
	{
		memcpy(id, devinfo.id, devinfo.id_length);
		print("[%s] Already Get ID %x %x %x %x %x - %d\n", __func__,
			id[0], id[1], id[2], id[3], id[4], devinfo.id_length);
		return 0;
	}
...
u64 nand_read_data(u8 * buf, u64 offset)
{
	...
	#ifndef MTK_TLC_NAND_SUPPORT // xiaolei: i think it is useless here
    offset = nand_find_safe_block(offset);
	#endif

    if (mtk_nand_read_devblk_hwecc(offset, buf) == FALSE)
    {
...
u64 nand_write_data(u8 * buf, u64 offset)
{
	...
	if(mtk_nand_erase(offset) == FALSE)
		print("mtk_nand_erase fail\n");

    if (mtk_nand_write_devblk_hwecc(offset, buf) == FALSE)
    {
...
int mt_get_part_info_by_name(const char *name, struct part_info_t *part_info)
{
    int ret = 0;
	int index = 0;
	part_t *part_ptr = NULL;
    pt_resident *temp_part = NULL;
	char* nameptr = name;

	u32 blksz, startblk, blks;
    ...
    if (!dev) {
        print("[%s] fail, err(no dev)\n", __func__);
        return 0;
    }

	blksz = dev->blksz;

    ret = load_exist_part_tab(nfi_buf);
	temp_part = (pt_resident*)nfi_buf;
    ...
    while (temp_part->name && index < PART_MAX_COUNT) {
        if (!strcmp(nameptr, temp_part->name)) {
		    print("[%s] Find partition %s (%s)! 0x%llx 0x%llx\n",
				__func__, temp_part->name, nameptr, temp_part->offset, temp_part->size);
			break;
        }
        temp_part++; index++;
    }

	startblk = BLKDEV_BK_SIZE(temp_part->offset);
	blks = BLKDEV_BK_SIZE(temp_part->size);

	print("[%s] partition %s startblk %d blks %d\n", __func__, name, startblk, blks);

    part_info->addr = (u64)startblk * (u64)blksz;
    part_info->active = 0;
