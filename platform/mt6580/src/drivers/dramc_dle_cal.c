...
#define mcSHOW_DBG_MSG(_x_)     must_print _x_
...
*/
...
#if LPDDR == 4    
    {
        ...
        ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(DRAMC_REG_MRS), DRAMC_MODE_REG_2 | 0x00000080);
    ...
    {    
        ...
        ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(DRAMC_REG_MRS), DRAMC_MODE_REG_2 | 0x00800000);
        ucstatus |= ucDram_Register_Read(mcSET_DRAMC_REG_ADDR(DRAMC_REG_SPCMD), &u4value);
        mcSET_BIT(u4value, POS_SPCMD_MRWEN);
        ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(DRAMC_REG_SPCMD), u4value);        
        mcDELAY_US(1);
    ...
    {
        //issue MR1[7] to enable write leveling
        ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(DRAMC_REG_MRS), DRAMC_MODE_REG_2);
        ucstatus |= ucDram_Register_Read(mcSET_DRAMC_REG_ADDR(DRAMC_REG_SPCMD), &u4value);
        mcSET_BIT(u4value, POS_SPCMD_MRWEN);
        ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(DRAMC_REG_SPCMD), u4value);
        mcDELAY_US(1);
...
#endif  
        mcSHOW_DBG_MSG(("CA%d     Shift %d     CA(%d~%d) Clk(%d~%d) %d\n", uiCA, iCAShift[uiCA], 
