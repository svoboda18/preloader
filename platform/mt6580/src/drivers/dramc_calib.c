...
#if 1
void DDR_PHY_reset_new(void)
{
    DRAMC_WRITE_REG((DRAMC_READ_REG(DRAMC_PHYCTL1)) | (1 << 28), DRAMC_PHYCTL1);
    DRAMC_WRITE_REG((DRAMC_READ_REG(DRAMC_GDDR3CTL1)) | (1 << 25), DRAMC_GDDR3CTL1);
    delay_a_while(1000);
    DRAMC_WRITE_REG((DRAMC_READ_REG(DRAMC_PHYCTL1)) & (~(1 << 28)),	 DRAMC_PHYCTL1);
    DRAMC_WRITE_REG((DRAMC_READ_REG(DRAMC_GDDR3CTL1)) & (~(1 << 25)), DRAMC_GDDR3CTL1);
}
...
int do_sw_rx_dq_dqs_calib(void)
{
    ...
    print("\n");
    print("RXDQS perbit dly sw calibr \n");
    print("1.0-31 bit dq dly val\n");
    print("\n");        
...
int do_sw_tx_dq_dqs_calib(void){
  ...
  //}
    ...
    dbg_print("============================================================================\n");
    for (uiBit=0; uiBit<4; uiBit++)
    {
    	ucmax_dqsdly_byte[uiBit] = ucdqmdly_byte[uiBit] = 0;
    }
    ...
    for (uiBit=0; uiBit<4 ; uiBit++) {
	dbg_print("DQS %d Final Delay=%d (%xh)\n",uiBit,ucmax_dqsdly_byte[uiBit], ucmax_dqsdly_byte[uiBit]);
    }
    ...
    for (uiBit=0; uiBit<4 ; uiBit++) {
    	ucdqmdly_byte[uiBit] = ucdqmdly_byte[uiBit] /8;
	dbg_print("DQM %d Final Delay=%d(%xh)\n",uiBit,ucdqmdly_byte[uiBit], ucdqmdly_byte[uiBit]);
    }
#endif
...
unsigned int DramcRxdatlatCal(unsigned int use_default_rx)
{
    ...
    {        
        ...
        print("TAP=%d, err=0x%x, begin=%d, first=%d, sum=%d\n", ii, u4err_value, ucbegin, ucfirst, ucsum);
    ...
    print("first=%d total=%d best=%d\n", ucfirst, ucsum, ucbest_step);
    
    //if (apply == 1)
    {
        if (ucsum == 0)
        {
            print("DATLAT fail, write back!\n");
...
dramc_calib_exit:
    ...
    print("DRAMC calibr takes %d CPU cycle\n\r", temp);
#endif
    return err;
}
