...
void CONFIG_ISEE_DEVS(tee_dev_t_ptr devs, u32 type, u64 addr,
			u32 irq, u32 dapc_num, u32 param0)
{
...
void CONFIG_ISEE_FLAGS_MULTI_SELECTION(u64 *flags,
		u32 set_value, u64 mask, u32 offset)
{
	u64 teei_flags = *flags;
...
static void teei_boot_param_dump(tee_arg_t_ptr teearg)
{
	...
	IMSG_DEBUG("%s teearg->ssiq_number[3] : 0x%x\n", __func__,
					teearg->ssiq_number[3]);
	IMSG_DEBUG("%s teearg->ssiq_number[4] : 0x%x\n", __func__,
					teearg->ssiq_number[4]);
	...
	IMSG_ERROR("%s teearg->tee_dev[4].dev_type : 0x%x\n", __func__,
					teearg->tee_dev[4].dev_type);
	IMSG_ERROR("%s teearg->tee_dev[4].base_addr : 0x%x\n", __func__,
					teearg->tee_dev[4].base_addr);
	IMSG_ERROR("%s teearg->tee_dev[4].intr_num : 0x%x\n", __func__,
					teearg->tee_dev[4].intr_num);
	IMSG_ERROR("%s teearg->tee_dev[4].apc_num : 0x%x\n", __func__,
					teearg->tee_dev[4].apc_num);
	IMSG_ERROR("%s teearg->tee_dev[4].param[0]: 0x%x\n", __func__,
...
void teei_boot_param_prepare(u32 param_addr, u32 tee_entry,
				u64 teei_sec_dram_size, u64 dram_base,
				u64 dram_size, u32 chip_hw_code)
{
	...
	IMSG_ERROR("%s microtrust secDRamSize: 0x%x\n", __func__,
						teearg->secDRamSize);

    /* SSI Reserve */
	teearg->total_number_spi = 256;
/* Platform Support total 256 SPIs and 32 PPIs */
	//teearg->ssiq_number[0] = (32 + 248);
/* Platform reserve SPI ID 248 for TEEI, which is ID 280 */
...
/* Platform reserve SPI ID 248 for TEEI, which is ID 280 */
	...
	CONFIG_ISEE_FLAGS(&teearg->flags, ISEE_FLAGS_LOG_LEVEL_INFO,
		ISEE_FLAGS_LOG_LEVEL_MASK, ISEE_FLAGS_LOG_LEVEL_OFFSET);
#else
	CONFIG_ISEE_FLAGS(&teearg->flags, ISEE_FLAGS_LOG_LEVEL_WARNING,
		ISEE_FLAGS_LOG_LEVEL_MASK, ISEE_FLAGS_LOG_LEVEL_OFFSET);
#endif
#if CFG_MICROTRUST_TEE_MIN_MEM_SUPPORT
	CONFIG_ISEE_FLAGS(&teearg->flags, ISEE_FLAGS_SOTER_TYPE_MINIMAL,
		ISEE_FLAGS_SOTER_TYPE_MASK, ISEE_FLAGS_SOTER_TYPE_OFFSET);
#else
	CONFIG_ISEE_FLAGS(&teearg->flags, ISEE_FLAGS_SOTER_TYPE_NORMAL,
		ISEE_FLAGS_SOTER_TYPE_MASK, ISEE_FLAGS_SOTER_TYPE_OFFSET);
#endif
	CONFIG_ISEE_FLAGS(&teearg->flags,
		ISEE_FLAGS_VERIFY_MODE_DISABLE_COUNTER,
		ISEE_FLAGS_VERIFY_MODE_MASK, ISEE_FLAGS_VERIFY_MODE_OFFSET);
#if CFG_RPMB_SET_KEY
	CONFIG_ISEE_FLAGS(&teearg->flags, ISEE_FLAGS_SECURE_STORAGE_RPMB,
		ISEE_FLAGS_SECURE_STORAGE_MASK,
		ISEE_FLAGS_SECURE_STORAGE_OFFSET);
#else
	...
	CONFIG_ISEE_FLAGS_MULTI_SELECTION(&teearg->flags,
	ISEE_FLAGS_SOTER_FEATURE_RESET, ISEE_FLAGS_SOTER_FEATURE_MASK,
		ISEE_FLAGS_SOTER_FEATURE_OFFSET);
#if CFG_MICROTRUST_TUI_SUPPORT
	CONFIG_ISEE_FLAGS_MULTI_SELECTION(&teearg->flags,
	ISEE_FLAGS_SOTER_FEATURE_TUI, ISEE_FLAGS_SOTER_FEATURE_MASK,
		ISEE_FLAGS_SOTER_FEATURE_OFFSET);
#endif
#if CFG_MICROTRUST_GP_SUPPORT
	CONFIG_ISEE_FLAGS_MULTI_SELECTION(&teearg->flags,
	ISEE_FLAGS_SOTER_FEATURE_GP, ISEE_FLAGS_SOTER_FEATURE_MASK,
		ISEE_FLAGS_SOTER_FEATURE_OFFSET);
#endif
	CONFIG_ISEE_FLAGS_MULTI_SELECTION(&teearg->flags,
	ISEE_FLAGS_SOTER_FEATURE_DRM, ISEE_FLAGS_SOTER_FEATURE_MASK,
		ISEE_FLAGS_SOTER_FEATURE_OFFSET);
	if (is_tee_64_bit_supported(tee_entry)) {
	CONFIG_ISEE_FLAGS_MULTI_SELECTION(&teearg->flags,
	ISEE_FLAGS_SOTER_FEATURE_AARCH64,
	ISEE_FLAGS_SOTER_FEATURE_MASK, ISEE_FLAGS_SOTER_FEATURE_OFFSET);
	}
	if (!seclib_sbc_enabled()) {
	CONFIG_ISEE_FLAGS_MULTI_SELECTION(&teearg->flags,
	ISEE_FLAGS_SOTER_FEATURE_EFUSEDISABLE,
	ISEE_FLAGS_SOTER_FEATURE_MASK, ISEE_FLAGS_SOTER_FEATURE_OFFSET);
	}
	/* Default disabled Multi-Ta */
	CONFIG_ISEE_FLAGS(&teearg->flags,
	ISEE_FLAGS_SOTER_SYSTEM_DEBUG_OPEN,
	ISEE_FLAGS_SOTER_SYSTEM_DEBUG_MASK,
	ISEE_FLAGS_SOTER_SYSTEM_DEBUG_OFFSET);
#if (CFG_RPMB_SET_KEY && CFG_RPMB_SET_KEY_DELAY)
	if (rpmb_key_flag == 1)
		CONFIG_ISEE_FLAGS(&teearg->flags,
		ISEE_FLAGS_RPMB_KEY_NOT_PROGRAMMED,
		ISEE_FLAGS_RPMB_KEY_PROGRAM_MASK,
		ISEE_FLAGS_RPMB_KEY_PROGRAM_OFFSET);
	else
		CONFIG_ISEE_FLAGS(&teearg->flags,
		ISEE_FLAGS_RPMB_KEY_IS_PROGRAMMED,
		ISEE_FLAGS_RPMB_KEY_PROGRAM_MASK,
		ISEE_FLAGS_RPMB_KEY_PROGRAM_OFFSET);
#endif
	IMSG_DEBUG("%s teearg->flags :  0x%x\n", __func__,
		(unsigned int)teearg->flags);
	...
	case UART0_BASE:
		CONFIG_ISEE_DEVS(&(teearg->tee_dev[0]),
		MT_UART16550, UART0_BASE, MT_UART0_IRQ,
		MT_UART0_DAPC, CFG_LOG_BAUDRATE);
	break;
	case UART1_BASE:
		CONFIG_ISEE_DEVS(&(teearg->tee_dev[0]), MT_UART16550,
		UART1_BASE, MT_UART1_IRQ,
		MT_UART1_DAPC, CFG_LOG_BAUDRATE);
	break;
	case UART2_BASE:
		CONFIG_ISEE_DEVS(&(teearg->tee_dev[0]), MT_UART16550,
		UART2_BASE, MT_UART2_IRQ,
		MT_UART2_DAPC, CFG_LOG_BAUDRATE);
	break;
#ifdef UART3_BASE
	case UART3_BASE:
		CONFIG_ISEE_DEVS(&(teearg->tee_dev[0]), MT_UART16550,
		UART3_BASE, MT_UART3_IRQ,
...
#ifdef TEEI_NOT_HAVE_SGPT
	CONFIG_ISEE_DEVS(&(teearg->tee_dev[1]), MT_SEC_GPT, 0, MT_SEC_GPT_IRQ,
	MT_SEC_GPT_DAPC, 0);
#else
	CONFIG_ISEE_DEVS(&(teearg->tee_dev[1]), MT_SEC_GPT, MT_SEC_GPT_BASE,
	MT_SEC_GPT_IRQ,
	MT_SEC_GPT_DAPC, 0);
#endif
    /* SEC GPTWDT parameters */
	CONFIG_ISEE_DEVS(&(teearg->tee_dev[2]), MT_SEC_WDT, MT_SEC_WDT_BASE,
	MT_SEC_WDT_IRQ,
	MT_SEC_WDT_DAPC, 0);
    /* SEC CRYPTO parameters */
	CONFIG_ISEE_DEVS(&(teearg->tee_dev[3]), MT_SEC_CRYPTO,
	MT_SEC_CRYPTO_BASE,
	MT_SEC_CRYPTO_IRQ,
...
void teei_key_param_prepare(u32 param_addr, u8 *hwuid, u32 hwid_size,
		u8 *hrid, u32 hrid_size, u8 *rpmb_key)
{
...
#ifdef TEEI_SGPT_32K_FREQ
	teedev1->param[1] = FREQ_32K;
#else
	teedev1->param[1] = FREQ_13M;
#endif
...
//val = 0x0;
//__raw_writel(val, base_addr + SEC_APXGPT1_PRESCALE);
/* 13M clock, one division */
#ifdef TEEI_SGPT_32K_FREQ
	val = 0x10;
#else
	val = 0x0;
