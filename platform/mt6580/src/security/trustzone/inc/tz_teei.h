...
#define KEY_LEN 32
/* #define CFG_MICROTRUST_FP_SUPPORT 1 */
...
enum isee_flags {
	...
	ISEE_FLAGS_RPMB_KEY_PROGRAM_MASK	= 0x0000000002000000,
};
...
#define SPI_DEV_ATTR(PLATFORM, SPI_NUM, GPIO_NUM, GPIO_MODE, \
			SPI_BASE,  PAD, IRQ, DEVAPC, DMA) \
	{ (unsigned long int)SPI_BASE, (unsigned int)PAD, (unsigned int)IRQ, \
	(unsigned int)DEVAPC, (unsigned int)DMA }


/**************************************************************************
 * EXPORTED FUNCTIONS
 **************************************************************************/
void teei_boot_param_prepare(u32 param_addr,
				u32 tee_entry,
				u64 teei_sec_dram_size,
				u64 dram_base,
				u64 dram_size,
				u32 chip_hw_code);

void teei_key_param_prepare(u32 param_addr,
				u8 *hwuid,
				u32 hwid_size,
				u8 *hrid,
				u32 hrid_size,
