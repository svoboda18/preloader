...
DRAMC_CTX_T DramCtx_LPDDR4 =
{
  ...
  FSP_0 , //// DRAM Fast switch point type, only for LP4, useless in LP3  
  ODT_OFF,
  CBT_BYTE_MODE1,   // LP4 mass release dram is CBT byte mode
  DBI_OFF,  //read DBI
  DBI_OFF,  // write DBI
  PACKAGE_POP,        // DRAM_PACKAGE_T
...
void Dramc_DDR_Reserved_Mode_AfterSR(void)
{
	...
	*((volatile unsigned int*)DRAMC_WBR) = DRAMC_BROADCAST_OFF;	
	...
	if(u4DramType == 1)
	{
		*((volatile unsigned int*)(DDRPHY1AO_BASE + 0x0284 )) |= (0x1 << 20);//Close CHB controller clk
		/*TINFO="Setting LP3 WL/RL "*/
		//! MR2 for R0 1866 RL/WL
		*((volatile unsigned int*)(DRAMC0_AO_BASE + 0x005c)) = (0x1c << 0) | (0x2 << 8) | (0x0 << 24);
		//! MRW trigger
		*((volatile unsigned int*)(DRAMC0_AO_BASE + 0x0060)) = (1 << 0) ;
		...
		//! MRW trigger disable
		*((volatile unsigned int*)(DRAMC0_AO_BASE + 0x0060)) &= ~(1 << 0) ;
		//! MR2 for R1 1866 RL/WL
		*((volatile unsigned int*)(DRAMC0_AO_BASE + 0x005c)) = (0x1c << 0) | (0x2 << 8) | (0x1 << 24);
		//! MRW trigger
		*((volatile unsigned int*)(DRAMC0_AO_BASE + 0x0060)) = (1 << 0) ;
		//! polling done (nao + 0x88[0])
		while((*((volatile unsigned int*)(DRAMC0NAO_BASE + 0x0088)) & 0x1) == 0 );
	...
	for (freq=0;freq<3;freq++)
	{
		...
		for(ch=0;ch<2;ch++)
		{
			for(rank=0;rank<2;rank++)
			{
				...
				mcSHOW_DBG_MSG(("p->pSavetimeData->u1CBTFinalDelay_Save[%d][%d][%d]=%d;\n",ch,rank,DDRfreq,p->pSavetimeData->u1CBTFinalDelay_Save[ch][rank][DDRfreq]));
...
p->pSavetimeData->u1CBTFinalDelay_Save[0][0][4]=1;
...
p->pSavetimeData->u1CBTFinalDelay_Save[0][1][4]=0;
...
p->pSavetimeData->u1CBTFinalDelay_Save[1][0][4]=2;
...
p->pSavetimeData->u1CBTFinalDelay_Save[1][1][4]=0;
...
p->pSavetimeData->u1CBTFinalDelay_Save[0][0][2]=4;
...
p->pSavetimeData->u1CBTFinalDelay_Save[0][1][2]=0;
...
p->pSavetimeData->u1CBTFinalDelay_Save[1][0][2]=5;
...
p->pSavetimeData->u1CBTFinalDelay_Save[1][1][2]=0;
...
p->pSavetimeData->u1CBTFinalDelay_Save[0][0][1]=4;
...
p->pSavetimeData->u1CBTFinalDelay_Save[0][1][1]=0;
...
p->pSavetimeData->u1CBTFinalDelay_Save[1][0][1]=5;
...
p->pSavetimeData->u1CBTFinalDelay_Save[1][1][1]=0;
...
int Init_DRAM(DRAM_DRAM_TYPE_T dram_type, DRAM_CBT_MODE_T dram_cbt_mode, DRAM_INFO_BY_MRR_T *DramInfo)
{
...
#endif
    {
        mcSHOW_DBG_MSG(("\n\n[KIBO] ETT version 0.0.0.1\n dram_type %d, cbt_mode %d VENDOR=%d\n\n", p->dram_type, p->dram_cbt_mode, p->vendor_id));
...
//ETT_NO_DRAM #if 0 	
    ...
    if(global_which_test  == 0)          
        ...
        p->support_rank_num =DramRankNumberDetection(p);
...
#if ENABLE_MRR_AFTER_FIRST_K 
    Init_DRAM(DramConfig.dram_type, DramConfig.dram_cbt_mode, &DramInfo);
#endif
    Init_DRAM(DramConfig.dram_type, DramConfig.dram_cbt_mode, NULL);
}
...
void DPI_SW_main_LP4(DRAMC_CTX_T *DramConfig)
    ...
    DramConfig->dram_fsp = FSP_0;
    // DRAM CBT mode, only for LP4, useless in LP3
    DramConfig->dram_cbt_mode = CBT_BYTE_MODE1;
    // IC and DRAM package
    DramConfig->package = PACKAGE_POP;
    // bus width
