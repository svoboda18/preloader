...
signed int fgauge_get_soff_time(void *data)
{
	...
	if (m > 1000)
		print( "[fgauge_get_soff_time] fg_get_data_ready_status timeout 1 !\r\n");
	...
	if (m > 1000)
		print("[fgauge_get_soff_time] fg_get_data_ready_status timeout 2 !\r\n");
...
void fuel_gauge_init(void)
{
	...
	hw_id = upmu_get_reg_value(MT6355_HWCID);
	sw_id = upmu_get_reg_value(MT6355_SWCID);
	...
	print(
		"[fuel_gauge_init] fg_reset_status %d do_init_fgadc_reset %d fg_curr_time %d fg_soff_valid:%d \
			shutdown_pmic_time %d hw_id 0x%x sw_id 0x%x, %d %d 0x%x 0x%x\n",
			fg_reset_status, do_init_fgadc_reset, fg_curr_time, fg_soff_valid, 
			shutdown_pmic_time, hw_id, sw_id,
			boot_vbat, shutdowntime, rtc_rdwr_fginit_bits(NULL), reset_sel);
	is_fg_init = true;
...
void fg_set_2sec_reboot(void)
{
	...
	pmic_config_interface((U32)(PMIC_SYSTEM_INFO_CON0_ADDR), (U32)(1),
		(U32)(0x0001),
		(U32)(0));
...
/*
	U16 rvalue, wvalue;
	int hw_id = upmu_get_reg_value(MT6355_HWCID);
	U16 temp_value;
