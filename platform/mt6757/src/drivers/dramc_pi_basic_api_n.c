...
void DDRPhyPLLSetting(DRAMC_CTX_T *p)
{
    ...
    {
        vIO32WriteFldAlign_All(DDRPHY_SHU1_PLL5, 0x8a00, SHU1_PLL5_RG_RPHYPLL_SDM_PCW);
        vIO32WriteFldAlign_All(DDRPHY_SHU1_PLL7, 0x8a00, SHU1_PLL7_RG_RCLRPLL_SDM_PCW);        
    }
           ...
           // if(p->frequency<=1200)
            {
                ...
                DramcModeRegWrite(p, 0xc, u1MR12Value[u1ChannelIdx][u1MRFsp]); //MR12 VREF-CA 
...
{        
    ...
    //Darren+  for DRAM normal/byte mode
    if(p->dram_cbt_mode==CBT_BYTE_MODE1)
...
static void SetTxDlyDcmEnable(DRAMC_CTX_T *p)//Save more active power
{
    ...
    else
    {
        vIO32WriteFldMulti_All(DDRPHY_MISC_CG_CTRL5, 
            P_Fld(1, MISC_CG_CTRL5_K_R_CA_PI_DCM_EN) | P_Fld(1, MISC_CG_CTRL5_K_R_CA_DLY_DCM_EN) |
            P_Fld(1, MISC_CG_CTRL5_K_R_DQ0_DLY_DCM_EN) | P_Fld(1, MISC_CG_CTRL5_K_R_DQ1_DLY_DCM_EN) |
            P_Fld(1, MISC_CG_CTRL5_K_R_DQ0_PI_DCM_EN) | P_Fld(1, MISC_CG_CTRL5_K_R_DQ1_PI_DCM_EN) 
            );
    }
}
#endif
...
//HW sync gating tracking
...
#if SET_CKE_2_RANK_INDEPENDENT
    {
        ...
        { //Kibo Plus Only
            ...
            mcSHOW_DBG_MSG(("SET_CKE_2_RANK_INDEPENDENT: ON\n"));
        }
    }
#else
    mcSHOW_DBG_MSG(("SET_CKE_2_RANK_INDEPENDENT: OFF\n"));
#endif
...
U32 TestEngineCompare(DRAMC_CTX_T *p)
{
    ...
    if(p->test_pattern <= TEST_XTALK_PATTERN)
    {
        ...
        u4err_value = DramcEngine2Run(p, TE_OP_READ_CHECK, p->test_pattern);
        DramcEngine2End(p);
    }
...
void DramcTest_DualSch_stress(DRAMC_CTX_T *p)
{
	...
	while(count<10)
	{
        ...
        u1MR12Value[p->channel][p->dram_fsp] = 0x14;
        DramcModeRegWrite(p, 12, u1MR12Value[p->channel][p->dram_fsp]);
