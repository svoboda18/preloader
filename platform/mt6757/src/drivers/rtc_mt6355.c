...
bool rtc_boot_check(void)
{
	...
	/* Turning off 75K div 2 clock */
	pmic_config_interface(PMIC_RG_RTC_75K_CK_PDN_ADDR, 1, PMIC_RG_RTC_75K_CK_PDN_MASK, PMIC_RG_RTC_75K_CK_PDN_SHIFT);
	pmic_config_interface(FQMTR_CLK_CK_PDN_SET, 1, FQMTR_CLK_CK_PDN_MASK, FQMTR_CLK_CK_PDN_SHIFT);
	rtc_clean_mark();
...
void rtc_set_xomode(bool bIs32kLess)
{
	...
	if (!is_clk_buf_from_pmic()) {
		/* XO_XMODE_M=0 & XO_EN32K_M=0 is buffer mode */
		pmic_config_interface(PMIC_XO_XMODE_M_ADDR, 0, PMIC_XO_XMODE_M_MASK, PMIC_XO_XMODE_M_SHIFT);
	...
	if (!is_clk_buf_from_pmic()) {
		// DE patch to select external 32K
		pmic_config_interface(PMIC_XO_EN32K_MAN_ADDR, 1, PMIC_XO_EN32K_MAN_MASK, PMIC_XO_EN32K_MAN_SHIFT);
		pmic_config_interface(PMIC_XO_EN32K_M_ADDR, 0, PMIC_XO_EN32K_M_MASK, PMIC_XO_EN32K_M_SHIFT);
	}
