...
static void mtk_wdt_reset_deglitch_enable(void)
{
    ...
    DRV_WriteReg32(MTK_WDT_RSTDEG_EN2, MTK_WDT_RSTDEG_EN2_KEY|MTK_WDT_RSTDEG_BY_32K);
}
...
int mtk_wdt_is_pmic_full_reset(void)
{
    ...
    static bool cold_reset = false;

    if (first_check) {
        first_check = false;
        print("[Reg check 1][0x%x]=0x%x\n", MT6351_PMIC_RG_RSV_SWREG_ADDR,
            upmu_get_reg_value(MT6351_PMIC_RG_RSV_SWREG_ADDR));
        if (upmu_get_reg_value(MT6351_PMIC_RG_RSV_SWREG_ADDR)){
            print("[Reg check][0x%x]=0x%x\n", MT6351_PMIC_RG_RSV_SWREG_ADDR,
                upmu_get_reg_value(MT6351_PMIC_RG_RSV_SWREG_ADDR));
            pmic_config_interface(MT6351_PMIC_RG_RSV_SWREG_ADDR, 0, 0xff, 0); //clear status at 0x22
            print("[Reg check][0x%x]=0x%x\n", MT6351_PMIC_RG_RSV_SWREG_ADDR,
                upmu_get_reg_value(MT6351_PMIC_RG_RSV_SWREG_ADDR));
            cold_reset = true;
        } else
...
void mtk_wdt_init(void)
{
	...
	print("RGU DEBUG_CTL: %x\n", DRV_Reg32(MTK_WDT_DEBUG_CTL));
	print("RGU LATCH_CTL: %x\n", DRV_Reg32(MTK_WDT_LATCH_CTL));
	print("RGU DEBUG1: %x\n", DRV_Reg32((MTK_WDT_BASE+0x0504)));
	print("RGU DEBUG2: %x\n", DRV_Reg32((MTK_WDT_BASE+0x0508)));
    print("==== Dump RGU Reg End ====\n");
    
    rgu_mode = DRV_Reg32(MTK_WDT_MODE);

    wdt_sta = mtk_wdt_get_status(); /* This function will store the reset reason: Time out/ SW trigger */
