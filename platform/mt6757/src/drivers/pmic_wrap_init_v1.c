...
{
	unsigned int pwrap_prj_segment = (seclib_get_devinfo_with_index(30) & 0x000000E0) >> 5;

#ifndef DUAL_PMICS
	if ((pwrap_prj_segment == 0x0) || (pwrap_prj_segment == 0x1)) {
		/* MT6757 */
		WRAP_WR32(PMIC_WRAP_STAUPD_GRPEN, 0x54);
	} else if ((pwrap_prj_segment == 0x3) || (pwrap_prj_segment == 0x7)) {
		/* MT6757P */
...
static signed int _pwrap_init_sistrobe(int dual_si_sample_settings)
{
	...
	for (si_dly = 0; si_dly < 10; si_dly++) {
...
#endif
		{
			...
			if ((rdata & 0x7fff) != (test_data[i] & 0x7fff)) {
				...
				PWRAPERR("InitSiStrobe (%x, %x, %x) Data Boundary Is Found !!\n",
				         si_dly, WRAP_RD32(PMIC_WRAP_SI_CK_CON), rdata);
				error = 1;
				break;
			}
		}
		...
		PWRAPLOG("si_dly = %x, *PMIC_WRAP_RESERVED = %x, rdata = %x\n",
		         si_dly, reserved, rdata);
		PWRAPLOG("si_dly = %x, *SI_CK_CON = %x, rdata = %x\n",
		         si_dly, WRAP_RD32(PMIC_WRAP_SI_CK_CON), rdata);
	}
	...
	PWRAPLOG("SI Strobe Calibration For PMIC 0 Done, (si_sample_ctrl:%x, %x)\n", si_sample_ctrl, reserved);
	PWRAPLOG("SI Strobe Calibration For PMIC 0 Done, (SI_CK_CON:%x, RG_SPI_DLY_SEL:%x)\n", WRAP_RD32(PMIC_WRAP_SI_CK_CON), si_dly);
...
static void _pwrap_set_DCM(void)
{
...
#endif
	/* no debounce */
	WRAP_WR32(PMIC_WRAP_DCM_DBC_PRD, 0x0);
	...
	sub_return = WRAP_RD32(PMIC_CLOCK_DCM);
	PWRAPLOG("disable PMIC_CLOCK_DCM=0x%x\n", sub_return);
