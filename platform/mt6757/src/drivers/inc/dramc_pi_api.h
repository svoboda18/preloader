...
//SW option
#if !__FLASH_TOOL_DA__ && !defined(SLT)
...
#if !__ETT__ && !__FLASH_TOOL_DA__ && !defined(SLT)
// Preloader: using config CFG_DRAM_CALIB_OPTIMIZATION to identify
#define PL_VERSION 0x99000006
...
//       U8 u1RxWinPerbitDQM_bypass_Save[CHANNEL_MAX][RANK_MAX][PLL_FREQ_SEL_MAX][DQS_NUMBER];  //for bypass rxwindow
    ...
    U8 u1CBTCsDelay_Save[CHANNEL_MAX][RANK_MAX][PLL_FREQ_SEL_MAX]; 

    S8 s1ClockDuty_clk_delay_cell[CHANNEL_MAX][RANK_MAX];
    S8 s1DQSDuty_clk_delay_cell[CHANNEL_MAX][RANK_MAX];
