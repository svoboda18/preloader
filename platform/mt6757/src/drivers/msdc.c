...
void msdc_dump_card_status(u32 card_status)
{
	...
	if (card_status & R1_OUT_OF_RANGE)
		msdc_printf("\t[CARD_STATUS] Out of Range\n");
	if (card_status & R1_ADDRESS_ERROR)
		msdc_printf("\t[CARD_STATUS] Address Error\n");
	if (card_status & R1_BLOCK_LEN_ERROR)
		msdc_printf("\t[CARD_STATUS] Block Len Error\n");
	if (card_status & R1_ERASE_SEQ_ERROR)
		msdc_printf("\t[CARD_STATUS] Erase Seq Error\n");
	if (card_status & R1_ERASE_PARAM)
		msdc_printf("\t[CARD_STATUS] Erase Param\n");
	if (card_status & R1_WP_VIOLATION)
		msdc_printf("\t[CARD_STATUS] WP Violation\n");
	if (card_status & R1_CARD_IS_LOCKED)
		msdc_printf("\t[CARD_STATUS] Card is Locked\n");
	if (card_status & R1_LOCK_UNLOCK_FAILED)
		msdc_printf("\t[CARD_STATUS] Lock/Unlock Failed\n");
	if (card_status & R1_COM_CRC_ERROR)
		msdc_printf("\t[CARD_STATUS] Command CRC Error\n");
	if (card_status & R1_ILLEGAL_COMMAND)
		msdc_printf("\t[CARD_STATUS] Illegal Command\n");
	if (card_status & R1_CARD_ECC_FAILED)
		msdc_printf("\t[CARD_STATUS] Card ECC Failed\n");
	if (card_status & R1_CC_ERROR)
		msdc_printf("\t[CARD_STATUS] CC Error\n");
	if (card_status & R1_ERROR)
		msdc_printf("\t[CARD_STATUS] Error\n");
	if (card_status & R1_UNDERRUN)
		msdc_printf("\t[CARD_STATUS] Underrun\n");
	if (card_status & R1_OVERRUN)
		msdc_printf("\t[CARD_STATUS] Overrun\n");
	if (card_status & R1_CID_CSD_OVERWRITE)
		msdc_printf("\t[CARD_STATUS] CID/CSD Overwrite\n");
	if (card_status & R1_WP_ERASE_SKIP)
		msdc_printf("\t[CARD_STATUS] WP Eraser Skip\n");
	if (card_status & R1_CARD_ECC_DISABLED)
		msdc_printf("\t[CARD_STATUS] Card ECC Disabled\n");
	if (card_status & R1_ERASE_RESET)
		msdc_printf("\t[CARD_STATUS] Erase Reset\n");
	if (card_status & R1_READY_FOR_DATA)
		msdc_printf("\t[CARD_STATUS] Ready for Data\n");
	if (card_status & R1_SWITCH_ERROR)
		msdc_printf("\t[CARD_STATUS] Switch error\n");
	if (card_status & R1_URGENT_BKOPS)
		msdc_printf("\t[CARD_STATUS] Urgent background operations\n");
	if (card_status & R1_APP_CMD)
		msdc_printf("\t[CARD_STATUS] App Command\n");

	msdc_printf("\t[CARD_STATUS] '%s' State\n",
...
int msdc_wait_rsp(struct mmc_host *host, struct mmc_command *cmd)
{
	...
	if ((status & MSDC_INT_RSPCRCERR) || (status & MSDC_INT_ACMDCRCERR)) {
		...
		error = MMC_ERR_BADCRC;
		msdc_printf("[SD%d] CMD(%d): RSP(%d) ERR(BADCRC)\n",
		       host->id, opcode, cmd->rsptyp);
	} else if ((status & MSDC_INT_CMDTMO) || (status & MSDC_INT_ACMDTMO)) {
		error = MMC_ERR_TIMEOUT;
		msdc_printf("[SD%d] CMD(%d): RSP(%d) ERR(CMDTO) AUTO(%d)\n",
		       host->id, opcode, cmd->rsptyp, status & MSDC_INT_ACMDTMO ? 1: 0);
	} else {
...
out:
		...
		while (left_sz) {
			...
			MSDC_DMA_ON();
			//msdc_printf("nblks(%d),xfer_sz(%d),left_sz(%d)\n",nblks,xfer_sz,left_sz);
...
int msdc_init(int id, struct mmc_host *host, int clksrc, int mode)
{
	...
	msdc_printf("set up GPIO for MSDC\n");
#endif
...
#if defined(MMC_MSDC_DRV_PRELOADER) && !defined(FPGA_PLATFORM)
#if defined(PMIC_CHIP_MT6355)
	/*sd card default power off*/
#else
	/*disable sd card power*/
	pmic_config_interface(0x0A2E, 0x0, 0x1, 1); // Disable RG_VMCH_EN
	pmic_config_interface(0x0AAA, 0x0, 0x1, 1); // Disable RG_VMC_EN
#endif
#endif
