...
void AutoRefreshCKEOff(DRAMC_CTX_T *p)
{
    //if(p->vendor_id==VENDOR_SAMSUNG && p->dram_type==TYPE_LPDDR3)
    {
        ...
        vIO32WriteFldAlign_All(DRAMC_REG_CKECTRL, 1, CKECTRL_CKEFIXOFF);
        vIO32WriteFldAlign_All(DRAMC_REG_CKECTRL, 1, CKECTRL_CKE1FIXOFF);
        mcDELAY_US(1);
    }
}
...
void DDRPhyPLLSetting(DRAMC_CTX_T *p)
{
    ...
    else
    {
        ...
        if((mt_get_chip_sw_ver()==CHIP_SW_VER_02) && (p->frequency<=800))//Lewis@20161129: Fix DDR1600 S-idle Vcore power is too big problem.
        {
            u1MIDPICAP_SEL = 0;
        }
    }
    ...
    //if(p->vendor_id==VENDOR_SAMSUNG && p->dram_type==TYPE_LPDDR3)
    {
        ...
        vIO32WriteFldAlign_All(DDRPHY_MISC_SPM_CTRL1, 0xffffffff, MISC_SPM_CTRL1_PHY_SPM_CTL1);
    }
    ...
    { 
        if(p->frequency==1600 /*&& p->vendor_id==VENDOR_HYNIX*/)
        {
            vIO32WriteFldAlign_All(DDRPHY_SHU1_PLL5, 0x7300, SHU1_PLL5_RG_RPHYPLL_SDM_PCW);
            vIO32WriteFldAlign_All(DDRPHY_SHU1_PLL7, 0x7300, SHU1_PLL7_RG_RCLRPLL_SDM_PCW);
        }
        else
        {
    ...
    vIO32WriteFldMulti_All(DDRPHY_SHU1_CA_CMD6, P_Fld(0x2, SHU1_CA_CMD6_RG_ARPI_MIDPI_VTH_SEL_CA)
    ...
    //if(p->vendor_id==VENDOR_SAMSUNG && p->dram_type==TYPE_LPDDR3)
    {
        ...
        vIO32WriteFldAlign_All(DRAMC_REG_CKECTRL, 0, CKECTRL_CKEFIXON);
        vIO32WriteFldAlign_All(DRAMC_REG_CKECTRL, 0, CKECTRL_CKE1FIXON);
        vIO32WriteFldAlign_All(DRAMC_REG_CKECTRL, 0, CKECTRL_CKEFIXOFF);
        vIO32WriteFldAlign_All(DRAMC_REG_CKECTRL, 0, CKECTRL_CKE1FIXOFF);
    }
}
...
void DramcPowerOnSequence(DRAMC_CTX_T *p)
{
    ...
    {
    //reset dram = low
    vIO32WriteFldAlign_All(DRAMC_REG_ADDR(DRAMC_REG_DDRCONF0), 0, DDRCONF0_GDDR3RST);  
    ...
    //reset dram = low
    vIO32WriteFldAlign_All(DRAMC_REG_ADDR(DRAMC_REG_DDRCONF0), 1, DDRCONF0_GDDR3RST);  
...
DRAM_STATUS_T DramcModeRegInit_LP4(DRAMC_CTX_T *p)
{
    ...
    for(u1ChannelIdx=0; u1ChannelIdx<(p->support_channel_num); u1ChannelIdx++)
    {
        ...
        {
            mcSHOW_DBG_MSG(("[ModeRegInit_LP4] CH%u Rank%u\n", u1ChannelIdx, u4RankIdx));          
            mcFPRINTF((fp_A60501, "[ModeRegInit_LP4] for CH%u Rank%u\n", u1ChannelIdx, u4RankIdx));    
            #if VENDER_JV_LOG
            mcSHOW_DBG_MSG5(("\n[ModeRegInit_LP4] CH%u Rank%u\n", u1ChannelIdx, u4RankIdx)); 
            #endif
           ...
           // if(p->frequency<=1200)
            ...
            //else
            {
                ...
                if(p->dram_type == TYPE_LPDDR4P)  //(p->odt_onoff)
                    DramcModeRegWrite(p, 0xb, 0x0);     //ODT
                else
...
static DRAM_STATUS_T UpdateInitialSettings_LP4(DRAMC_CTX_T *p)
{
    ...
    vIO32WriteFldAlign(DDRPHY_B0_DQ5, 0xe, B0_DQ5_RG_RX_ARDQ_EYE_VREF_SEL_B0);
    ...
    {
        // LP4x eye fine-tune
        vIO32WriteFldAlign_All(DDRPHY_B0_DQ8, 0x1, B0_DQ8_RG_TX_ARDQ_EN_LP4P_B0);
        vIO32WriteFldAlign_All(DDRPHY_B1_DQ8, 0x1, B1_DQ8_RG_TX_ARDQ_EN_LP4P_B1); 
        vIO32WriteFldAlign_All(DDRPHY_CA_CMD9, 0x1, CA_CMD9_RG_TX_ARCMD_EN_LP4P);    
    }
    ...
    vIO32WriteFldAlign(DDRPHY_SHU1_CA_CMD7, 0x09E0, SHU1_CA_CMD7_RG_ARCMD_REV);
    ...
    vIO32WriteFldMulti(DDRPHY_SHU1_CA_CMD8, P_Fld(0x1, SHU1_CA_CMD8_K_R_DMRANK_CHG_PIPE_CG_IG_CA)
    ...
    if(mt_get_chip_sw_ver()==CHIP_SW_VER_02) //Kibo/Kibo+
    ...
    vIO32WriteFldAlign(DRAMC_REG_PERFCTL0, 0x1, PERFCTL0_REORDEREN);
    vIO32WriteFldAlign(DRAMC_REG_PERFCTL0, 0x1, PERFCTL0_RWSPLIT);
    ...
    if(p->vendor_id==VENDOR_HYNIX)
    {
        ...
        vIO32WriteFldMulti(DDRPHY_SHU1_B0_DQ7, P_Fld(1, SHU1_B0_DQ7_DQ_REV_B0_BIT_09_READ_BASE_EN)
                                            | P_Fld(!p->odt_onoff, SHU1_B0_DQ7_DQ_REV_B0_BIT_08_ODT_DISABLE)
                                            | P_Fld(0, SHU1_B0_DQ7_DQ_REV_B0_BIT_01_DQS_PULL_DN)
                                            | P_Fld(0, SHU1_B0_DQ7_DQ_REV_B0_BIT_00_DQSB_PULL_UP));
        vIO32WriteFldMulti(DDRPHY_SHU1_B1_DQ7, P_Fld(1, SHU1_B1_DQ7_DQ_REV_B1_BIT_09_READ_BASE_EN)
                                            | P_Fld(!p->odt_onoff, SHU1_B1_DQ7_DQ_REV_B1_BIT_08_ODT_DISABLE)
                                            | P_Fld(0, SHU1_B1_DQ7_DQ_REV_B1_BIT_01_DQS_PULL_DN)
                                            | P_Fld(0, SHU1_B1_DQ7_DQ_REV_B1_BIT_00_DQSB_PULL_UP));
        ...
        vIO32WriteFldMulti(DDRPHY_SHU1_B0_DQ7, P_Fld(0, SHU1_B0_DQ7_DQ_REV_B0_BIT_09_READ_BASE_EN)
                                            | P_Fld(0, SHU1_B0_DQ7_DQ_REV_B0_BIT_08_ODT_DISABLE)
                                            | P_Fld(1, SHU1_B0_DQ7_DQ_REV_B0_BIT_01_DQS_PULL_DN)
                                            | P_Fld(1, SHU1_B0_DQ7_DQ_REV_B0_BIT_00_DQSB_PULL_UP));
        vIO32WriteFldMulti(DDRPHY_SHU1_B1_DQ7, P_Fld(0, SHU1_B1_DQ7_DQ_REV_B1_BIT_09_READ_BASE_EN)
                                            | P_Fld(0, SHU1_B1_DQ7_DQ_REV_B1_BIT_08_ODT_DISABLE)
                                            | P_Fld(1, SHU1_B1_DQ7_DQ_REV_B1_BIT_01_DQS_PULL_DN)
                                            | P_Fld(1, SHU1_B1_DQ7_DQ_REV_B1_BIT_00_DQSB_PULL_UP));
        #else
        vIO32WriteFldMulti(DDRPHY_SHU1_B0_DQ7, P_Fld(0, SHU1_B0_DQ7_DQ_REV_B0_BIT_09_READ_BASE_EN)
                                            | P_Fld(0, SHU1_B0_DQ7_DQ_REV_B0_BIT_08_ODT_DISABLE)
                                            | P_Fld(0, SHU1_B0_DQ7_DQ_REV_B0_BIT_01_DQS_PULL_DN)
...
static DRAM_STATUS_T UpdateInitialSettings_LP3(DRAMC_CTX_T *p)
{
    ...
    //Darren-vIO32WriteFldAlign(DRAMC_REG_DRAMCTRL, 0x0, DRAMCTRL_CLKWITRFC);
    vIO32WriteFldAlign(DRAMC_REG_DRAMCTRL, 0x1, DRAMCTRL_TMRR2WDIS);
...
// Darren++++++
    ...
    vIO32WriteFldMulti(DDRPHY_CA_DLL_ARPI1, P_Fld(0x0, CA_DLL_ARPI1_RG_ARPI_MCTL_JUMP_EN_CA)
    ...
    vIO32WriteFldMulti(DDRPHY_B0_DLL_ARPI1, P_Fld(0x0, B0_DLL_ARPI1_RG_ARPI_MCTL_JUMP_EN_B0)
    ...
    vIO32WriteFldMulti(DDRPHY_B1_DLL_ARPI1, P_Fld(0x0, B1_DLL_ARPI1_RG_ARPI_MCTL_JUMP_EN_B1)
    ...
    vIO32WriteFldMulti(DDRPHY_CA_DLL_ARPI1+(1<<POS_BANK_NUM), P_Fld(0x0, CA_DLL_ARPI1_RG_ARPI_MCTL_JUMP_EN_CA)
                | P_Fld(0x0, CA_DLL_ARPI1_RG_ARPI_FB_JUMP_EN_CA)
                | P_Fld(0x0, CA_DLL_ARPI1_RG_ARPI_CS_JUMP_EN) 
                | P_Fld(0x0, CA_DLL_ARPI1_RG_ARPI_CLK_JUMP_EN)
                | P_Fld(0x0, CA_DLL_ARPI1_RG_ARPI_CMD_JUMP_EN)
                | P_Fld(0x0, CA_DLL_ARPI1_RG_ARPI_CLKIEN_JUMP_EN));
    vIO32WriteFldMulti(DDRPHY_B0_DLL_ARPI1+(1<<POS_BANK_NUM), P_Fld(0x0, B0_DLL_ARPI1_RG_ARPI_MCTL_JUMP_EN_B0)
                | P_Fld(0x0, B0_DLL_ARPI1_RG_ARPI_FB_JUMP_EN_B0)
                | P_Fld(0x0, B0_DLL_ARPI1_RG_ARPI_DQS_JUMP_EN_B0) 
                | P_Fld(0x0, B0_DLL_ARPI1_RG_ARPI_DQM_JUMP_EN_B0)
                | P_Fld(0x0, B0_DLL_ARPI1_RG_ARPI_DQ_JUMP_EN_B0)
                | P_Fld(0x0, B0_DLL_ARPI1_RG_ARPI_DQSIEN_JUMP_EN_B0));
    vIO32WriteFldMulti(DDRPHY_B1_DLL_ARPI1+(1<<POS_BANK_NUM), P_Fld(0x0, B1_DLL_ARPI1_RG_ARPI_MCTL_JUMP_EN_B1)
                | P_Fld(0x0, B1_DLL_ARPI1_RG_ARPI_FB_JUMP_EN_B1)
                | P_Fld(0x0, B1_DLL_ARPI1_RG_ARPI_DQS_JUMP_EN_B1) 
                | P_Fld(0x0, B1_DLL_ARPI1_RG_ARPI_DQM_JUMP_EN_B1)
                | P_Fld(0x0, B1_DLL_ARPI1_RG_ARPI_DQ_JUMP_EN_B1)
...
static void DramcSetting_Olympus_LP3(DRAMC_CTX_T *p)
{
    ...
    //chg_mem_en = 1
    vIO32WriteFldAlign(DDRPHY_MISC_CG_CTRL0, 0x1, MISC_CG_CTRL0_MEM_EN);
...
#if (fcFOR_CHIP_ID == fcOlympus)    
    vIO32WriteFldAlign(DDRPHY_CKMUX_SEL, 0x1, CKMUX_SEL_R_PHYCTRLMUX);
#endif    
...
static void DramcSetting_Olympus_LP4_ByteMode_DDR2667(DRAMC_CTX_T *p)
{
    ...
    vIO32WriteFldAlign(DRAMC_REG_SHU_DQS2DQ_TX, 0x7, SHU_DQS2DQ_TX_FREQ_RATIO_TX);
    ...
    //            | P_Fld(0x1, SHU1_CA_CMD6_RG_ARPI_MIDPI_EN_CA));
    //*((volatile unsigned int *)(DRAMC_WBR)) = DRAMC_BROADCAST_ON; 
    //vIO32WriteFldAlign(DDRPHY_SHU1_PLL5, 0x3300, SHU1_PLL5_RG_RPHYPLL_SDM_PCW);
    //vIO32WriteFldAlign(DDRPHY_SHU1_PLL7, 0x3300, SHU1_PLL7_RG_RCLRPLL_SDM_PCW);
...
static void DramcSetting_Olympus_LP4_ByteMode_DDR1600(DRAMC_CTX_T *p)
{
    ...
    vIO32WriteFldAlign(DRAMC_REG_SHU_DQS2DQ_TX, 0x4, SHU_DQS2DQ_TX_FREQ_RATIO_TX);
    ...
    else
    {
        vIO32WriteFldMulti(DRAMC_REG_SHURK0_SELPH_DQSG0, P_Fld(0x2, SHURK0_SELPH_DQSG0_TX_DLY_DQS1_GATED_P1) 
                    ...
                    | P_Fld(0x2, SHURK0_SELPH_DQSG0_TX_DLY_DQS0_GATED_P1) 
                    | P_Fld(0x1, SHURK0_SELPH_DQSG0_TX_DLY_DQS0_GATED));
        vIO32WriteFldMulti(DRAMC_REG_SHURK0_SELPH_DQSG1, P_Fld(0x2, SHURK0_SELPH_DQSG1_REG_DLY_DQS1_GATED_P1) 
                    | P_Fld(0x6, SHURK0_SELPH_DQSG1_REG_DLY_DQS1_GATED) 
    ...
    {
        vIO32WriteFldMulti(DRAMC_REG_SHURK1_SELPH_DQSG0, P_Fld(0x2, SHURK1_SELPH_DQSG0_TX_DLY_R1DQS1_GATED_P1) 
                    | P_Fld(0x2, SHURK1_SELPH_DQSG0_TX_DLY_R1DQS1_GATED) 
                    | P_Fld(0x2, SHURK1_SELPH_DQSG0_TX_DLY_R1DQS0_GATED_P1) 
                    | P_Fld(0x2, SHURK1_SELPH_DQSG0_TX_DLY_R1DQS0_GATED));
...
static void DramcSetting_Olympus_LP4_ByteMode(DRAMC_CTX_T *p)
{
    ...
    *((volatile unsigned int *)(DRAMC_WBR)) = DRAMC_BROADCAST_ON;
    ...
    vIO32WriteFldMulti(DDRPHY_B1_DQ2, P_Fld(0x0, B1_DQ2_RG_TX_ARDQ_OE_DIS_B1) 
                ...
                | P_Fld(0x0, B1_DQ2_RG_TX_ARDQS1_OE_DIS) 
                | P_Fld(0x0, B1_DQ2_RG_TX_ARDQS1_ODTEN_DIS));
    vIO32WriteFldAlign(DDRPHY_MISC_RXDVS1, 0x00000007, MISC_RXDVS1_R_IN_GATE_EN_LOW_OPT); //sagy
    ...
    *((volatile unsigned int *)(DRAMC_WBR)) = DRAMC_BROADCAST_OFF;
    ...
    vIO32WriteFldAlign(DDRPHY_CA_DLL_ARPI1+(1<<POS_BANK_NUM), 0x0, CA_DLL_ARPI1_RG_ARPISM_MCK_SEL_CA);//CH_B CA slave mode
    *((volatile unsigned int *)(DRAMC_WBR)) = DRAMC_BROADCAST_ON;
#else
    vIO32WriteFldAlign(DDRPHY_CA_DLL_ARPI1, 0x1, CA_DLL_ARPI1_RG_ARPISM_MCK_SEL_CA);
#endif
    ...
    vIO32WriteFldMulti(DDRPHY_B1_DLL_ARPI2, P_Fld(0x0, B1_DLL_ARPI2_RG_ARPI_CG_MCK_B1) 
        ...
        *((volatile unsigned int *)(DRAMC_WBR)) = DRAMC_BROADCAST_OFF;
        vIO32WriteFldMulti(DDRPHY_CA_DLL_ARPI3, P_Fld(0x1, CA_DLL_ARPI3_RG_ARPI_MCTL_EN_CA) 
                    ...
                    | P_Fld(0x1, CA_DLL_ARPI3_RG_ARPI_CLK_EN) 
                    | P_Fld(0x1, CA_DLL_ARPI3_RG_ARPI_CMD_EN));
        vIO32WriteFldMulti(DDRPHY_CA_DLL_ARPI3+(1<<POS_BANK_NUM), P_Fld(0x0, CA_DLL_ARPI3_RG_ARPI_MCTL_EN_CA) //CH_B CA slave
                    | P_Fld(0x1, CA_DLL_ARPI3_RG_ARPI_FB_EN_CA) 
                    | P_Fld(0x1, CA_DLL_ARPI3_RG_ARPI_CS_EN) 
                    | P_Fld(0x1, CA_DLL_ARPI3_RG_ARPI_CLK_EN) 
                    | P_Fld(0x1, CA_DLL_ARPI3_RG_ARPI_CMD_EN));
    ...
    *((volatile unsigned int *)(DRAMC_WBR)) = DRAMC_BROADCAST_OFF;
    vIO32WriteFldAlign(DDRPHY_SHU1_CA_CMD6, 0x3, SHU1_CA_CMD6_RG_ARPI_RESERVE_CA);
    vIO32WriteFldAlign(DDRPHY_SHU1_CA_CMD6+(1<<POS_BANK_NUM), 0x1, SHU1_CA_CMD6_RG_ARPI_RESERVE_CA);
    //vIO32WriteFldAlign(DDRPHY_SHU1_CA_CMD6+(1<<POS_BANK_NUM), 0x9, SHU1_CA_CMD6_RG_ARPI_RESERVE_CA); //MTK_Darren+ for HQA
    *((volatile unsigned int *)(DRAMC_WBR)) = DRAMC_BROADCAST_ON;
#else
    vIO32WriteFldAlign(DDRPHY_SHU1_CA_CMD6, 0x3, SHU1_CA_CMD6_RG_ARPI_RESERVE_CA);
#endif
    ...
    *((volatile unsigned int *)(DRAMC_WBR)) = DRAMC_BROADCAST_OFF;
    ...
    vIO32WriteFldMulti(DDRPHY_CA_DLL_ARPI0+(1<<POS_BANK_NUM), P_Fld(0x0, CA_DLL_ARPI0_RG_ARDLL_PD_CK_SEL_CA)
                | P_Fld(0x1, CA_DLL_ARPI0_RG_ARDLL_FASTPJ_CK_SEL_CA));//CH_B CA slave mode
    *((volatile unsigned int *)(DRAMC_WBR)) = DRAMC_BROADCAST_ON;
#else
    vIO32WriteFldMulti(DDRPHY_CA_DLL_ARPI0, P_Fld(0x1, CA_DLL_ARPI0_RG_ARDLL_PD_CK_SEL_CA) | P_Fld(0x0, CA_DLL_ARPI0_RG_ARDLL_FASTPJ_CK_SEL_CA));
#endif
    ...
    */
    ...
    vIO32WriteFldMulti(DRAMC_REG_SHUCTRL2, P_Fld(0x0, SHUCTRL2_HWSET_WLRL) 
                ...
                | P_Fld(0x1, SHUCTRL2_R_DVFS_DLL_CHA) 
                | P_Fld(0xa, SHUCTRL2_R_DLL_IDLE));
    vIO32WriteFldMulti(DRAMC_REG_DVFSDLL, P_Fld(0x13, DVFSDLL_DLL_IDLE_SHU4) 
                | P_Fld(0x13, DVFSDLL_DLL_IDLE_SHU3) 
                | P_Fld(0xb, DVFSDLL_DLL_IDLE_SHU2) 
                | P_Fld(0x1, DVFSDLL_DLL_LOCK_SHU_EN));
    ...
    if(vGet_Dram_CBT_Mode(p)==CBT_BYTE_MODE1)
    {
        ...
        vIO32WriteFldAlign(DRAMC_REG_SPCMDCTRL, 0x1, SPCMDCTRL_CLR_EN);
        vIO32WriteFldAlign(DRAMC_REG_SHU_SCINTV, 0x15, SHU_SCINTV_MRW_INTV);
    ...
    else
    {
        vIO32WriteFldAlign(DRAMC_REG_DRAMCTRL, 0x0, DRAMCTRL_CTOREQ_HPRI_OPT);
        vIO32WriteFldMulti(DRAMC_REG_PERFCTL0, P_Fld(0x1, PERFCTL0_REORDEREN) 
                    | P_Fld(0x1, PERFCTL0_REORDER_MODE));
        vIO32WriteFldAlign(DRAMC_REG_RSTMASK, 0x0, RSTMASK_GT_SYNC_MASK);
        vIO32WriteFldAlign(DRAMC_REG_RKCFG, 0x1, RKCFG_DQSOSC2RK);
        //vIO32WriteFldAlign(DRAMC_REG_MRS, 0x1, MRS_MPCRK);
        vIO32WriteFldMulti(DRAMC_REG_MRS, P_Fld(1, MRS_MPCRKEN) | P_Fld(0, MRS_MPCRK));
        vIO32WriteFldMulti(DRAMC_REG_HW_MRR_FUN, P_Fld(0x1, HW_MRR_FUN_TRPMRR_EN) 
                    | P_Fld(0x1, HW_MRR_FUN_TRCDMRR_EN));
        vIO32WriteFldAlign(DRAMC_REG_EYESCAN, 0x1, EYESCAN_RG_RX_MIOCK_JIT_EN);
    ...
    vIO32WriteFldMulti(DDRPHY_MISC_CTRL1, P_Fld(0x0, MISC_CTRL1_R_DMRXDVS_SHUREG_EN) 
                | P_Fld(0x1, MISC_CTRL1_R_DMDQMDBI));
    vIO32WriteFldMulti(DRAMC_REG_SHU_RANKCTL, P_Fld(0x1, SHU_RANKCTL_PICGLAT) 
                | P_Fld(0x0, SHU_RANKCTL_DMSTBLAT));
    vIO32WriteFldMulti(DRAMC_REG_REFCTRL1, P_Fld(0x1, REFCTRL1_REF_QUE_AUTOSAVE_EN) 
...
//francis remove                            | P_Fld(0x1, SHU1_PLL7_RG_RCLRPLL_SDM_PCW_CHG));
        ...
        vIO32WriteFldMulti(DRAMC_REG_SHU_AC_TIME_05T, P_Fld(0x0, SHU_AC_TIME_05T_TWTR_M05T) 
                ...
                | P_Fld(0x0, SHU_AC_TIME_05T_TRFC_05T) 
                | P_Fld(0x1, SHU_AC_TIME_05T_TRC_05T));
        vIO32WriteFldAlign(DRAMC_REG_SHU_DQS2DQ_TX, 0x0, SHU_DQS2DQ_TX_FREQ_RATIO_TX);
        vIO32WriteFldMulti(DRAMC_REG_SHURK2_SELPH_DQSG1, P_Fld(0x4, SHURK2_SELPH_DQSG1_REG_DLY_R2DQS1_GATED_P1) 
                | P_Fld(0x0, SHURK2_SELPH_DQSG1_REG_DLY_R2DQS1_GATED) 
                | P_Fld(0x4, SHURK2_SELPH_DQSG1_REG_DLY_R2DQS0_GATED_P1) 
                | P_Fld(0x0, SHURK2_SELPH_DQSG1_REG_DLY_R2DQS0_GATED));
...
///TODO: DVFS_Enable
    ...
    *((volatile unsigned int *)(DRAMC_WBR)) = DRAMC_BROADCAST_OFF;    
    ...
    vIO32WriteFldMulti(DRAMC_REG_SHUCTRL+(1<<POS_BANK_NUM), P_Fld(0x0, SHUCTRL_R_DRAMC_CHA) | P_Fld(0x1, SHUCTRL_SHU_PHYRST_SEL));

    vIO32WriteFldMulti(DRAMC_REG_SHUCTRL2, P_Fld(0x1, SHUCTRL2_R_DVFS_DLL_CHA));
    vIO32WriteFldMulti(DRAMC_REG_SHUCTRL2+(1<<POS_BANK_NUM), P_Fld(0x0, SHUCTRL2_R_DVFS_DLL_CHA));
    *((volatile unsigned int *)(DRAMC_WBR)) = DRAMC_BROADCAST_ON;
    ...
    *((volatile unsigned int *)(DRAMC_WBR)) = DRAMC_BROADCAST_OFF;
    ...
    vIO32WriteFldAlign(DRAMC_REG_DRAMC_PD_CTRL+(1<<POS_BANK_NUM), 0x1, DRAMC_PD_CTRL_DCMEN2);
    ...
    {   U8 channel_idx;
            ...
            {
                if(p->frequency == 1600 || p->frequency == 1333)
                {
...
#define TOTAL_AC_TIMING_NUMBER  8
...
{
    U8 u1TRFC=0, u1TRFC_05T=0, u1TXREFCNT=0;
        
    if(u1IsLP4Family(p->dram_type) && (p->density !=0))  //already read MR8 for density update
    {
        switch(p->density)
        {
...
DRAM_STATUS_T DdrUpdateACTimingReg(DRAMC_CTX_T *p, const U32* ACTable)
{
    ...
    vIO32WriteFldMulti_All(DRAMC_REG_ADDR(DRAMC_REG_SHU_ACTIM2),  
                                    ...
                                    P_Fld( ACTable[AC_TIMING_TRTW_ODT_OFF + 1], SHU_ACTIM2_TR2W)               |
    ...
    vIO32WriteFldMulti_All(DRAMC_REG_ADDR(DRAMC_REG_SHU_ACTIM2),  
                                    P_Fld( ACTable[AC_TIMING_TFAW], SHU_ACTIM2_TFAW)         | 
                                    P_Fld( ACTable[AC_TIMING_TRTW_ODT_OFF + r2w_odt_onoff], SHU_ACTIM2_TR2W)               |
                                    P_Fld( ACTable[AC_TIMING_TRTP], SHU_ACTIM2_TRTP)         |    
                                    P_Fld( ACTable[AC_TIMING_TXP], SHU_ACTIM2_TXP));
    }
    ...
    vIO32WriteFldAlign_All(DRAMC_REG_ADDR(DRAMC_REG_SHURK1_DQSCTL), ACTable[AC_TIMING_DQSINCTL_FOR_GATING], SHURK1_DQSCTL_R1DQSINCTL); //Rank1 DQSINCTL, no use in A-PHY.
...
#if ((!SW_CHANGE_FOR_SIMULATION) && (!FOR_DV_SIMULATION_USED) && (!__ETT__))
DRAM_STATUS_T DdrUpdateACTiming_EMI(DRAMC_CTX_T *p, AC_TIMING_EXTERNAL_T *ACRegFromEmi)
{
   ...
   if(ACRegFromEmi == NULL)
    ...
    ACTiming[AC_TIMING_DMCATRAIN_INTV] =  ACTimingTable[u1TimingIdx][AC_TIMING_DMCATRAIN_INTV]; //sagy
...
{        
...
#if (fcFOR_CHIP_ID == fcOlympus) || (fcFOR_CHIP_ID == fcElbrus)
    ...
    //Darren+  for DRAM normal, byte and mixed mode
    if ((p->dram_cbt_mode[RANK_0] == CBT_NORMAL_MODE) && (p->dram_cbt_mode[RANK_1] == CBT_NORMAL_MODE))
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_RSTMASK), 0, ARBCTL_RSV_DRAM_CBT_MIXED); 
    else if ((p->dram_cbt_mode[RANK_0] == CBT_BYTE_MODE1) && (p->dram_cbt_mode[RANK_1] == CBT_BYTE_MODE1))
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_RSTMASK), 1, ARBCTL_RSV_DRAM_CBT_MIXED); 
    else if ((p->dram_cbt_mode[RANK_0] == CBT_NORMAL_MODE) && (p->dram_cbt_mode[RANK_1] == CBT_BYTE_MODE1))
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_RSTMASK), 2, ARBCTL_RSV_DRAM_CBT_MIXED); 
    else if ((p->dram_cbt_mode[RANK_0] == CBT_BYTE_MODE1) && (p->dram_cbt_mode[RANK_1] == CBT_NORMAL_MODE))
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_RSTMASK), 3, ARBCTL_RSV_DRAM_CBT_MIXED); 
    else
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_RSTMASK), 0, ARBCTL_RSV_DRAM_CBT_MIXED);
...
void DramcImpedenceTrackingEnable(DRAMC_CTX_T *p)
{
    ...
    vIO32WriteFldMulti(DRAMC_REG_IMPCAL, P_Fld(1, IMPCAL_IMPCAL_HW) | P_Fld(0, IMPCAL_IMPCAL_EN) | P_Fld(1, IMPCAL_IMPCAL_SWVALUE_EN) | P_Fld(1, IMPCAL_IMPCAL_NEW_OLD_SL));
    
    vIO32WriteFldAlign(DRAMC_REG_REFCTRL0+(1<<POS_BANK_NUM), 1, REFCTRL0_DRVCGWREF);
    vIO32WriteFldAlign_All(DRAMC_REG_REFCTRL0, 1, REFCTRL0_K_DQDRVSWUPD);   

    vIO32WriteFldAlign_All(DRAMC_REG_IMPCAL, 1, IMPCAL_K_IMPCAL_BYPASS_UP_CA_DRV);    
}
#endif
...
{
    U32 cha_backupReg0x64, cha_backupReg0xC8, cha_backupReg0xD0;
    U32 chb_backupReg0x64, chb_backupReg0xC8, chb_backupReg0xD0;

    cha_backupReg0x64 = u4IO32Read4B(DRAMC_REG_SPCMDCTRL);
    cha_backupReg0xC8 = u4IO32Read4B(DRAMC_REG_DQSOSCR);
    cha_backupReg0xD0 = u4IO32Read4B(DRAMC_REG_DUMMY_RD);
    chb_backupReg0x64 = u4IO32Read4B(DRAMC_REG_SPCMDCTRL + (1<<POS_BANK_NUM));
    chb_backupReg0xC8 = u4IO32Read4B(DRAMC_REG_DQSOSCR + (1<<POS_BANK_NUM));
    chb_backupReg0xD0 = u4IO32Read4B(DRAMC_REG_DUMMY_RD + (1<<POS_BANK_NUM));    
    ...
    //Restore backup regs
    vIO32Write4B(DRAMC_REG_SPCMDCTRL, cha_backupReg0x64);
    vIO32Write4B(DRAMC_REG_DQSOSCR, cha_backupReg0xC8);
    vIO32Write4B(DRAMC_REG_DUMMY_RD, cha_backupReg0xD0);
    vIO32Write4B(DRAMC_REG_SPCMDCTRL + (1<<POS_BANK_NUM), chb_backupReg0x64);
    vIO32Write4B(DRAMC_REG_DQSOSCR + (1<<POS_BANK_NUM), chb_backupReg0xC8);
    vIO32Write4B(DRAMC_REG_DUMMY_RD + (1<<POS_BANK_NUM), chb_backupReg0xD0);
...
{
    U32 u4WbrBackup = 0;
    u4WbrBackup = *((UINT32P)DRAMC_WBR);
    *((UINT32P)DRAMC_WBR) = DRAMC_BROADCAST_OFF;
    ...
    vIO32WriteFldAlign(DDRPHY_MISC_SPM_CTRL2, 0xffffffff, MISC_SPM_CTRL2_PHY_SPM_CTL2);
    vIO32WriteFldAlign(DDRPHY_MISC_SPM_CTRL2 + (1 << POS_BANK_NUM), 0x7fffffff, MISC_SPM_CTRL2_PHY_SPM_CTL2);//Lewis@20160805: Change CHB CA DLL to slave mode, fix LP4 LPM(DDR1600) MD EE in S0(suspend, deep idle, SODI)

    *((UINT32P)DRAMC_WBR) = u4WbrBackup;
    return;
}
...
void Switch26MHzDisableDummyReadRefreshAllBank(DRAMC_CTX_T *p)
{
    ...
    vIO32WriteFldAlign_All(DDRPHY_MISC_CG_CTRL0, 1, MISC_CG_CTRL0_MEM_EN);
    vIO32WriteFldAlign_All(DDRPHY_MISC_CG_CTRL0, 0, MISC_CG_CTRL0_MEM_EN);
...
void EnableDramcPhyDCM(DRAMC_CTX_T *p, bool bEn)
{
    ...
    *((UINT32P)DRAMC_WBR) = DRAMC_BROADCAST_OFF;
    ...
    {   
        ...
        //mem_dcm
        //*((UINT32P)(0x10001078)) &= ~((0x1 << 7) | (0x1 << 6));
        vIO32Write4B(0x10001078, 0x806003BE);
        vIO32Write4B(0x10001078, 0x806003BF);
        vIO32Write4B(0x10001078, 0x806003BE);        
    }
    else
    {
        ...
        //mem_dcm
        //*((UINT32P)(0x10001078)) |= ((0x1 << 7) | (0x1 << 6));
        vIO32Write4B(0x10001078, 0x8060037E);
        vIO32Write4B(0x10001078, 0x8060037F);
        vIO32Write4B(0x10001078, 0x8060037E);        
    }
    if(p->dram_type == TYPE_LPDDR3)//CHB dramc clock off 
    {
        vIO32WriteFldAlign(DDRPHY_MISC_CG_CTRL0 + (0x1 << POS_BANK_NUM), 1, MISC_CG_CTRL0_K_RG_CG_DRAMC_CHB_CK_OFF);        
    }
    else
    {
        vIO32WriteFldAlign(DDRPHY_MISC_CG_CTRL0 + (0x1 << POS_BANK_NUM), 0, MISC_CG_CTRL0_K_RG_CG_DRAMC_CHB_CK_OFF);        
    }
    //Disable CHANNEL_EMI DCM for MD latency issue
    vIO32WriteFldAlign_All(DDRPHY_MISC_CG_CTRL0, 0x1, MISC_CG_CTRL0_RG_CG_EMI_OFF_DISABLE); // Darren+ disable chn emi dcm
...
#if(fcFOR_CHIP_ID == fcOlympus)
        vIO32Write4B(DDRPHY_CA_DLL_ARPI3+(1<<POS_BANK_NUM), 0x3A000);//Olympus: Synchronous mode, CHB M_CK is no use
#elif(fcFOR_CHIP_ID == fcElbrus)
        vIO32Write4B(DDRPHY_CA_DLL_ARPI3+(1<<POS_BANK_NUM), 0xBA000);//Elbrus: Asynchronous mode, CHB M_CK needs open
#else
        mcSHOW_ERR_MSG(("!Confirm CHB PHY 0x18C setting\n"));
#endif
    }
    else
    {
        vIO32Write4B(DDRPHY_CA_DLL_ARPI3, 0xBA000);
        vIO32Write4B(DDRPHY_CA_DLL_ARPI3+(1<<POS_BANK_NUM), 0x3A800);    
    }
...
void HwSaveForSR(DRAMC_CTX_T *p)
{
    if(u1IsLP4Family(p->dram_type))
    {
        ...
        vIO32WriteFldMulti_All(DRAMC_REG_SREFCTRL, P_Fld(0, SREFCTRL_SREF2_OPTION)
...
//  mcSHOW_DBG_MSG(("=== [SetCKE2RankIndependent] ===\n"));
    ...
    vIO32WriteFldMulti_All(DRAMC_REG_CKECTRL, P_Fld(0, CKECTRL_K_CKE2RANK_OPT3)
                    | P_Fld(0, CKECTRL_K_CKE2RANK_OPT5)
                    | P_Fld(0, CKECTRL_K_CKE2RANK_OPT6)
                    | P_Fld(1, CKECTRL_K_CKE2RANK_OPT7)
                    | P_Fld(0, CKECTRL_K_CKE2RANK_OPT8)
...
#if TX_DLY_DCM_ENABLE
static void SetTxDlyDcmEnable(DRAMC_CTX_T *p)//Save more active power
{
    if(u1IsLP4Family(p->dram_type))
    {
        vIO32WriteFldMulti_All(DDRPHY_MISC_CG_CTRL5, 
            P_Fld(1, MISC_CG_CTRL5_K_R_CA_PI_DCM_EN) | P_Fld(1, MISC_CG_CTRL5_K_R_CA_DLY_DCM_EN) |
            P_Fld(1, MISC_CG_CTRL5_K_R_DQ0_DLY_DCM_EN) | P_Fld(1, MISC_CG_CTRL5_K_R_DQ1_DLY_DCM_EN)
            );
    }
    else
    {
        ...
        vIO32WriteFldMulti(DDRPHY_MISC_CG_CTRL5, 
            P_Fld(1, MISC_CG_CTRL5_K_R_CA_PI_DCM_EN) | P_Fld(1, MISC_CG_CTRL5_K_R_CA_DLY_DCM_EN) |
            P_Fld(1, MISC_CG_CTRL5_K_R_DQ0_DLY_DCM_EN) | P_Fld(1, MISC_CG_CTRL5_K_R_DQ1_DLY_DCM_EN) |
            P_Fld(0, MISC_CG_CTRL5_K_R_DQ0_PI_DCM_EN) | P_Fld(1, MISC_CG_CTRL5_K_R_DQ1_PI_DCM_EN) 
            );
        vIO32WriteFldMulti(((U32)CHANNEL_B<<POS_BANK_NUM) + DDRPHY_MISC_CG_CTRL5, 
            P_Fld(0, MISC_CG_CTRL5_K_R_CA_PI_DCM_EN) | P_Fld(1, MISC_CG_CTRL5_K_R_CA_DLY_DCM_EN) |
            P_Fld(1, MISC_CG_CTRL5_K_R_DQ0_DLY_DCM_EN) | P_Fld(1, MISC_CG_CTRL5_K_R_DQ1_DLY_DCM_EN) |
            P_Fld(0, MISC_CG_CTRL5_K_R_DQ0_PI_DCM_EN) | P_Fld(0, MISC_CG_CTRL5_K_R_DQ1_PI_DCM_EN) 
            );
    }
}
#endif

#if TX_RX_CG_PIPELINE_ENABLE
static void SetTxRxCgPipelineEnable(DRAMC_CTX_T *p)
{
    if(u1IsLP4Family(p->dram_type))
    {
        vIO32WriteFldMulti_All(DDRPHY_SHU1_B0_DQ8, 
            P_Fld(0, SHU1_B0_DQ8_K_R_DMRANK_CHG_PIPE_CG_IG_B0) | P_Fld(0, SHU1_B0_DQ8_K_R_DMRANK_PIPE_CG_IG_B0) |
            P_Fld(0, SHU1_B0_DQ8_K_R_DMRDSEL_TOG_PIPE_CG_IG_B0) | P_Fld(0, SHU1_B0_DQ8_K_R_DMRDSEL_PIPE_CG_IG_B0) |
            P_Fld(0, SHU1_B0_DQ8_K_R_DMDQSIEN_FLAG_PIPE_CG_IG_B0) | P_Fld(0, SHU1_B0_DQ8_K_R_DMDQSIEN_FLAG_SYNC_CG_IG_B0) |
            P_Fld(0, SHU1_B0_DQ8_K_R_DMSTBEN_SYNC_CG_IG_B0) | P_Fld(0, SHU1_B0_DQ8_K_R_DMRXDLY_CG_IG_B0) |
            P_Fld(0, SHU1_B0_DQ8_K_R_DMRXDVS_RDSEL_TOG_PIPE_CG_IG_B0) | P_Fld(0, SHU1_B0_DQ8_K_R_DMRXDVS_RDSEL_PIPE_CG_IG_B0) |
            P_Fld(1, SHU1_B0_DQ8_K_R_DMRX_TOPHY_CG_IG_B0) | P_Fld(0, SHU1_B0_DQ8_K_R_DMRODTEN_CG_IG_B0) 
            );

        vIO32WriteFldMulti_All(DDRPHY_SHU1_B1_DQ8, 
            P_Fld(0, SHU1_B1_DQ8_K_R_DMRANK_CHG_PIPE_CG_IG_B1) | P_Fld(0, SHU1_B1_DQ8_K_R_DMRANK_PIPE_CG_IG_B1) |
            P_Fld(0, SHU1_B1_DQ8_K_R_DMRDSEL_TOG_PIPE_CG_IG_B1) | P_Fld(0, SHU1_B1_DQ8_K_R_DMRDSEL_PIPE_CG_IG_B1) |
            P_Fld(0, SHU1_B1_DQ8_K_R_DMDQSIEN_FLAG_PIPE_CG_IG_B1) | P_Fld(0, SHU1_B1_DQ8_K_R_DMDQSIEN_FLAG_SYNC_CG_IG_B1) |
            P_Fld(0, SHU1_B1_DQ8_K_R_DMSTBEN_SYNC_CG_IG_B1) | P_Fld(0, SHU1_B1_DQ8_K_R_DMRXDLY_CG_IG_B1) |
            P_Fld(0, SHU1_B1_DQ8_K_R_DMRXDVS_RDSEL_TOG_PIPE_CG_IG_B1) | P_Fld(0, SHU1_B1_DQ8_K_R_DMRXDVS_RDSEL_PIPE_CG_IG_B1) |
            P_Fld(1, SHU1_B1_DQ8_K_R_DMRX_TOPHY_CG_IG_B1) | P_Fld(0, SHU1_B1_DQ8_K_R_DMRODTEN_CG_IG_B1) 
            );

        vIO32WriteFldMulti_All(DDRPHY_SHU1_CA_CMD8, 
            P_Fld(0, SHU1_CA_CMD8_K_R_DMRANK_CHG_PIPE_CG_IG_CA) | P_Fld(0, SHU1_CA_CMD8_K_R_DMRANK_PIPE_CG_IG_CA) |
            P_Fld(0, SHU1_CA_CMD8_K_R_DMRDSEL_TOG_PIPE_CG_IG_CA) | P_Fld(0, SHU1_CA_CMD8_K_R_DMRDSEL_PIPE_CG_IG_CA) |
            P_Fld(0, SHU1_CA_CMD8_K_R_DMDQSIEN_FLAG_PIPE_CG_IG_CA) | P_Fld(0, SHU1_CA_CMD8_K_R_DMDQSIEN_FLAG_SYNC_CG_IG_CA) |
            P_Fld(0, SHU1_CA_CMD8_K_R_DMSTBEN_SYNC_CG_IG_CA) | P_Fld(0, SHU1_CA_CMD8_K_R_DMRXDLY_CG_IG_CA) |
            P_Fld(0, SHU1_CA_CMD8_K_R_DMRXDVS_RDSEL_TOG_PIPE_CG_IG_CA) | P_Fld(0, SHU1_CA_CMD8_K_R_DMRXDVS_RDSEL_PIPE_CG_IG_CA) |
            P_Fld(1, SHU1_CA_CMD8_K_R_DMRX_TOPHY_CG_IG_CA) | P_Fld(0, SHU1_CA_CMD8_K_R_DMRODTEN_CG_IG_CA) 
            );

        vIO32WriteFldAlign_All(DDRPHY_MISC_CTRL3, 0, MISC_CTRL3_K_R_DDRPHY_COMB_CG_IG);
    }
    else
    {
        //CHB
        vIO32WriteFldMulti(DDRPHY_SHU1_B0_DQ8 + (1<<POS_BANK_NUM), 
            P_Fld(0, SHU1_B0_DQ8_K_R_DMRANK_CHG_PIPE_CG_IG_B0) | P_Fld(0, SHU1_B0_DQ8_K_R_DMRANK_PIPE_CG_IG_B0) |
            P_Fld(0, SHU1_B0_DQ8_K_R_DMRDSEL_TOG_PIPE_CG_IG_B0) | P_Fld(0, SHU1_B0_DQ8_K_R_DMRDSEL_PIPE_CG_IG_B0) |
            P_Fld(0, SHU1_B0_DQ8_K_R_DMDQSIEN_FLAG_PIPE_CG_IG_B0) | P_Fld(0, SHU1_B0_DQ8_K_R_DMDQSIEN_FLAG_SYNC_CG_IG_B0) |
            P_Fld(0, SHU1_B0_DQ8_K_R_DMSTBEN_SYNC_CG_IG_B0) | P_Fld(0, SHU1_B0_DQ8_K_R_DMRXDLY_CG_IG_B0) |
        ...
        //CHB
        vIO32WriteFldMulti(DDRPHY_SHU1_CA_CMD8 + (1<<POS_BANK_NUM), 
            P_Fld(0, SHU1_CA_CMD8_K_R_DMRANK_CHG_PIPE_CG_IG_CA) | P_Fld(0, SHU1_CA_CMD8_K_R_DMRANK_PIPE_CG_IG_CA) |
            P_Fld(0, SHU1_CA_CMD8_K_R_DMRDSEL_TOG_PIPE_CG_IG_CA) | P_Fld(0, SHU1_CA_CMD8_K_R_DMRDSEL_PIPE_CG_IG_CA) |
            P_Fld(0, SHU1_CA_CMD8_K_R_DMDQSIEN_FLAG_PIPE_CG_IG_CA) | P_Fld(0, SHU1_CA_CMD8_K_R_DMDQSIEN_FLAG_SYNC_CG_IG_CA) |
            P_Fld(0, SHU1_CA_CMD8_K_R_DMSTBEN_SYNC_CG_IG_CA) | P_Fld(0, SHU1_CA_CMD8_K_R_DMRXDLY_CG_IG_CA) |
...
#if DDRPHY_PI_CLOCK_CG_IMPROVEMENT 
static void SetDDRPhyPiCgImprovement(DRAMC_CTX_T *p)
{
    int shuIdx = 0;
    for(shuIdx=DRAM_DFS_SHUFFLE_1; shuIdx<=DRAM_DFS_SHUFFLE_4; shuIdx++)
    {
        vIO32WriteFldAlign_All(DDRPHY_SHU1_CA_CMD0+(0x500*shuIdx), 1, SHU1_CA_CMD0_K_RG_ARPISM_MCK_SEL_CA);
        vIO32WriteFldMulti_All(DDRPHY_SHU1_B0_DQ0+(0x500*shuIdx), P_Fld(1, SHU1_B0_DQ0_K_RG_ARPISM_MCK_SEL_B0) |
            P_Fld(1, SHU1_B0_DQ0_RG_TX_ARPI_CG_DQM_CTRL_B0) |
            P_Fld(1, SHU1_B0_DQ0_RG_TX_ARPI_CG_DQ_CTRL_B0) 
            );
        vIO32WriteFldMulti_All(DDRPHY_SHU1_B1_DQ0+(0x500*shuIdx), P_Fld(1, SHU1_B1_DQ0_K_RG_ARPISM_MCK_SEL_B1) |
            P_Fld(1, SHU1_B1_DQ0_RG_TX_ARPI_CG_DQM_CTRL_B1) |
            P_Fld(1, SHU1_B1_DQ0_RG_TX_ARPI_CG_DQ_CTRL_B1)
            );

        if(!u1IsLP4Family(p->dram_type)) //LP3
        {
            vIO32WriteFldMulti_All(DDRPHY_SHU1_CA_CMD0+(0x500*shuIdx), P_Fld(1, SHU1_CA_CMD0_RG_TX_ARPI_CG_DQM_CTRL) |
                P_Fld(1, SHU1_CA_CMD0_RG_TX_ARPI_CG_DQ_CTRL) 
                );
        }
    }
}
...
void DramcRunTimeConfig(DRAMC_CTX_T *p)
{
    ...
    if(u1IsLP4Family(p->dram_type) && mt_get_chip_hw_ver()!=0xCA00) //LP4 Olympus E1 can't open
    {
        U8 backup_channel;
        backup_channel = p->channel;
        vSetPHY2ChannelMapping(p, CHANNEL_A);
...
#ifdef HW_GATING
    if(p->dram_type == TYPE_LPDDR3|| (u1IsLP4Family(p->dram_type) && mt_get_chip_hw_ver()!=0xCA00)) //LP4 Olympus E1 can't open
    {
...
#if TX_DLY_DCM_ENABLE
    SetTxDlyDcmEnable(p);
    mcSHOW_DBG_MSG(("TX_DLY_DCM_ENABLE: ON\n"));
#else
    mcSHOW_DBG_MSG(("TX_DLY_DCM_ENABLE: OFF\n"));
#endif

#if TX_RX_CG_PIPELINE_ENABLE
    SetTxRxCgPipelineEnable(p);
    mcSHOW_DBG_MSG(("TX_RX_CG_PIPELINE_ENABLE: ON\n"));
#else
    mcSHOW_DBG_MSG(("TX_RX_CG_PIPELINE_ENABLE: OFF\n"));
#endif

#if DDRPHY_PI_CLOCK_CG_IMPROVEMENT
    SetDDRPhyPiCgImprovement(p);
    mcSHOW_DBG_MSG(("DDRPHY_PI_CLOCK_CG_IMPROVEMENT: ON\n"));
#else
    mcSHOW_DBG_MSG(("DDRPHY_PI_CLOCK_CG_IMPROVEMENT: OFF\n"));
#endif
    mcSHOW_DBG_MSG(("=========================\n"));
}
...
//    if ((log2loopcount > 15) || (log2loopcount < 0))		// U8 >=0 always.
    ...
    if (testaudpat == 2)   // xtalk
    {
        ...
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_TEST2_3), P_Fld(0, TEST2_3_TESTAUDPAT)|P_Fld(log2loopcount,TEST2_3_TESTCNT)); //don¡¦t use audio pattern

        // set addr 0x48[16] to 1, TESTXTALKPAT = 1
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_TEST2_4), P_Fld(1, TEST2_4_TESTXTALKPAT)|P_Fld(0,TEST2_4_TESTAUDMODE)|P_Fld(0,TEST2_4_TESTAUDBITINV));  //use XTALK pattern, don¡¦t use audio pattern

        // R_DMTESTSSOPAT=0, R_DMTESTSSOXTALKPAT=0
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_TEST2_4), P_Fld(0, TEST2_4_TESTSSOPAT)|P_Fld(0,TEST2_4_TESTSSOXTALKPAT));   //don¡¦t use sso, sso+xtalk pattern      
    }
...
DRAM_STATUS_T DramcRegDump(DRAMC_CTX_T *p)
{
    ...
    mcSHOW_DBG_MSG(("\n=================PHY_A_BASE================================\n"));
    DramcRegDumpRange(Channel_A_PHY_BASE_ADDRESS, Channel_A_PHY_BASE_ADDRESS+0x1FCC);
    mcSHOW_DBG_MSG(("\n=================PHY_B_BASE================================\n"));
    DramcRegDumpRange(Channel_B_PHY_BASE_ADDRESS, Channel_B_PHY_BASE_ADDRESS+0x1FCC);
...
DRAM_STATUS_T NoDramDramcRegDump(DRAMC_CTX_T *p)
{
    ...
    mcSHOW_DBG_MSG(("\n//=================PHY_A_BASE================================\n"));
    NoDramDramcRegDumpRange(Channel_A_PHY_BASE_ADDRESS, Channel_A_PHY_BASE_ADDRESS+0x1FCC);
    mcSHOW_DBG_MSG(("\n//=================PHY_B_BASE================================\n"));
    NoDramDramcRegDumpRange(Channel_B_PHY_BASE_ADDRESS, Channel_B_PHY_BASE_ADDRESS+0x1FCC);
...
DRAM_TA_PATTERN_T last_testpat;
    ...
    if (last_testpat != TA_PATTERN_TA43)
    {
        ...
        //if #0 == #1 goto -13*4 (¦A©¹«e 8) (label_new_rndseed_loop_start)
...
void DramcDFSUpdateFreqRatio(DRAMC_CTX_T *p)
{
    ...
    //francis todo
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_SHU_DQS2DQ_TX), u1FreqRatioTX[0], SHU_DQS2DQ_TX_FREQ_RATIO_TX);
...
{
    {DDRPHY_BASE_ADDR+(0x0C00), DDRPHY_BASE_ADDR+(0x0C1C) },
    {DDRPHY_BASE_ADDR+(0x0C80), DDRPHY_BASE_ADDR+(0x0C9C) },
    {DDRPHY_BASE_ADDR+(0x0D00), DDRPHY_BASE_ADDR+(0x0D1C) },
...
{
#ifdef HW_GATING
    if(p->dram_type == TYPE_LPDDR3|| (u1IsLP4Family(p->dram_type) && mt_get_chip_hw_ver()!=0xCA00))
    {
...
void DramcDFSDirectJump(DRAMC_CTX_T *p, U8 shu_level)
{
    ...
    vIO32WriteFldAlign_All(DRAMC_REG_SHUCTRL, 0, SHUCTRL_R_OTHER_SHU_GP);
    ...
    #if ENABLE_TX_TRACKING
    //vIO32WriteFldAlign_All(DRAMC_REG_ADDR(DRAMC_REG_SLP4_TESTMODE), 1, SLP4_TESTMODE_K_DQSOSCENDIS);
    //vIO32WriteFldAlign_All(DRAMC_REG_ADDR(DRAMC_REG_DQSOSCR), 1, DQSOSCR_DQSOSCENDIS);
    //vIO32WriteFldAlign_All(DRAMC_REG_ADDR(DRAMC_REG_DQSOSCR), 1, DQSOSCR_DQSOSCRDIS);    
    #endif

    if(*((volatile unsigned int *)(0x10212c08)))
    {
        mcSHOW_DBG_MSG(("DMA ongoing\n"));
    }
    else
    {
        mcSHOW_DBG_MSG(("DMA is done\n"));
    }
...
void DramcRxInputDlyTrackingRG(DRAMC_CTX_T *p)
{
    ...
    backup_rank = u1GetRank(p);
        ...
        if (p->frequency == 1600 || p->frequency == 1866)
        {
            vIO32WriteFldAlign_All(DRAMC_REG_ADDR(DDRPHY_SHU1_B0_DQ5),0x3, SHU1_B0_DQ5_RG_RX_ARDQS0_DVS_DLY_B0);
            vIO32WriteFldAlign_All(DRAMC_REG_ADDR(DDRPHY_SHU1_B1_DQ5),0x3, SHU1_B1_DQ5_RG_RX_ARDQS0_DVS_DLY_B1);
        }
...
{
    U32 u4CLK_DBG_CFG = *((volatile unsigned int *)(0x1000010c));
    U32 u4CLK26CALI_0 = *((volatile unsigned int *)(0x10000220));
    U8  u1CLKMuxSel = 57;
    U32 u4CalCnt;
    U32 u4Freq;
    ...
    //AD_MPLL_208M_CK
    u1CLKMuxSel=50;
    *((volatile unsigned int *)(0x1000010c)) = (u4CLK_DBG_CFG & 0xFF00FFFC) | (u1CLKMuxSel<<16);    
    *((volatile unsigned int *)(0x10000104)) &= 0x00FFFFFF;
    *((volatile unsigned int *)(0x10000220)) = (u4CLK26CALI_0 & 0xFFFFEFEF) | (0x1<<4 | 0x1<<12);
    while((*((volatile unsigned int *)(0x10000220)) & 0x00000010)!=0) { mcDELAY_US(1); }
    u4CalCnt = *((volatile unsigned int *)(0x10000224)) & 0x0000FFFF;
    u4Freq = u4CalCnt*26/1024;
    mcSHOW_DBG_MSG(("AD_MPLL_208M_CK FREQUENCY=%d\n", u4Freq));    
    ...
    *((volatile unsigned int *)(0x10000220)) = (u4CLK26CALI_0 & 0xFFFFEFEF) | (0x1<<4 | 0x1<<12);
    ...
    u4CalCnt = *((volatile unsigned int *)(0x10000224)) & 0x0000FFFF;
    u4Freq = u4CalCnt*26/1024;
    mcSHOW_DBG_MSG(("DA_MPLL_52M_DIV_CK FREQUENCY=%d\n", u4Freq));    


    //FMEM_CK_BFE_DCM_CH0
    //vIO32WriteFldAlign(DDRPHY_MISC_CG_CTRL0, 0x1, Fld(1,21,AC_MSKB2));
    vIO32WriteFldAlign(DDRPHY_MISC_CTRL1, 0x1, MISC_CTRL1_CK_BFE_DCM_EN);
    vIO32WriteFldAlign(DDRPHY_SHU1_PLL9, 0x1, SHU1_PLL9_RG_RPHYPLL_MONCK_EN);
    
    u1CLKMuxSel=25;
    *((volatile unsigned int *)(0x1000010c)) = (u4CLK_DBG_CFG & 0xFFFF00FC) | ((u1CLKMuxSel<<8) | 0x1);
    *((volatile unsigned int *)(0x10000104)) &= 0x00FFFFFF;
    *((volatile unsigned int *)(0x10000220)) = (u4CLK26CALI_0 & 0xFFFFEFEF) | (0x1<<4 | 0x1<<12);
    while((*((volatile unsigned int *)(0x10000220)) & 0x00000010)!=0) { mcDELAY_US(1); }
    u4CalCnt = *((volatile unsigned int *)(0x10000224)) & 0x0000FFFF;
    u4Freq = u4CalCnt*26/1024;
    mcSHOW_DBG_MSG(("FMEM_CK_BEF_DCM_CH0 FREQUENCY=%d\n", u4Freq));        
    ...
    *((volatile unsigned int *)(0x10000220)) = (u4CLK26CALI_0 & 0xFFFFEFEF) | (0x1<<4 | 0x1<<12);
    ...
    u4CalCnt = *((volatile unsigned int *)(0x10000224)) & 0x0000FFFF;
    u4Freq = u4CalCnt*26/1024;
    mcSHOW_DBG_MSG(("FMEM_CK_AFT_DCM_CH0 FREQUENCY=%d\n", u4Freq));    
    //vIO32WriteFldAlign(DDRPHY_MISC_CG_CTRL0, 0x0, Fld(1,21,AC_MSKB2));
    vIO32WriteFldAlign(DDRPHY_MISC_CTRL1, 0x0, MISC_CTRL1_CK_BFE_DCM_EN);
    vIO32WriteFldAlign(DDRPHY_SHU1_PLL9, 0x0, SHU1_PLL9_RG_RPHYPLL_MONCK_EN);
}
#elif (fcFOR_CHIP_ID == fcElbrus)
/*
1. Select fqmeter input to ckgen_dbgmux. CLK_CFG_20[14] = 0 or 1 (0: abist_clk_01, 1: abist_clk_02)(addr: 0x10210200)
2. Select meter clock input. CLK_CFG_20[13:8] for abist_clk_01 (addr:  0x10210200)
                                             CLK_CFG_20[12:8] for abist_clk_02
3. setup meter div. CLK_MISC_CFG_1[7:0] = 0 (addr: 0x10210414)
4. Trigger freq meter. CLK26CALI_0[0] = 1(addr: 0x10210520)
4.5
5. Read meter count CLK26CALI_1[15:0](addr: 0x10210524)
6. Calculate measured freq. freq = (26 * cal_cnt) / 1024;
*/
void DDRPhyFreqMeter()
{
    U32 u4CLK_CFG_20 = *((volatile unsigned int *)(0x10210200));
    ...
    //AD_MPLL_208M_CK
    u1CLKMuxSel=30;
    *((volatile unsigned int *)(0x10210200)) = (u4CLK_CFG_20 & 0xFF80FFFF) | (u1CLKMuxSel<<8);    
    *((volatile unsigned int *)(0x10210414)) &= 0xFFFFFF00;
    *((volatile unsigned int *)(0x10210520)) = (u4CLK26CALI_0 & 0xFFFFFF7E) | ((0x1)|(0x1)<<7);
    ...
    vIO32WriteFldAlign(DDRPHY_MISC_CTRL1, 0x1, MISC_CTRL1_CK_BFE_DCM_EN);
    vIO32WriteFldAlign(DDRPHY_SHU1_PLL9, 0x1, SHU1_PLL9_RG_RPHYPLL_MONCK_EN);
    *((volatile unsigned int *)(0x10210200)) = (u4CLK_CFG_20 & 0xFF80FFFF) | ((u1CLKMuxSel<<8) | (0x1)<<14);    
    *((volatile unsigned int *)(0x10210414)) &= 0xFFFFFF00;
    *((volatile unsigned int *)(0x10210520)) = (u4CLK26CALI_0 & 0xFFFFFF7E) | ((0x1)|(0x1)<<7);
    while((*((volatile unsigned int *)(0x10210520)) & 0x00000001)!=0) { mcDELAY_US(1); }
    u4CalCnt = *((volatile unsigned int *)(0x10210524)) & 0x0000FFFF;
    u4Freq = u4CalCnt*26/1024;
    mcSHOW_DBG_MSG(("DRAMC_CH0_CGCLK FREQUENCY=%d\n", u4Freq));    
    vIO32WriteFldAlign(DDRPHY_MISC_CG_CTRL0, 0x0, Fld(1,21,AC_MSKB2));
    vIO32WriteFldAlign(DDRPHY_MISC_CTRL1, 0x0, MISC_CTRL1_CK_BFE_DCM_EN);
    vIO32WriteFldAlign(DDRPHY_SHU1_PLL9, 0x0, SHU1_PLL9_RG_RPHYPLL_MONCK_EN);
}
...
{
#if 0
    U8 u1ShuLevel = u4IO32ReadFldAlign(SPM_DRAMC_DPY_CLK_SW_CON2, DRAMC_DPY_CLK_SW_CON2_SW_DR_SHU_LEVEL);
...
{       
    ...
    *((UINT32P)DRAMC_WBR) = DRAMC_BROADCAST_OFF;
    ...
    {
        vIO32Write4B_All(DDRPHY_CA_DLL_ARPI2, 0x00);
    }
...
void ddrphy_int_lpbk_test_seq (DRAMC_CTX_T *p)
{
    ...
    *((UINT32P)(DRAMC_WBR)) = DRAMC_BROADCAST_ON;
    ...
    *GPIO_DOUT1 = 0x00000000; //PAD_EINT8/9/10
    ...
    #if 0//Rx DQ Delay
    {
        ...
        for(iDelay=1; iDelay<64; iDelay++)
        {
            ...
            for(ch=CHANNEL_A; ch<CHANNEL_MAX; ch++)
            {
                p->channel = ch;
                for (ii=0; ii<4; ii++)
                {
