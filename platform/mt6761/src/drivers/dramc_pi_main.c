...
#if defined(SLT)
    U8 i=0;
    U32 u4backup_vcore, u4now_vcore;
#endif
...
// scy: reduce code size by removing unused LPDDR3 structure
    ...
    Set_MDL_Used_Flag(get_mdl_used);
      ...
      {
          u4backup_vcore = dramc_get_vcore_voltage();
      #if !__ETT__
      ...
      dramc_set_vcore_voltage(737500);
      u4now_vcore = dramc_get_vcore_voltage();
      mcSHOW_DBG_MSG(("Get PI glitch SLT Vcore=%d(x100)\n", u4now_vcore));
      ...
      for(i=0;i<3;i++)
      {
          ...
          dramc_set_vcore_voltage(810000);
          u4now_vcore = dramc_get_vcore_voltage();
          mcSHOW_DBG_MSG(("Get PI glitch SLT Vcore=%d(x100)\n", u4now_vcore));
          DFSInitForCalibration(p);
      for(i=0;i<3;i++)
      ...
      p->freq_sel = gFreqTbl_LP3[DRAM_DFS_SHUFFLE_3].freq_sel;
      DDRPhyFreqSel(p, p->freq_sel);
      dramc_set_vcore_voltage(u4backup_vcore);
      DFSInitForCalibration(p);
      }
