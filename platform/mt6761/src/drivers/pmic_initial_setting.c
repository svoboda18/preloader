...
void pmic_default_voltage(void)
{
	...
	vproc_vol = 800000;
	if (mtk_regulator_set_voltage(&reg_vsram_proc, vsram_proc_vol, vsram_proc_max_uV))
		ret |= (1 << MTK_REGULATOR_VSRAM_OTHERS);

	if (mtk_regulator_set_voltage(&reg_vproc, vproc_vol, vproc_max_uV))
		ret |= (1 << MTK_REGULATOR_VCORE);
#endif
#if 1
...
/* for MT6357 CRV vsram sleep setting */
static void wk_vsram_sleep_set(void)
{
	if (!is_pmic_mrv()) {
		pmic_config_interface(0x170C, 0x1D0, 0x1FF, 0);
		pmic_config_interface(0x19B4, 0xD, 0x7F, 0);
		pmic_config_interface(0x19D2, 0x35, 0x7F, 0);
		/* VSRAM_PROC slew rate setting */
		pmic_config_interface(0x19B6, 0xF0F, 0x7F7F, 0);
		/* VSRAM_OTHERS slew rate setting */
		pmic_config_interface(0x19D4, 0x70F, 0x7F7F, 0);
	}
}
...
void pmic_init_setting(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(init_setting); i++)
        ...
        pmic_config_interface(PMIC_TMA_KEY_ADDR, 0x0,
			      PMIC_TMA_KEY_MASK,
			      PMIC_TMA_KEY_SHIFT);
#endif /* INIT_SETTING_VERIFIED */
	wk_vsram_sleep_set();
	print("[PMIC] pmic_init_setting end. v180413\n");
	/*mt6357_dump_register();*/

	pmic_default_voltage();

	/* Set VPROC's corresponding VSRAM_ as 0.51875V, while chip version 1 and DREQ 1 */
	/* VSRAM_RPOC power souce from VPROC */
