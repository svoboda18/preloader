...
void DDRPhyPLLSetting(DRAMC_CTX_T *p)
{
    ...
    if (p->frequency <= 1200)//Lewis@20161129: Fix DDR1600 S-idle Vcore power is too big problem.
    {
        u1MIDPICAP_SEL = 0;
    }
    else if (p->frequency <= 1333)
    ...
    else if(p->frequency==1600 || p->frequency==400)
    {
        u2SDM_PCW = 0x7b00;
        if(p->frequency==400)
        {
    ...
    {   
    #if !__ETT__
        if ((!u1IsLP4Family(p->dram_type)) && (seclib_get_devinfo_with_index(19) & 0x1))
        {
            u2SDM_PCW = 0x6600;
        }
...
void DDRPhyReservedRGSetting(DRAMC_CTX_T *p)
{
    ...
    if(u1IsLP4Family(p->dram_type) && p->frequency>=1200)
        u1PSMUX_DRV_SEL=1;
...
static DRAM_STATUS_T UpdateInitialSettings_LP4(DRAMC_CTX_T *p)
{
    ...
    vIO32WriteFldMulti(DRAMC_REG_SHU_IMPCAL1, P_Fld(8, SHU_IMPCAL1_IMPCAL_CALICNT) | P_Fld(0x10, SHU_IMPCAL1_IMPCALCNT) | P_Fld(4, SHU_IMPCAL1_IMPCAL_CALEN_CYCLE) | P_Fld((p->frequency*25/8000)+1, SHU_IMPCAL1_IMPCAL_CHKCYCLE));
    ...
    //Merlot don't have this feature, [Cervino DVT]dram auto refersh rate by hardware mr4 for rank0 and rank1 (Berson)
...
static DRAM_STATUS_T UpdateInitialSettings_LP3(DRAMC_CTX_T *p)
{
...
#if (fcFOR_CHIP_ID == fcMerlot) //ChA-B1, ChB-B0/B1/CA are used as RX
    ...
    //Merlot don't have this feature, [Cervino DVT]dram auto refersh rate by hardware mr4 for rank0 and rank1 (Berson)
    vIO32WriteFldAlign_All(DRAMC_REG_SPCMDCTRL, 0x0, SPCMDCTRL_HMR4_TOG_OPT);

    //[fcCervino DVT]RX FIFO debug feature, MP setting should enable debug function
    vIO32WriteFldAlign(DDRPHY_B0_DQ9, 0x1, B0_DQ9_R_DMRXFIFO_STBENCMP_EN_B0);
    vIO32WriteFldAlign(DDRPHY_B1_DQ9, 0x1, B1_DQ9_R_DMRXFIFO_STBENCMP_EN_B1);
...
#if (fcFOR_CHIP_ID == fcMerlot)                                        //before switch clock from 26M to PHY, need to init PHY clock first
    ...
    */
    ...
    //if(p->frequency==933)
    {
        ...
        vIO32WriteFldMulti(DRAMC_REG_SHU_DQSG_RETRY, P_Fld(0x1, SHU_DQSG_RETRY_R_RETRY_USE_BURST_MDOE)
                    ...
                    | P_Fld(0x0, SHU_DQSG_RETRY_R_XSR_RETRY_SPM_MODE)//don't let SPM trigger gating retry
                    #else
