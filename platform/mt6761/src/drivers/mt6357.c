...
void mt6357_wdtdbg_vosel(void)
{
	...
	step_uV = step_uv[(mt6357_regu_info[MTK_REGULATOR_VSRAM_OTHERS].vtype)];
	print("latch VSRAM_OTHERS %d uV\n", vsram_others_min_uV + step_uV * gray_to_binary(val));
...
#endif /*--LDO_SUPPORT--*/
		/* wk for MT6357 CRV/MRV */
		if (is_pmic_mrv()) {
			if (mt6357_regulator[i].id == MTK_REGULATOR_VSRAM_OTHERS)
				mt6357_regulator[i].id = MTK_REGULATOR_VSRAM_PROC;
			else if (mt6357_regulator[i].id == MTK_REGULATOR_VSRAM_PROC)
				mt6357_regulator[i].id = MTK_REGULATOR_VSRAM_OTHERS;
		}
