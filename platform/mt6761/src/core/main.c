...
#define SSPM_CFGREG_AHB_M1_ADDR     (SSPM_SW_RSTN + 0x74)
    ...
    /* Set DELSEL setting*/
    DRV_WriteReg32(SSPM_SRAM_BIST, 0x24F3001B);
#if defined(MTK_AB_OTA_UPDATER)
...
#if SSPM_DBG_MODE
    /* GPIO pinmux - SSPM UART */
    DRV_SetReg32(0x10005300, 0x00550000);

    /* GPIO pinmux - SSPM JTAG */
    DRV_SetReg32(0x10005330, 0x00044444);
#endif
...
static int bldr_load_images(u32 *jump_addr)
{
    ...
    *pass_size_slt_md = size;
    ...
    sspm_dbg("mblock_reserve 0x%x bytes @0x%x to SSPM\n", SSPM_MEM_SIZE, addr);
