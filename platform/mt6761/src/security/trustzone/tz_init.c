...
u32 tee_get_load_addr(u32 maddr)
{
...
#if CFG_GOOGLE_TRUSTY_SUPPORT
    align_addr = (0x1) << __builtin_ctz(PL_TRUSTY_TEE_BASE);
    limit_addr = PL_TRUSTY_TEE_BASE + tee_secmem_size;
    ...
    if (tee_secmem_start != PL_TRUSTY_TEE_BASE) {
    ...
    DBG_MSG("%s align_addr: 0x%llx, limit_addr: 0x%llx\n", MOD, align_addr, limit_addr);
    tee_secmem_start = (u32)mblock_reserve_ext(&bootarg.mblock_info,
        (u64)(tee_secmem_size), (u64)TEE_MEM_ALIGNMENT,
...
static void tee_sec_config(void)
{
...
#if CFG_TEE_SUPPORT
#if CFG_TRUSTKERNEL_TEE_SUPPORT
    tz_sec_mem_init(tee_entry_addr, tee_entry_addr + tee_secmem_size - TKCORE_SHM_SIZE_LIMIT -1, SECURE_OS_MPU_REGION_ID);
    ...
    {
    u32 atf_dram_addr = PL_ATF_BASE;
    atf_dram_addr = atf_dram_addr & ~((1 << EMI_MPU_ALIGN_BITS) - 1);

    DBG_MSG("%s ATF entry addr:0x%x, dram addr:0x%x\n", MOD, BL31_BASE, atf_dram_addr);

    tz_sec_mem_init(atf_dram_addr, atf_dram_addr + PL_ATF_MAX_SIZE - 1, ATF_MPU_REGION_ID);
    DBG_MSG("%s set secure memory protection : 0x%x, 0x%x (%d)\n", MOD, atf_dram_addr,
        atf_dram_addr + PL_ATF_MAX_SIZE - 1, ATF_MPU_REGION_ID);
    }
#endif
}
...
void trustzone_post_init(void)
{
    ...
    teearg->atf_irq_num = (32 + 247); /* reserve SPI ID 247 for ATF log, which is ID 279 */
