...
void tz_sec_mem_init(u32 start, u32 end, u32 mpu_region)
{
	...
	case SECURE_OS_MPU_REGION_ID:
		...
		FORBIDDEN, FORBIDDEN, FORBIDDEN, FORBIDDEN, FORBIDDEN, SEC_RW, FORBIDDEN, FORBIDDEN, \
		FORBIDDEN, FORBIDDEN, FORBIDDEN, SEC_RW, FORBIDDEN, FORBIDDEN, FORBIDDEN, SEC_RW);
		break;
	case ATF_MPU_REGION_ID:
		SET_ACCESS_PERMISSION(region_info.apc, LOCK, \
		FORBIDDEN, FORBIDDEN, FORBIDDEN, FORBIDDEN, FORBIDDEN, FORBIDDEN, FORBIDDEN, FORBIDDEN, \
		FORBIDDEN, FORBIDDEN, FORBIDDEN, FORBIDDEN, FORBIDDEN, FORBIDDEN, FORBIDDEN, SEC_RW);
		break;
	default:
		DBG_MSG("%s Warning - MPU region '%d' is not supported in pre-loader!\n", MOD, mpu_region);
		return;
	}

	region_info.start = (u64) start;
	region_info.end = (u64) end;
	region_info.region = mpu_region;
	DBG_MSG("%s MPU [0x%llx-0x%llx]\n", MOD, region_info.start, region_info.end);
