...
int part_load(blkdev_t *bdev, part_t *part, u32 *addr, u32 offset, u32 *size)
{
    ...
    u8 *cur_part_name = partition_getCurPartName();
    ...
    if (img_auth_required) {
        ...
        #endif
        ret = sec_img_auth_init(cur_part_name, part_hdr->info.name, sec_feature_mask);
        if (ret) {
            pal_log_err("cert vfy fail\n");
            ASSERT(0);
        }
    ...
    ms = get_timer(ms);
    pal_log_err("part: %s img: %s cert vfy(%d ms)\n", cur_part_name, part_hdr->info.name, ms);
#endif
    ...
    ms = get_timer(ms);
    pal_log_err(" part: %s img: %s vfy(%d ms)\n", cur_part_name, part_hdr->info.name, ms);
#endif
