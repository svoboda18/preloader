...
#define CFG_ATF_ADDR    CFG_ATF_ROM_MEMADDR
#define CFG_ATF_SIZE    0x40000     //256KB
...
static void usbdl_init_image(void)
{
	...
	// 0. Init image list
	memcpy(g_image_list[0].partition_name, lk, sizeof(lk));
	g_image_list[0].start_addr = CFG_LK_ADDR - PART_HDR_BUF_SIZE;
	g_image_list[0].length = CFG_LK_SIZE;

	memcpy(g_image_list[1].partition_name, logo, sizeof(logo));
	g_image_list[1].start_addr = CFG_LOGO_ADDR - PART_HDR_BUF_SIZE;
	g_image_list[1].length = CFG_LOGO_SIZE;

	memcpy(g_image_list[2].partition_name, boot, sizeof(boot));
	g_image_list[2].start_addr = CFG_BOOT_ADDR;
	g_image_list[2].length = CFG_BOOT_SIZE;

	memcpy(g_image_list[3].partition_name, atf, sizeof(atf));
	g_image_list[3].start_addr = CFG_ATF_ADDR - PART_HDR_BUF_SIZE;
	g_image_list[3].length = CFG_ATF_SIZE;

	for (i = 0; i < IMAGE_MAX_NUM; i++) {
		print("%s part: %s, addr: 0x%x, size: 0x%x\n", MOD, g_image_list[i].partition_name, g_image_list[i].start_addr, g_image_list[i].length);
	}
}
...
static void usbdl_boot_image(void)
{
	...
	if (!strcmp(img_name, lk)) {
		usbdl_put_word(status);
		print("%s Jump to LK\n", MOD);
		bldr_jump(g_image_list[0].start_addr + PART_HDR_BUF_SIZE, jump_arg, sizeof(boot_arg_t));
	} else if (!strcmp(img_name, atf)) {
		usbdl_put_word(status);
		print("%s Jump to ATF\n", MOD);
		bldr_jump64(g_image_list[0].start_addr + PART_HDR_BUF_SIZE, jump_arg, sizeof(boot_arg_t));
	} else {
		status = 1;
		usbdl_put_word(status);
		print("%s Unknown Jump Image.\n", MOD);
	}
}
...
static void usbdl_send_image(void)
{
	...
	// 1. get partition name
	usbdl_get_data(img_name, 64);
	print("%s SEND_IMAGE: img_name = %s\n", MOD, img_name);

	// 2. get length, which includes sig len
	usbdl_get_dword(&img_len);
	print("%s SEND_IMAGE: img_len = 0x%x\n", MOD, img_len);

	// 3. check lengh and partition name if valid.
	for (i = 0; i < IMAGE_MAX_NUM; i++) {
		if ((!strcmp(img_name, g_image_list[i].partition_name)) && (img_len > 0) && (img_len <= g_image_list[i].length)) {
			print("%s SEND_IMAGE: Verify PASS.\n", MOD);
			if(i == 3) // in atf case, move trustzon.bin to dram buffer then extract atf.
				img_addr = tee1_buf;
			else
				img_addr = g_image_list[i].start_addr;
			break;
		}
	}
	
	print("%s SEND_IMAGE: img_addr = 0x%x\n", MOD, img_addr);
	if (!img_addr) {
		print("%s Unknown image Received.\n", MOD);
		//return;
	}
	...
	print("%s SEND_IMAGE: My Checksum = 0x%x\n", MOD, my_checksum32);

	usbdl_get_dword(&checksum32);
	print("%s SEND_IMAGE: XFLASH checksum = 0x%x\n", MOD, checksum32);
	...
	//usbdl_put_word(status);
      ...
      if(i == 3) {
          ...
          part_hdr_t *part_hdr = (part_hdr_t *)img_addr;
          print("%s MOVE ATF dst=0x%x, src=0x%x, size=0x%x\n", MOD, (g_image_list[i].start_addr+PART_HDR_BUF_SIZE), (img_addr+PART_HDR_BUF_SIZE), part_hdr->info.dsize);
          memcpy( (u8 *)(g_image_list[i].start_addr+PART_HDR_BUF_SIZE), (u8 *)(img_addr+PART_HDR_BUF_SIZE), part_hdr->info.dsize);

          // move secure os to tee address
          #if CFG_TEE_SUPPORT
          part_hdr_t *part_hdr_tee = (u8 *)(img_addr+PART_HDR_BUF_SIZE+(part_hdr->info.dsize));
          u32 maddr = part_hdr_tee->info.maddr;
          maddr = tee_get_load_addr(maddr);
          print("%s MOVE TEE dst=0x%x, src=0x%x, size=0x%x\n", MOD, maddr, (img_addr+PART_HDR_BUF_SIZE+(part_hdr->info.dsize)+PART_HDR_BUF_SIZE), part_hdr_tee->info.dsize);
          memcpy((u8 *)maddr, (u8 *)(img_addr+PART_HDR_BUF_SIZE+(part_hdr->info.dsize)+PART_HDR_BUF_SIZE), part_hdr_tee->info.dsize);

          int ret = tee_verify_image(&maddr);
          if (ret)
              print("%s TEE verify fail\n", MOD);
          tee_set_entry(maddr);
          #endif
      }
}
