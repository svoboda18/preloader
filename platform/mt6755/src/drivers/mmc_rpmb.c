...
int mmc_rpmb_readdata(u16 addr, u16 cnt,u8 *buf)
{
    struct mmc_host *host = mmc_get_host(0);
    struct mmc_card *card = mmc_get_card(0);
    struct mmc_rpmb_cfg rpmb_cfg;
    ...
    ret = mmc_set_part_config(card, val);
    if (ret) {
        print("%s, mmc_set_part_config failed!! (%x)\n", __func__, ret);
        return ret;
    }

    print("%s, mmc_set_part_config done!!\n", __func__);
    ...
    ret = mmc_rpmb_start_req(card, &rpmb_req);
    if (ret) {
        print("%s, mmc_rpmb_part_ops failed!! (%x)\n", __func__, ret);
        return ret;
    }
    ...
    ret = rpmb_cfg.result;
    if(ret){
	print("%s, mmc_rpmb result err!! (%x)\n", __func__, ret);
        return ret;
    }
    data_frame = rpmb_req.data_frame;
    memcpy(buf, data_frame + RPMB_DATA_BEG, RPMB_SZ_DATA);
