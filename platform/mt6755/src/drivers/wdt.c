...
#include "emi_hw.h"
#include "upmu_hw.h" /* for PMIC full reset */
...
#define PMIC_RG_RSV_SWREG_ADDR MT6351_PMIC_RG_RSV_SWREG_ADDR
#endif

int mtk_wdt_is_pmic_full_reset(void)
{
#if CONFIG_WDT_PMIC_FULL_RST_COMMON_API
	...
	/*
	 * PMIC common API may be failed or not supported in
	 * some platforms, e.g., MT6799.
	 *
	 * Use a non-volatile register (reset only after battery is removed)
	 * for PMIC full reset detection.
	 *
	 * In MT6755, use bit 0 in register PMIC_RG_RSV_SWREG.
	 */
