...
static inline void save_atf_log_buf_to_log_store(void)
{
    ...
    ((struct sram_log_header *)SRAM_LOG_ADDR)->dram_buf.atf_log_addr = atf_log_buf_start;
...
u32 tee_get_load_addr(u32 maddr)
{
    ...
    tee_secmem_start = (u32)mblock_reserve_ext(&bootarg.mblock_info,
            (u64)(tee_secmem_size + ATF_LOG_BUFFER_SIZE), (u64)align_addr, limit_addr, 0, "teei-reserved");
    ...
    DBG_MSG("%s atf_log_buf_start: 0x%x, tee_secmem_start: 0x%x\n", MOD, atf_log_buf_start, tee_secmem_start);
...
#if !CFG_TEE_SUPPORT
    {
        u64 limit_addr = 0x80000000;
        atf_arg_t_ptr teearg = (atf_arg_t_ptr)trustzone_get_atf_boot_param_addr();
        atf_log_buf_start = (u32)mblock_reserve_ext(&bootarg.mblock_info,
...
void trustzone_post_init(void)
{
    ...
    teearg->devinfo[2] = seclib_get_devinfo_with_index(E_AREA22);
    ...
    teei_key_param_prepare(TEE_PARAMETER_KEY, (u8 *)teearg->hwuid, sizeof(teearg->hwuid),
                            (u8 *)teearg->HRID, sizeof(teearg->HRID), rpmb_key);
#endif
    /* Program RPMB Key */
