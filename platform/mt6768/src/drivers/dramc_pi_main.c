...
void vSetVcoreByFreq(DRAMC_CTX_T *p)
{
	...
	#else
		#ifdef VOLTAGE_SEL
		vcore = (vcore_voltage_select(p->dram_type, KOPP0)+ vcore_voltage_select(p->dram_type, KOPP1)) >> 1 ;
		if (!vcore)
		#endif
	...
	#else
		 #ifdef VOLTAGE_SEL
                vcore = (vcore_voltage_select(p->dram_type, KOPP1) + vcore_voltage_select(p->dram_type, KOPP2)) >> 1;
                if (!vcore)
                #endif
	...
	#else
		#ifdef VOLTAGE_SEL
                vcore = (vcore_voltage_select(p->dram_type, KOPP2) + vcore_voltage_select(p->dram_type, KOPP4)) >> 1;
		if (!vcore)
                #endif
	...
	#else
		#ifdef VOLTAGE_SEL
        	vcore = vcore_voltage_select(p->dram_type, KOPP0);
                if (!vcore)
                #endif
	...
	#else
		#ifdef VOLTAGE_SEL
                vcore = (vcore_voltage_select(p->dram_type, KOPP0) + vcore_voltage_select(p->dram_type, KOPP2)) >> 1;
                if (!vcore)
                #endif
...
#if ENABLE_READ_DBI
	mcSHOW_DBG_MSG(("!!!! Enable DRAMC R-DBI !!!!\n"));
	/* Read DBI ON */
	vSetRank(p, RANK_0);
	vSetPHY2ChannelMapping(p, CHANNEL_A);
...
#if ENABLE_WRITE_DBI
	mcSHOW_DBG_MSG(("!!!! Enable DRAMC W-DBI !!!!\n"));
	/* Write DBI ON */
	for (channel_idx = CHANNEL_A; channel_idx < p->support_channel_num; channel_idx++)
	{
...
int GetDramInforAfterCalByMRR(DRAMC_CTX_T *p, DRAM_INFO_BY_MRR_T *DramInfo)
{
	...
	for (u1RankIdx = 0; u1RankIdx < (p->support_rank_num); u1RankIdx++)
	{
	...
	#endif
		{
			...
			if (DramInfo != NULL)
				DramInfo->u8MR8RankSize[u1RankIdx] = u8Size;
		}

		mcSHOW_DBG_MSG(("RK%d, DieNum %d, Density %llx, RKsize %llx.\n", u1RankIdx, u1DieNumber, u8Size, p->ranksize[u1RankIdx]));
	}
...
int Init_DRAM(DRAM_DRAM_TYPE_T dram_type, DRAM_CBT_MODE_EXTERN_T dram_cbt_mode_extern, DRAM_INFO_BY_MRR_T *DramInfo, U8 get_mdl_used)
{
	...
	if (u1IsLP4Family(p->dram_type))
	{
		if (doe_get_config("dram_fix_2400_0800") || doe_get_config("dram_fix_2400_0700")
			|| doe_get_config("all_shu_2400_0800") || doe_get_config("all_shu_2400_0700"))
		{
			...
			if (doe_get_config("all_shu_2400_0800") || doe_get_config("all_shu_2400_0700"))
			{
				DramcSaveToShuffleReg(p, DRAM_DFS_SHUFFLE_1, DRAM_DFS_SHUFFLE_3);
			}
		}
		else if(doe_get_config("dram_fix_1534_0700") || doe_get_config("dram_fix_1534_0625")
			|| doe_get_config("all_shu_1534_0700") || doe_get_config("dram_dvs_fix_1534"))
		{
			DramcSaveToShuffleReg(p, DRAM_DFS_SHUFFLE_1, DRAM_DFS_SHUFFLE_2);
			DramcSaveToShuffleReg(p, DRAM_DFS_SHUFFLE_1, DRAM_DFS_SHUFFLE_3);
		}
...
void DPI_SW_main_LP3(void)
{
	...
	DramConfig->enable_rx_scan_vref = DISABLE;
	DramConfig->enable_tx_scan_vref = DISABLE;
	/* 6765 and 6761 have DLP3, need to give it initial value */
	DramConfig->bDLP3 = 0;
	...
	DramcModeRegInit_LP3(DramConfig, 0);
#endif
...
void DPI_SW_main_LP4(void)
{
	...
	DramConfig->support_channel_num = CHANNEL_DUAL;
	DramConfig->channel = CHANNEL_B;
	DramConfig->support_rank_num = RANK_DUAL;
	/* DramRank */
	DramConfig->rank = RANK_0;
