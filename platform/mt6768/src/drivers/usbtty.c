...
usbdl_init (void)
{
    ...
    string->bDescriptorType = USB_DESCRIPTOR_TYPE_STRING;
    str2wide (USBD_DIAG_INTERFACE_STR, string->wData);
#endif
    ...
    mt_usb_diag_alt_if->interface_descriptor = &interface_descriptors[2];
    mt_usb_diag_alt_if->endpoints = NUM_DIAG_ENDPOINTS;
    mt_usb_diag_alt_if->endpoints_descriptor_array = fs_diag_ep_descriptor_ptrs;
#endif
...
#ifdef DIAG_COMPOSITE_PRELOADER
    mt_usb_diag_alt_if->endpoints_descriptor_array = hs_diag_ep_descriptor_ptrs;
#endif
