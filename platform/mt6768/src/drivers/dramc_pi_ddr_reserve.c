...
void Dramc_DDR_Reserved_Mode_setting(void)
{
	...
	{
		vIO32WriteFldAlign(DDRPHY_MISC_CG_CTRL0 + SHIFT_TO_CHB_ADDR, 0, MISC_CG_CTRL0_RG_CG_DRAMC_CHB_CK_OFF); /* Open CHB controller clk */
	}
	/* Backup regs */
	...
	else/* LP3 */
	{
		...
		u4Chb_cke_backup = u4IO32Read4B(DRAMC_REG_CKECTRL + SHIFT_TO_CHB_ADDR);
		/* CKE fix on */
	...
	*((UINT32P)(0x100063b8 )) &= ~(0x1 << 1);
	...
	{
		vIO32WriteFldAlign_All(DDRPHY_B1_DQ5, 0x1, B1_DQ5_RG_RX_ARDQ_VREF_EN_B1);
		vIO32WriteFldAlign(DDRPHY_B0_DQ5 + SHIFT_TO_CHB_ADDR, 0x1, B0_DQ5_RG_RX_ARDQ_VREF_EN_B0);
		vIO32WriteFldAlign(DDRPHY_CA_CMD5 + SHIFT_TO_CHB_ADDR, 0x1, CA_CMD5_RG_RX_ARCMD_VREF_EN);
	}
	/* RG_*MCK8X_EN=1 */
	...
	else
	{
		...
		/* | P_Fld(0x0, CA_DLL_ARPI2_RG_ARPI_CG_CLKIEN)//11th */
			| P_Fld(0x0, CA_DLL_ARPI2_RG_ARPI_MPDIV_CG_CA));
		vIO32WriteFldMulti(DDRPHY_CA_DLL_ARPI2 + SHIFT_TO_CHB_ADDR, P_Fld(0x0, CA_DLL_ARPI2_RG_ARPI_CG_MCK_CA)/* not include 11th bit(CA_DLL_ARPI2_RG_ARPI_CG_CLKIEN) */
			| P_Fld(0x0, CA_DLL_ARPI2_RG_ARPI_CG_MCK_FB2DLL_CA)
			| P_Fld(0x0, CA_DLL_ARPI2_RG_ARPI_CG_MCTL_CA)
			| P_Fld(0x0, CA_DLL_ARPI2_RG_ARPI_CG_FB_CA)
			| P_Fld(0x0, CA_DLL_ARPI2_RG_ARPI_CG_CS)
	...
	*DRAMC_WBR = 0x3;
	...
	mcDELAY_US(1);
	/* 2nd DLL enable */
	vIO32WriteFldAlign(DDRPHY_CA_DLL_ARPI2 + SHIFT_TO_CHB_ADDR, 0x1, CA_DLL_ARPI2_RG_ARDLL_PHDET_EN_CA);
...
void Dramc_DDR_Reserved_Mode_AfterSR(void)
{
	...
	if (u4DramType == 1)
	{   /* LPDDR3 */
		vIO32WriteFldAlign(DDRPHY_MISC_CG_CTRL0 + SHIFT_TO_CHB_ADDR, 1, MISC_CG_CTRL0_RG_CG_DRAMC_CHB_CK_OFF); /* Close CHB controller clk */
	...
	{   /* LPDDR4 */
		...
		for (u1channel = CHANNEL_A; u1channel < p->support_channel_num; u1channel++)
		{
            ...
            if (bSupport2Rank == TRUE)
            {
                ...
                CKEFixOnOff(p, CKE_WRITE_TO_ALL_RANK, CKE_FIXON, CKE_WRITE_TO_ONE_CHANNEL);//CKE fix on with 2 rank
                ...
                DramcModeRegWriteByRank(p, RANK_0, 13, u4LP4_MR13FSP);
                vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_RKCFG), backup_rkcfg, RKCFG_CS2RANK);  
                CKEFixOnOff(p, CKE_WRITE_TO_ALL_RANK, CKE_DYNAMIC, CKE_WRITE_TO_ONE_CHANNEL);//CKE fix on with 2 rank
            }
            else
            {
                DramcModeRegWriteByRank(p, RANK_0, 13, u4LP4_MR13FSP);            
	...
	if (u4DramType == 1)
	{
		...
		vIO32Write4B(DRAMC_REG_CKECTRL + SHIFT_TO_CHB_ADDR, u4Chb_cke_backup);
	}
#endif
#endif
...
#ifdef USE_TA2_IN_DDR_RESERVE_MODE
    mcDELAY_MS(100);//For TX PI reload after MPC(DQSOSC)
	{
		int i = 0;
		for (i = 0; i < 10; i++)
		{
