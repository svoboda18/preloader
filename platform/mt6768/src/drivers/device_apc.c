...
void device_APC_dom_setup(void)
{
	...
	/* SSPM will access its memory region, which is protected as SEC_RW_ONLY by EMI MPU @ preloader.
	 * So this setting is necessary.
	 */
...
// INFRA MASTER DOMAIN settings
	...
	reg_set_field(DEVAPC_AO_INFRA_MAS_DOM_3, SCP, DOMAIN_3);

	/* Set MFG(GPU) to DOMAIN6(MFG) */
	reg_set_field(DEVAPC_AO_INFRA_MAS_DOM_3, MFG, DOMAIN_6);

	/* Set SSPM to DOMAIN8(SSPM) */
	reg_set_field(DEVAPC_AO_INFRA_MAS_DOM_2, SSPM, DOMAIN_8);
...
#if (CFG_GZ_SUPPORT && CFG_GZ_REMAP)
	if (is_booting_el2())
		reg_set_field(DEVAPC_AO_INFRA_DOM_RMP_1, INFRA_AP_HYP, DOMAIN_6); //remap AP_HYP (original domain 11) to domain 6
#endif
