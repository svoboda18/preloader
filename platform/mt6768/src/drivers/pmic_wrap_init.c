...
static signed int _pwrap_init_sistrobe(int dual_si_sample_settings)
{
	...
	if (rdata != DEFAULT_VALUE_READ_TEST) {
		...
		PWRAPERR("Failed, rdata = %x, exp = 0x5aa5\n", rdata);
		return 0x10;
	}
...
static void _pwrap_config_mpu(void)
{
	...
	WRAP_WR32(PMIC_WRAP_MPU_PMIC_RGN0, 0x067E0600);     //End and Start address of Region 0
	WRAP_WR32(PMIC_WRAP_MPU_PMIC_RGN0_PER, 0xffffc3fb); //Permission of Region 0 for Domain 15~0
	WRAP_WR32(PMIC_WRAP_MPU_PMIC_RGN1, 0x10FE0800);     //End and Start address of Region 1
	WRAP_WR32(PMIC_WRAP_MPU_PMIC_RGN1_PER, 0xfffff3c8); //Permission of Region 1 for Domain 15~0
	WRAP_WR32(PMIC_WRAP_MPU_PMIC_RGN2, 0x1EFE1100);     //End and Start address of Region 2
	WRAP_WR32(PMIC_WRAP_MPU_PMIC_RGN2_PER, 0xfffff3c0); //Permission of Region 2 for Domain 15~0
	WRAP_WR32(PMIC_WRAP_MPU_PMIC_RGN3, 0x257E1F00);     //End and Start address of Region 3
	WRAP_WR32(PMIC_WRAP_MPU_PMIC_RGN3_PER, 0xfffffcc8); //Permission of Region 3 for Domain 15~0
	...
	PWRAPLOG("PMIC_WRAP_MPU_PMIC_RGN1 = 0x%x\n", WRAP_RD32(PMIC_WRAP_MPU_PMIC_RGN1));
	PWRAPLOG("PMIC_WRAP_MPU_PMIC_RGN1_PER = 0x%x\n", WRAP_RD32(PMIC_WRAP_MPU_PMIC_RGN1_PER));
	PWRAPLOG("PMIC_WRAP_MPU_PMIC_RGN2 = 0x%x\n", WRAP_RD32(PMIC_WRAP_MPU_PMIC_RGN2));
	PWRAPLOG("PMIC_WRAP_MPU_PMIC_RGN2_PER = 0x%x\n", WRAP_RD32(PMIC_WRAP_MPU_PMIC_RGN2_PER));
	PWRAPLOG("PMIC_WRAP_MPU_PMIC_RGN3 = 0x%x\n", WRAP_RD32(PMIC_WRAP_MPU_PMIC_RGN3));
	PWRAPLOG("PMIC_WRAP_MPU_PMIC_RGN3_PER = 0x%x\n", WRAP_RD32(PMIC_WRAP_MPU_PMIC_RGN3_PER));
}

signed int pwrap_enable_mpu(bool en)
{
	WRAP_WR32(PMIC_WRAP_MPU_PMIC_RGN_EN, 0x0000000f);  //Enable Region 0,1,2,3
...
static unsigned int pwrap_read_test(void)
{
	...
	PWRAPCRI("Read Test pass, return_value = 0x%d\n", return_value);
...
signed int pwrap_init(void)
{
	...
	WRAP_WR32(IOCFG_LT_DRV_CFG1_CLR, 0x07e00000);
	WRAP_WR32(IOCFG_LT_DRV_CFG1_SET, 0x01200000);
	...
	*/
	...
	WRAP_WR32(PMIC_WRAP_MONITOR_TARGET_WRITE, 0x0001); /* only write command for CH0 */
