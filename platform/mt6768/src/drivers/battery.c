...
void fuel_gauge_init(void)
{
	...
	hw_id = upmu_get_reg_value(MT6358_HWCID);
	sw_id = upmu_get_reg_value(MT6358_SWCID);
	...
	if (b_monitor_soff_validtime == 1) {
		...
		print("[fg_init] b_monitor_soff_validtime:%d fg_soff_valid %d, shutdown_pmic_time %d",
	...
	if (do_init_fgadc_reset == 1) {
		/* reset CON1 */
		ret = pmic_config_interface(MT6358_FGADC_CON1, 0x1F04, 0xFFFF, 0x0);
		mdelay(1);
		ret = pmic_config_interface(MT6358_FGADC_CON1, 0x0000, 0x1F04, 0x0);
...
void pl_battery_init(bool force_init)
{
	...
	pal_log_info("[pl_battery_init] is_fg_init:%d , force_init:%d bat:%d\n", 
