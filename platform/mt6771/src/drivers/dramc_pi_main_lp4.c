...
#if TX_K_DQM_WITH_WDBI
        #if SUPPORT_SAVE_TIME_FOR_CALIBRATION
        if(p->pSavetimeData->Bypass_TXWINDOW[_MappingFreqArray[p->freq_sel]] == 0) //if bypass TX K, DQM will be calculate form DQ. (no need to K DQM)
        #endif
        {
...
// scy: reduce code size by removing unused LPDDR3 structure
   ...
   // DramC & PHY init for all channels  
      DDRPhyFreqSel(p, gFreqTbl[DRAM_DFS_SHUFFLE_3].freq_sel);
      vSetVcoreByFreq(p);
