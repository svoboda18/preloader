...
#include "x_hal_io.h"
#else
#include "dramc_common_lp4.h"
#include "x_hal_io_lp4.h"
#endif
...
extern struct mtk_regulator reg_vddq;
#endif
#endif
...
void DDRPhyPLLSetting(DRAMC_CTX_T *p)
{
    ...
    if(u1IsLP4Family(p->dram_type))
    {
        ...
        vIO32WriteFldMulti(DDRPHY_CKMUX_SEL+((U32)CHANNEL_A<<POS_BANK_NUM), P_Fld(0, CKMUX_SEL_FMEM_CK_MUX) | P_Fld(0, CKMUX_SEL_FB_CK_MUX));
        vIO32WriteFldMulti(DDRPHY_CKMUX_SEL+((U32)CHANNEL_B<<POS_BANK_NUM), P_Fld(0, CKMUX_SEL_FMEM_CK_MUX) | P_Fld(0, CKMUX_SEL_FB_CK_MUX));
    ...
    if (p->frequency <= 933) // Only set DLL to all slave mode if DDR speed <= 1866 (for both LP3/LP4)
    {
        ...
        #if (fcFOR_CHIP_ID == fcSylvia)
        //Bi_anco need to set RG_ARPI_RESERVE_CA[1], and not RG_*RDLL_FAST_PSJP_CA
    ...
    ///TODO: MIDPI Init 2
    /* MIDPI Settings (O_lympus): DA_*RPI_MIDPI_EN, DA_*RPI_MIDPI_CKDIV4_EN
    ...
    //if(p->vendor_id==VENDOR_SAMSUNG && p->dram_type==TYPE_LPDDR3)
    {
        ...
        CKEFixOnOff(p, CKE_DYNAMIC, CKE_WRITE_TO_ALL_CHANNEL);
			...
			if((p->frequency == 1866) || (p->frequency == 1600))
			{
				...
				vIO32WriteFldAlign_All(DDRPHY_SHU1_PLL15, 0x0, SHU1_PLL15_RG_RPHYPLL_SDM_SSC_DELTA);
				vIO32WriteFldAlign_All(DDRPHY_SHU1_PLL21, 0x0, SHU1_PLL21_RG_RCLRPLL_SDM_SSC_DELTA);
    ...
    for(u1ChannelIdx=0; u1ChannelIdx<(p->support_channel_num); u1ChannelIdx++)
    {
        ...
        for(u4RankIdx =0; u4RankIdx < (U32)(p->support_rank_num); u4RankIdx++)
        {
            ...
            if(p->dram_type == TYPE_LPDDR4P)
            {
                ...
                #ifndef MT6771_FPGA
                #if (FOR_DV_SIMULATION_USED==0 && SW_CHANGE_FOR_SIMULATION==0)
			mtk_regulator_set_voltage(&reg_vddq, 600000, MAX_VDDQ);
                #endif
                #endif
           ...
           // if(p->frequency<=1200)
            {
                ...
                DramcModeRegWrite(p, 0xd, u1MR13Value); //FSP-0
            ...
            //else
            {
                ...
                DramcModeRegWrite(p, 0xd, u1MR13Value); //FSP-1
                //MR12 use previous value
                DramcModeRegWrite(p, 0xc, u1MR12Value[u1ChannelIdx][u4RankIdx][u1MRFsp]); //MR12 VREF-CA
                DramcModeRegWrite(p, 0x1, u1MR01Value[u1MRFsp]);
                ...
                {
                    //u1MR11Value = 0x3;              //ODT:60ohm
                    if(p->dram_cbt_mode[0]==CBT_NORMAL_MODE)
                    {
                        u1MR11Value = 0x23;              //CA ODT:120ohm for normal mode and mixed mode, DQ ODT:80ohm
                    }
                    else
                    {
                        u1MR11Value = 0x13;              //CA ODT:240ohm for byte mode, DQ ODT:80ohm
                    }
                ...
                else
                {
                    ...
                    else  //TYPE_LPDDR4x, LP4P
                    {
                        ...
                        if(p->dram_cbt_mode[0]==CBT_NORMAL_MODE && p->dram_cbt_mode[1]==CBT_BYTE_MODE1)
                        {
                            if(u4RankIdx==RANK_0)
                                u1MR22Value = 0x3c;     //SOC-ODT, ODTE-CK, ODTE-CS, Disable ODTD-CA
                            else
                                u1MR22Value = 0x34;     //ODTE-CK
                        }
                        else
                        {
                            u1MR22Value = 0x34;     //ODTE-CK
...
static DRAM_STATUS_T UpdateInitialSettings_LP4(DRAMC_CTX_T *p)
{
    ...
    #if 1//#ifndef Bi_anco_TO_BE_PORTING
    ...
    //vIO32WriteFldAlign(DDRPHY_SHU1_CA_DLL0, 0x1, SHU1_CA_DLL0_RG_ARPISM_MCK_SEL_CA_SHU); move to DramcSetting_O_lympus_LP4_ByteMode()
    ...
    #elif SUPPORT_HYNIX_RX_DQS_WEAK_PULL //WDQS is disable
    ...
    //Syl_via MP setting should set CKECTRL_CKELCKFIX as 1 to roolback to M17 (review by CF Chang)
...
static DRAM_STATUS_T UpdateInitialSettings_LP3(DRAMC_CTX_T *p)
{
    ...
    {
        //For JA_DE-like mode, reserve bit5 should be set as "0" before calibration (by Justin)
	...
	//DMSTBLAT should set as 3 for JA_DE-like mode 
    if(p->frequency>=800)
    {
    ...
    vIO32WriteFldMulti_All(DDRPHY_B0_DQ2, P_Fld(0x1, B0_DQ2_RG_TX_ARDQ_ODTEN_DIS_B0) 
                | P_Fld(0x1, B0_DQ2_RG_TX_ARDQS0_ODTEN_DIS));
    vIO32WriteFldMulti_All(DDRPHY_B1_DQ2, P_Fld(0x1, B1_DQ2_RG_TX_ARDQ_ODTEN_DIS_B1) 
                | P_Fld(0x1, B1_DQ2_RG_TX_ARDQS1_ODTEN_DIS));
    ...
    //wei-jen: RX rank_sel for CHA CA is not used(Bi_anco), set it's dly to 0 to save power
    vIO32WriteFldAlign(DDRPHY_SHU1_CA_CMD7, 0, SHU1_CA_CMD7_R_DMRANKRXDVS_CA);
...
static void DramcSetting_Olympus_LP3(DRAMC_CTX_T *p)
{
    ...
    //vIO32WriteFldAlign(DDRPHY_MISC_CTRL0+((U32)CHANNEL_B<<POS_BANK_NUM), 0x1, MISC_CTRL0_R_DMRDSEL_DIV2_OPT); //Use _All() instead
    ...
    */
    ...
    vIO32WriteFldAlign(DDRPHY_B1_DQ9, 0x1, B1_DQ9_RG_RX_ARDQS1_STBEN_RESETB);
    vIO32WriteFldAlign(DDRPHY_CA_CMD10+((U32)CHANNEL_B<<POS_BANK_NUM), 0x1, CA_CMD10_RG_RX_ARCLK_STBEN_RESETB);
    vIO32WriteFldAlign(DDRPHY_B0_DQ9+((U32)CHANNEL_B<<POS_BANK_NUM), 0x1, B0_DQ9_RG_RX_ARDQS0_STBEN_RESETB);
    vIO32WriteFldAlign(DDRPHY_B1_DQ9+((U32)CHANNEL_B<<POS_BANK_NUM), 0x1, B1_DQ9_RG_RX_ARDQS1_STBEN_RESETB);
    ...
    vIO32WriteFldAlign(DRAMC_REG_DRAMCTRL, 0x1, DRAMCTRL_REQQUE_THD_EN);
                //| P_Fld(0x1, DRAMCTRL_DPDRK_OPT)); DPDRK_OPT doesn't exist on Syl_via
...
static void DramcSetting_Olympus_LP4_ByteMode(DRAMC_CTX_T *p)
{
 ...
 //
    ...
    vIO32WriteFldAlign(DDRPHY_B0_DQ9, 0x1, B0_DQ9_RG_RX_ARDQS0_STBEN_RESETB);
    vIO32WriteFldAlign(DDRPHY_B1_DQ9, 0x1, B1_DQ9_RG_RX_ARDQS1_STBEN_RESETB);
    ...
    */
    ...
    vIO32WriteFldAlign(DDRPHY_B1_DQ9, 0x1, B1_DQ9_RG_RX_ARDQS1_DQSIENMODE);
    vIO32WriteFldAlign(DDRPHY_B0_DQ9, 0x1, B0_DQ9_RG_RX_ARDQS0_DQSIENMODE);
    ...
    vIO32WriteFldAlign(DRAMC_REG_SHU_STBCAL, 0x1, SHU_STBCAL_PICGLAT);
//Syl_via MP setting should set SHU_STBCAL_DMSTBLAT as 0x2 (review by HJ Huang)
    if(p->frequency<=800)
    {
...
//francis remove                            | P_Fld(0x1, SHU1_PLL7_RG_RCLRPLL_SDM_PCW_CHG));
    ...
    //vIO32WriteFldAlign(DRAMC_REG_SPCMDCTRL, 0x0, SPCMDCTRL_REFRDIS);//Lewis@20160613: Fix refresh rate is wrong while diable MR4
    vIO32WriteFldAlign(DRAMC_REG_DRAMCTRL, 0x1, DRAMCTRL_REQQUE_THD_EN);
                //| P_Fld(0x1, DRAMCTRL_DPDRK_OPT)); //DPDRK_OPT doesn't exit for Syl_via
...
///TODO: DVFS_Enable
    ...
    //Does Syl_via still require this code snippet?
    #if (fcFOR_CHIP_ID == fcSylvia)
    {
        U8 u1CrossLatch;
        U8 u1Hysteresis;  
...
DRAM_STATUS_T DdrUpdateACTimingReg(DRAMC_CTX_T *p, const ACTime_T *ACTbl)
{
    ...
    // Maybe "Save 1 MCK" will be set after Vinson project, but Bi_anco & Vin_son can not.(different with performance team)
...
void DumpGoldenSetting()
{
    ...
    u4Value = (*(volatile unsigned int *)CHA_DRAMC(0x38));
    mcSHOW_DBG_MSG(("0x%X[2] = 0x%X \n", CHA_DRAMC(0x38), (u4Value >> 2) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_DRAMC(0x38));
    mcSHOW_DBG_MSG(("0x%X[2] = 0x%X \n", CHB_DRAMC(0x38), (u4Value >> 2) & 0x1));

    u4Value = (*(volatile unsigned int *)CHA_DRAMC(0x38));
    mcSHOW_DBG_MSG(("0x%X[31] = 0x%X \n", CHA_DRAMC(0x38), (u4Value >> 31) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_DRAMC(0x38));
    mcSHOW_DBG_MSG(("0x%X[31] = 0x%X \n", CHB_DRAMC(0x38), (u4Value >> 31) & 0x1));

    u4Value = (*(volatile unsigned int *)CHA_DRAMC(0x38));
    mcSHOW_DBG_MSG(("0x%X[30] = 0x%X \n", CHA_DRAMC(0x38), (u4Value >> 30) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_DRAMC(0x38));
    mcSHOW_DBG_MSG(("0x%X[30] = 0x%X \n", CHB_DRAMC(0x38), (u4Value >> 30) & 0x1));

    u4Value = (*(volatile unsigned int *)CHA_DRAMC(0x38));
    mcSHOW_DBG_MSG(("0x%X[0] = 0x%X \n", CHA_DRAMC(0x38), (u4Value >> 0) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_DRAMC(0x38));
    mcSHOW_DBG_MSG(("0x%X[0] = 0x%X \n", CHB_DRAMC(0x38), (u4Value >> 0) & 0x1));

    u4Value = (*(volatile unsigned int *)CHA_DRAMC(0x38));
    mcSHOW_DBG_MSG(("0x%X[5] = 0x%X \n", CHA_DRAMC(0x38), (u4Value >> 5) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_DRAMC(0x38));
    mcSHOW_DBG_MSG(("0x%X[5] = 0x%X \n", CHB_DRAMC(0x38), (u4Value >> 5) & 0x1));

    u4Value = (*(volatile unsigned int *)CHA_DRAMC(0x38));
    mcSHOW_DBG_MSG(("0x%X[1] = 0x%X \n", CHA_DRAMC(0x38), (u4Value >> 1) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_DRAMC(0x38));
    mcSHOW_DBG_MSG(("0x%X[1] = 0x%X \n", CHB_DRAMC(0x38), (u4Value >> 1) & 0x1));

    u4Value = (*(volatile unsigned int *)CHA_PHY(0x284));
    mcSHOW_DBG_MSG(("0x%X[19][17:8] = 0x%X, 0x%X\n", CHA_PHY(0x284), (u4Value >> 19) & 0x1, (u4Value >> 8) & 0x3ff));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x284));
    mcSHOW_DBG_MSG(("0x%X[19][17:8] = 0x%X, 0x%X\n", CHB_PHY(0x284), (u4Value >> 19) & 0x1, (u4Value >> 8) & 0x3ff));

    u4Value = (*(volatile unsigned int *)CHA_PHY(0x284));
    mcSHOW_DBG_MSG(("0x%X[20] = 0x%X\n", CHA_PHY(0x284), (u4Value >> 20) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x284));
    mcSHOW_DBG_MSG(("0x%X[20] = 0x%X\n", CHB_PHY(0x284), (u4Value >> 20) & 0x1));

    u4Value = (*(volatile unsigned int *)CHA_PHY(0x28C));
    mcSHOW_DBG_MSG(("0x%X = 0x%X\n", CHA_PHY(0x28C), u4Value));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x28C));
    mcSHOW_DBG_MSG(("0x%X = 0x%X\n", CHB_PHY(0x28C), u4Value));

    u4Value = (*(volatile unsigned int *)CHA_PHY(0x2A8));
    mcSHOW_DBG_MSG(("0x%X[26] = 0x%X\n", CHA_PHY(0x2A8), (u4Value >> 26) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x2A8));
    mcSHOW_DBG_MSG(("0x%X[26] = 0x%X\n", CHB_PHY(0x2A8), (u4Value >> 26) & 0x1));

    u4Value = (*(volatile unsigned int *)CHA_PHY(0x2A8));
    mcSHOW_DBG_MSG(("0x%X[27] = 0x%X\n", CHA_PHY(0x2A8), (u4Value >> 27) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x2A8));
    mcSHOW_DBG_MSG(("0x%X[27] = 0x%X\n", CHB_PHY(0x2A8), (u4Value >> 27) & 0x1));

    u4Value = (*(volatile unsigned int *)CHA_PHY(0xC20));
    mcSHOW_DBG_MSG(("0x%X[31:20][19] = 0x%X, 0x%X\n", CHA_PHY(0xc20), (u4Value >> 20) & 0xfff, (u4Value >> 19) & 0x1));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0x1120));
    mcSHOW_DBG_MSG(("0x%X[31:20][19] = 0x%X, 0x%X\n", CHA_PHY(0x1120), (u4Value >> 20) & 0xfff, (u4Value >> 19) & 0x1));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0x1620));
    mcSHOW_DBG_MSG(("0x%X[31:20][19] = 0x%X, 0x%X\n", CHA_PHY(0x1620), (u4Value >> 20) & 0xfff, (u4Value >> 19) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0xC20));
    mcSHOW_DBG_MSG(("0x%X[31:20][19] = 0x%X, 0x%X\n", CHB_PHY(0xc20), (u4Value >> 20) & 0xfff, (u4Value >> 19) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x1120));
    mcSHOW_DBG_MSG(("0x%X[31:20][19] = 0x%X, 0x%X\n", CHB_PHY(0x1120), (u4Value >> 20) & 0xfff, (u4Value >> 19) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x1620));
    mcSHOW_DBG_MSG(("0x%X[31:20][19] = 0x%X, 0x%X\n", CHB_PHY(0x1620), (u4Value >> 20) & 0xfff, (u4Value >> 19) & 0x1));

    u4Value = (*(volatile unsigned int *)CHA_PHY(0xCA0));
    mcSHOW_DBG_MSG(("0x%X[31:20][19] = 0x%X, 0x%X\n", CHA_PHY(0xcA0), (u4Value >> 20) & 0xfff, (u4Value >> 19) & 0x1));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0x11A0));
    mcSHOW_DBG_MSG(("0x%X[31:20][19] = 0x%X, 0x%X\n", CHA_PHY(0x11A0), (u4Value >> 20) & 0xfff, (u4Value >> 19) & 0x1));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0x16A0));
    mcSHOW_DBG_MSG(("0x%X[31:20][19] = 0x%X, 0x%X\n", CHA_PHY(0x16A0), (u4Value >> 20) & 0xfff, (u4Value >> 19) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0xCA0));
    mcSHOW_DBG_MSG(("0x%X[31:20][19] = 0x%X, 0x%X\n", CHB_PHY(0xcA0), (u4Value >> 20) & 0xfff, (u4Value >> 19) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x11A0));
    mcSHOW_DBG_MSG(("0x%X[31:20][19] = 0x%X, 0x%X\n", CHB_PHY(0x11A0), (u4Value >> 20) & 0xfff, (u4Value >> 19) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x16A0));
    mcSHOW_DBG_MSG(("0x%X[31:20][19] = 0x%X, 0x%X\n", CHB_PHY(0x16A0), (u4Value >> 20) & 0xfff, (u4Value >> 19) & 0x1));

    u4Value = (*(volatile unsigned int *)CHA_PHY(0xD20));
    mcSHOW_DBG_MSG(("0x%X[31:20][19] = 0x%X, 0x%X\n", CHA_PHY(0xD20), (u4Value >> 20) & 0xfff, (u4Value >> 19) & 0x1));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0x1220));
    mcSHOW_DBG_MSG(("0x%X[31:20][19] = 0x%X, 0x%X\n", CHA_PHY(0x1220), (u4Value >> 20) & 0xfff, (u4Value >> 19) & 0x1));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0x1720));
    mcSHOW_DBG_MSG(("0x%X[31:20][19] = 0x%X, 0x%X\n", CHA_PHY(0x1720), (u4Value >> 20) & 0xfff, (u4Value >> 19) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0xD20));
    mcSHOW_DBG_MSG(("0x%X[31:20][19] = 0x%X, 0x%X\n", CHB_PHY(0xD20), (u4Value >> 20) & 0xfff, (u4Value >> 19) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x1220));
    mcSHOW_DBG_MSG(("0x%X[31:20][19] = 0x%X, 0x%X\n", CHB_PHY(0x1220), (u4Value >> 20) & 0xfff, (u4Value >> 19) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x1720));
    mcSHOW_DBG_MSG(("0x%X[31:20][19] = 0x%X, 0x%X\n", CHB_PHY(0x1720), (u4Value >> 20) & 0xfff, (u4Value >> 19) & 0x1));

    u4Value = (*(volatile unsigned int *)CHA_PHY(0x298));
    mcSHOW_DBG_MSG(("0x%X[18:16] = 0x%X\n", CHA_PHY(0x298), (u4Value >> 16) & 0x7));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x298));
    mcSHOW_DBG_MSG(("0x%X[18:16] = 0x%X\n", CHB_PHY(0x298), (u4Value >> 16) & 0x7));

    u4Value = (*(volatile unsigned int *)CHA_PHY(0x298));
    mcSHOW_DBG_MSG(("0x%X[22:20] = 0x%X\n", CHA_PHY(0x298), (u4Value >> 20) & 0x7));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x298));
    mcSHOW_DBG_MSG(("0x%X[22:20] = 0x%X\n", CHB_PHY(0x298), (u4Value >> 20) & 0x7));

    u4Value = (*(volatile unsigned int *)CHA_PHY(0x84));
    mcSHOW_DBG_MSG(("0x%X[20] = 0x%X\n", CHA_PHY(0x84), (u4Value >> 20) & 0x1));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0x104));
    mcSHOW_DBG_MSG(("0x%X[20] = 0x%X\n", CHA_PHY(0x104), (u4Value >> 20) & 0x1));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0x184));
    mcSHOW_DBG_MSG(("0x%X[20] = 0x%X\n", CHA_PHY(0x184), (u4Value >> 20) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x84));
    mcSHOW_DBG_MSG(("0x%X[20] = 0x%X\n", CHB_PHY(0x84), (u4Value >> 20) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x104));
    mcSHOW_DBG_MSG(("0x%X[20] = 0x%X\n", CHB_PHY(0x104), (u4Value >> 20) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x184));
    mcSHOW_DBG_MSG(("0x%X[20] = 0x%X\n", CHB_PHY(0x184), (u4Value >> 20) & 0x1));

    u4Value = (*(volatile unsigned int *)CHA_PHY(0xC34));
    mcSHOW_DBG_MSG(("0x%X[0] = 0x%X\n", CHA_PHY(0xC34), (u4Value >> 0) & 0x1));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0xCB4));
    mcSHOW_DBG_MSG(("0x%X[0] = 0x%X\n", CHA_PHY(0xCB4), (u4Value >> 0) & 0x1));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0xD34));
    mcSHOW_DBG_MSG(("0x%X[0] = 0x%X\n", CHA_PHY(0xD34), (u4Value >> 0) & 0x1));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0x1134));
    mcSHOW_DBG_MSG(("0x%X[0] = 0x%X\n", CHA_PHY(0x1134), (u4Value >> 0) & 0x1));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0x11B4));
    mcSHOW_DBG_MSG(("0x%X[0] = 0x%X\n", CHA_PHY(0x11B4), (u4Value >> 0) & 0x1));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0x1234));
    mcSHOW_DBG_MSG(("0x%X[0] = 0x%X\n", CHA_PHY(0x1234), (u4Value >> 0) & 0x1));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0x1634));
    mcSHOW_DBG_MSG(("0x%X[0] = 0x%X\n", CHA_PHY(0x1634), (u4Value >> 0) & 0x1));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0x16B4));
    mcSHOW_DBG_MSG(("0x%X[0] = 0x%X\n", CHA_PHY(0x16B4), (u4Value >> 0) & 0x1));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0x1734));
    mcSHOW_DBG_MSG(("0x%X[0] = 0x%X\n", CHA_PHY(0x1734), (u4Value >> 0) & 0x1));

    u4Value = (*(volatile unsigned int *)CHB_PHY(0xC34));
    mcSHOW_DBG_MSG(("0x%X[0] = 0x%X\n", CHB_PHY(0xC34), (u4Value >> 0) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0xCB4));
    mcSHOW_DBG_MSG(("0x%X[0] = 0x%X\n", CHB_PHY(0xCB4), (u4Value >> 0) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0xD34));
    mcSHOW_DBG_MSG(("0x%X[0] = 0x%X\n", CHB_PHY(0xD34), (u4Value >> 0) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x1134));
    mcSHOW_DBG_MSG(("0x%X[0] = 0x%X\n", CHB_PHY(0x1134), (u4Value >> 0) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x11B4));
    mcSHOW_DBG_MSG(("0x%X[0] = 0x%X\n", CHB_PHY(0x11B4), (u4Value >> 0) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x1234));
    mcSHOW_DBG_MSG(("0x%X[0] = 0x%X\n", CHB_PHY(0x1234), (u4Value >> 0) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x1634));
    mcSHOW_DBG_MSG(("0x%X[0] = 0x%X\n", CHB_PHY(0x1634), (u4Value >> 0) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x16B4));
    mcSHOW_DBG_MSG(("0x%X[0] = 0x%X\n", CHB_PHY(0x16B4), (u4Value >> 0) & 0x1));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x1734));
    mcSHOW_DBG_MSG(("0x%X[0] = 0x%X\n", CHB_PHY(0x1734), (u4Value >> 0) & 0x1));

    u4Value = (*(volatile unsigned int *)CHA_PHY(0xC1C));
    mcSHOW_DBG_MSG(("0x%X[19:17] = 0x%X\n", CHA_PHY(0xC1C), (u4Value >> 17) & 0x7));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0xC9C));
    mcSHOW_DBG_MSG(("0x%X[19:17] = 0x%X\n", CHA_PHY(0xC9C), (u4Value >> 17) & 0x7));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0xD1C));
    mcSHOW_DBG_MSG(("0x%X[19:17] = 0x%X\n", CHA_PHY(0xD1C), (u4Value >> 17) & 0x7));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0x111C));
    mcSHOW_DBG_MSG(("0x%X[19:17] = 0x%X\n", CHA_PHY(0x111C), (u4Value >> 17) & 0x7));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0x119C));
    mcSHOW_DBG_MSG(("0x%X[19:17] = 0x%X\n", CHA_PHY(0x119C), (u4Value >> 17) & 0x7));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0x121C));
    mcSHOW_DBG_MSG(("0x%X[19:17] = 0x%X\n", CHA_PHY(0x121C), (u4Value >> 17) & 0x7));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0x161C));
    mcSHOW_DBG_MSG(("0x%X[19:17] = 0x%X\n", CHA_PHY(0x161C), (u4Value >> 17) & 0x7));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0x169C));
    mcSHOW_DBG_MSG(("0x%X[19:17] = 0x%X\n", CHA_PHY(0x169C), (u4Value >> 17) & 0x7));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0x171C));
    mcSHOW_DBG_MSG(("0x%X[19:17] = 0x%X\n", CHA_PHY(0x171C), (u4Value >> 17) & 0x7));

    u4Value = (*(volatile unsigned int *)CHB_PHY(0xC1C));
    mcSHOW_DBG_MSG(("0x%X[19:17] = 0x%X\n", CHB_PHY(0xC1C), (u4Value >> 17) & 0x7));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0xC9C));
    mcSHOW_DBG_MSG(("0x%X[19:17] = 0x%X\n", CHB_PHY(0xC9C), (u4Value >> 17) & 0x7));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0xD1C));
    mcSHOW_DBG_MSG(("0x%X[19:17] = 0x%X\n", CHB_PHY(0xD1C), (u4Value >> 17) & 0x7));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x111C));
    mcSHOW_DBG_MSG(("0x%X[19:17] = 0x%X\n", CHB_PHY(0x111C), (u4Value >> 17) & 0x7));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x119C));
    mcSHOW_DBG_MSG(("0x%X[19:17] = 0x%X\n", CHB_PHY(0x119C), (u4Value >> 17) & 0x7));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x121C));
    mcSHOW_DBG_MSG(("0x%X[19:17] = 0x%X\n", CHB_PHY(0x121C), (u4Value >> 17) & 0x7));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x161C));
    mcSHOW_DBG_MSG(("0x%X[19:17] = 0x%X\n", CHB_PHY(0x161C), (u4Value >> 17) & 0x7));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x169C));
    mcSHOW_DBG_MSG(("0x%X[19:17] = 0x%X\n", CHB_PHY(0x169C), (u4Value >> 17) & 0x7));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x171C));
    mcSHOW_DBG_MSG(("0x%X[19:17] = 0x%X\n", CHB_PHY(0x171C), (u4Value >> 17) & 0x7));
    mcSHOW_DBG_MSG(("*** Low power related \n"));

    u4Value = (*(volatile unsigned int *)CHA_PHY(0x088));
    mcSHOW_DBG_MSG(("0x%X[31:10] = 0x%X\n", CHA_PHY(0x088), (u4Value >> 10) & 0x3fffff));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x088));
    mcSHOW_DBG_MSG(("0x%X[31:10] = 0x%X\n", CHB_PHY(0x088), (u4Value >> 10) & 0x3fffff));

    u4Value = (*(volatile unsigned int *)CHA_PHY(0x08C));
    mcSHOW_DBG_MSG(("0x%X = 0x%X\n", CHA_PHY(0x08C), u4Value));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x08C));
    mcSHOW_DBG_MSG(("0x%X = 0x%X\n", CHB_PHY(0x08C), u4Value));

    u4Value = (*(volatile unsigned int *)CHA_PHY(0x108));
    mcSHOW_DBG_MSG(("0x%X[31:10] = 0x%X\n", CHA_PHY(0x108), (u4Value >> 10) & 0x3fffff));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x108));
    mcSHOW_DBG_MSG(("0x%X[31:10] = 0x%X\n", CHB_PHY(0x108), (u4Value >> 10) & 0x3fffff));

    u4Value = (*(volatile unsigned int *)CHA_PHY(0x10C));
    mcSHOW_DBG_MSG(("0x%X = 0x%X\n", CHA_PHY(0x10C), u4Value));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x10C));
    mcSHOW_DBG_MSG(("0x%X = 0x%X\n", CHB_PHY(0x10C), u4Value));

    u4Value = (*(volatile unsigned int *)CHA_PHY(0x188));
    mcSHOW_DBG_MSG(("0x%X[31:10] = 0x%X\n", CHA_PHY(0x188), (u4Value >> 10) & 0x3fffff));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x188));
    mcSHOW_DBG_MSG(("0x%X[31:10] = 0x%X\n", CHB_PHY(0x188), (u4Value >> 10) & 0x3fffff));

    u4Value = (*(volatile unsigned int *)CHA_PHY(0x18C));
    mcSHOW_DBG_MSG(("0x%X = 0x%X\n", CHA_PHY(0x18C), u4Value));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x18C));
    mcSHOW_DBG_MSG(("0x%X = 0x%X\n", CHB_PHY(0x18C), u4Value));

    u4Value = (*(volatile unsigned int *)CHA_PHY(0x274));
    mcSHOW_DBG_MSG(("0x%X = 0x%X\n", CHA_PHY(0x274), u4Value));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x274));
    mcSHOW_DBG_MSG(("0x%X = 0x%X\n", CHB_PHY(0x274), u4Value));
    u4Value = (*(volatile unsigned int *)CHA_PHY(0x27C));
    mcSHOW_DBG_MSG(("0x%X = 0x%X\n", CHA_PHY(0x27C), u4Value));
    u4Value = (*(volatile unsigned int *)CHB_PHY(0x27C));
    mcSHOW_DBG_MSG(("0x%X = 0x%X\n", CHB_PHY(0x27C), u4Value));
 ...
 * Ex: Bi_anco DramC NAO last register definition
...
void DramcModeRegReadByRank(DRAMC_CTX_T *p, U8 u1Rank, U8 u1MRIdx, U16 *u2pValue)
{
    U16 u2Value = 0;    
    U8 u1RankBak;
    U8 u1Scinput_opt = 0; //Indicates old/new TMRRI design change mode (Since Syl_via/Can_non) DE: Jouling, Berson)

    u1Scinput_opt = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DRAMC_REG_DRSCTRL), DRSCTRL_RK_SCINPUT_OPT);

    /* Since Can_non/Syl_via, TMRRI design changed (2 kinds of modes depending on value of R_DMRK_SCINPUT_OPT)
...
U16 DDRPhyFMeter()
{
    ...
    return FMeter(CKGEN_FMETER, 60) << 2;
...
void O1Path_Test(DRAMC_CTX_T * p)
{
    ...
    U32 u4dq_o1, u4ca_o1, u4fail=0, loop=0, fail_cnt=0;
    ...
    for(channel_idx=CHANNEL_A; channel_idx<p->support_channel_num; channel_idx++)
    {
        ...
        if (u1IsLP4Family(p->dram_type))
        {
            // add new RG setting for Syl_via/Can_non O1Path pull dn/up setting
            ...
            vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_B0_DQ0), P_Fld(0x7, B0_DQ0_RG_RX_ARDQ7_OFFC_B0)|
                                                              P_Fld(0x7, B0_DQ0_RG_RX_ARDQ6_OFFC_B0)|
                                                              P_Fld(0x7, B0_DQ0_RG_RX_ARDQ5_OFFC_B0)|
                                                              P_Fld(0x7, B0_DQ0_RG_RX_ARDQ4_OFFC_B0)|
                                                              P_Fld(0x7, B0_DQ0_RG_RX_ARDQ3_OFFC_B0)|
                                                              P_Fld(0x7, B0_DQ0_RG_RX_ARDQ2_OFFC_B0)|
                                                              P_Fld(0x7, B0_DQ0_RG_RX_ARDQ1_OFFC_B0)|
                                                              P_Fld(0x7, B0_DQ0_RG_RX_ARDQ0_OFFC_B0));
            vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_B0_DQ1), 0x7, B0_DQ1_RG_RX_ARDQM0_OFFC_B0);

            vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_B1_DQ3), 1, B1_DQ3_RG_RX_ARDQ_OFFC_EN_B1);
            vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_B1_DQ0), P_Fld(0x7, B1_DQ0_RG_RX_ARDQ7_OFFC_B1)|
                                                              P_Fld(0x7, B1_DQ0_RG_RX_ARDQ6_OFFC_B1)|
                                                              P_Fld(0x7, B1_DQ0_RG_RX_ARDQ5_OFFC_B1)|
                                                              P_Fld(0x7, B1_DQ0_RG_RX_ARDQ4_OFFC_B1)|
                                                              P_Fld(0x7, B1_DQ0_RG_RX_ARDQ3_OFFC_B1)|
                                                              P_Fld(0x7, B1_DQ0_RG_RX_ARDQ2_OFFC_B1)|
                                                              P_Fld(0x7, B1_DQ0_RG_RX_ARDQ1_OFFC_B1)|
                                                              P_Fld(0x7, B1_DQ0_RG_RX_ARDQ0_OFFC_B1));
            vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_B1_DQ1), 0x7, B1_DQ1_RG_RX_ARDQM0_OFFC_B1);
            ...
            if (u4dq_o1!=0 || u4ca_o1!=0)
                u4fail = 1;
            mcSHOW_ERR_MSG(("[Write Leveling][%d] CH_%d PULL DN=1, DQ-DQM O1=0x%x (%s), CS-CKE-CA O1=0x%x (%s)\n",loop,channel_idx,u4dq_o1,(u4dq_o1==0)?"pass":"fail",u4ca_o1,(u4ca_o1==0)?"pass":"fail"));

            // Justin: remove original pull up driver, use new design for Can_non/Syl_via
            ...
            do
            {
                ...
                if (u4dq_o1!=0x1FF01FF || u4ca_o1!=0x6C3F) //refer to RG coda field
                    u4fail = 1;
                mcSHOW_ERR_MSG(("[Write Leveling][%d] CH_%d PULL UP=1, DQ-DQM O1=0x%x (%s), CS-CKE-CA O1=0x%x (%s)\n",loop,channel_idx,u4dq_o1,(u4dq_o1==0x1FF01FF)?"pass":"fail",u4ca_o1,(u4ca_o1==0x6C3F)?"pass":"fail"));
                if (u4fail==0)
                    break;
            } while(loop++ <10);
