    ...
    for(u1ChannelIdx=0; u1ChannelIdx<(p->support_channel_num); u1ChannelIdx++)
    {
        ...
        for(u4RankIdx =0; u4RankIdx < (U32)(p->support_rank_num); u4RankIdx++)
        {
           ...
           // if(p->frequency<=1200)
            ...
            {
                u1MR13Value[u1MRFsp] &= 0x3f; //[7]FSP_OP=0, [6]FSP_WR=0
            }
            else
            {
                u1MR13Value[u1MRFsp] |= 0xc0; //[7]FSP_OP=1, [6]FSP_WR=1
            }
    
            DramcModeRegWrite(p, 13, u1MR13Value[u1MRFsp]);
...
static DRAM_STATUS_T UpdateInitialSettings_LP4(DRAMC_CTX_T *p)
{
    ...
    //fcSylvia MP setting should set CKECTRL_CKELCKFIX as 1 to roolback to M17 (review by CF Chang)
    vIO32WriteFldAlign(DRAMC_REG_CKECTRL, 0x1, CKECTRL_CKELCKFIX);

    //Gating error problem happened in M17 has been solved by setting this RG as 0 (when RODT tracking on (fcBianco), TX DLY of byte2,3 must not be zero)
...
static DRAM_STATUS_T UpdateInitialSettings_LP3(DRAMC_CTX_T *p)
{
    ...
    {
        //For fcJADE-like mode, reserve bit5 should be set as "0" before calibration (by Justin)
	...
	//DMSTBLAT should set as 3 for fcJADE-like mode 
    if(p->frequency==933)
    {
    ...
    vIO32WriteFldMulti_All(DRAMC_REG_PERFCTL0, P_Fld(0x1, PERFCTL0_RWSPLIT) //synchronous EMI -> can turn on RWSPLIT
                                            | P_Fld(0x0, PERFCTL0_REORDEREN));//Lewis@20170623: Set REORDEREN to 1 in LP3 In order to be consistent with fcKibo+. SBR is off in EMI.
    #endif
...
//fcSylvia MP setting should set SHU_STBCAL_DMSTBLAT as 0x2 (review by HJ Huang)
    if(p->frequency<=800)
    {
...
void DramcModeReg_Check(DRAMC_CTX_T *p)
{
    ...
    U8 u1match=0, fsp_start, fsp_end;
    ...
    fsp_end = p->dram_fsp;
#else
    fsp_start = p->dram_fsp;
    fsp_end = p->dram_fsp;
#endif
    
    for (u1MRFsp=fsp_start; u1MRFsp<=fsp_end; u1MRFsp++)
    {
        for(u1ChannelIdx=0; u1ChannelIdx<(p->support_channel_num); u1ChannelIdx++)
        {
            vSetPHY2ChannelMapping(p, u1ChannelIdx);
...
void O1Path_Test(DRAMC_CTX_T * p)
{
    ...
    for(channel_idx=CHANNEL_A; channel_idx<p->support_channel_num; channel_idx++)
    {
        ...
        if (u1IsLP4Family(p->dram_type))
        {
            // add new RG setting for fcSylvia/fcCannon O1Path pull dn/up setting
            ...
            // Justin: remove original pull up driver, use new design for fcCannon/fcSylvia
