...
extern struct mtk_regulator reg_vddq, reg_vcore, reg_vdram, reg_vio18;
#endif
#endif
...
void vSetVcoreByFreq(DRAMC_CTX_T *p)
{
    ...
    // for 3733, 3600
		if(p->frequency >= 1800)
			#ifdef VCORE_BIN
				vcore = get_vcore_opp_volt(p->dram_type, 0);
			#else
				vcore = SEL_PREFIX_VCORE(LP4, KOPP0);
			#endif
		else if(p->frequency == 1600)
			#ifdef VCORE_BIN
				#ifdef MTK_LP4_DDR3200
				vcore = (get_vcore_opp_volt(p->dram_type, 0) + get_vcore_opp_volt(p->dram_type, 1)) >> 1;
				#else
				if (seclib_get_devinfo_with_index(30) & 0x20)
					vcore = (get_vcore_opp_volt(p->dram_type, 0) + get_vcore_opp_volt(p->dram_type, 1)) >> 1;
				else
					vcore = (get_vcore_opp_volt(p->dram_type, 1) + get_vcore_opp_volt(p->dram_type, 2)) >> 1;
				#endif
			#else
			...
			#ifdef VCORE_BIN
				vcore = (get_vcore_opp_volt(p->dram_type, 2) + get_vcore_opp_volt(p->dram_type, 3)) >> 1;
			#else
    ...
    else
    {
        ...
        if(p->frequency >= 933)
            ...
            vcore = (SEL_PREFIX_VCORE(LP3, KOPP2) + SEL_PREFIX_VCORE(LP3, KOPP3)) >> 1;
		else
			return ;
    }
#endif
	...
	if (vio18) {
		if (vio18 > STD_VIO18) {
			...
			if (tmp > 80000) {
				tmp = tmp % 100000;
				pmic_config_interface( PMIC_RG_VIO18_VOCAL_ADDR, (vio18 - tmp - STD_VIO18) / 10000, PMIC_RG_VIO18_VOCAL_MASK, PMIC_RG_VIO18_VOCAL_SHIFT);
			}
			mtk_regulator_set_votrim(&reg_vio18, tmp);
		} else if (vio18 < STD_VIO18) {
			mtk_regulator_set_votrim(&reg_vio18, vio18 - STD_VIO18);
		}
	}
#endif
...
//			mtk_regulator_set_voltage(&reg_vddq, vddq, MAX_VDDQ);
	...
	print("Vio18 = %d\n", mtk_regulator_get_voltage(&reg_vio18) + mtk_regulator_get_votrim(&reg_vio18));
...
U32 vGetVoltage(DRAMC_CTX_T *p, U32 get_voltage_type)
{
    ...
    if (get_voltage_type==2)
        return mtk_regulator_get_voltage(&reg_vddq) + mtk_regulator_get_votrim(&reg_vddq);
#endif
...
#if (defined(DRAM_HQA) || defined(__ETT__)) && (FOR_DV_SIMULATION_USED == 0)
    get_vcore = mtk_regulator_get_voltage(&reg_vcore)/100;
#endif        
...
DRAM_STATUS_T DramcSave_Time_For_Cal_Init(DRAMC_CTX_T *p)
{
    ...
    else
    {
        ...
        if (seclib_get_devinfo_with_index(30) & 0x20) {
					if (LP4_DDR2400>LP4_DDR3200)
	        {
	            p->pSavetimeData->Bypass_RDDQC[_MappingFreqArray[LP4_DDR2400]]=1;
	            p->pSavetimeData->Bypass_RXWINDOW[_MappingFreqArray[LP4_DDR2400]]=1;
	            p->pSavetimeData->Bypass_TXWINDOW[_MappingFreqArray[LP4_DDR2400]]=1;
	        }
	        else
	        {
	            p->pSavetimeData->Bypass_RDDQC[_MappingFreqArray[LP4_DDR2400]]=0;
	            p->pSavetimeData->Bypass_RXWINDOW[_MappingFreqArray[LP4_DDR2400]]=0;
	            p->pSavetimeData->Bypass_TXWINDOW[_MappingFreqArray[LP4_DDR2400]]=0;
	        }
        } else {
        ...
        if (seclib_get_devinfo_with_index(30) & 0x20) {
	        ...
	        p->pSavetimeData->Bypass_RDDQC[_MappingFreqArray[LP4_DDR3200]]=1;
	        p->pSavetimeData->Bypass_RXWINDOW[_MappingFreqArray[LP4_DDR3200]]=1;
	        p->pSavetimeData->Bypass_TXWINDOW[_MappingFreqArray[LP4_DDR3200]]=1;
      	} else {
	        // force RXDQC, Rx, Tx without K again for highest frequency (for run time Tx eye scan test)
	        p->pSavetimeData->Bypass_RDDQC[_MappingFreqArray[LP4_MIDDLE_FREQSEL]]=1;
	        p->pSavetimeData->Bypass_RXWINDOW[_MappingFreqArray[LP4_MIDDLE_FREQSEL]]=1;
...
#else
        if (seclib_get_devinfo_with_index(30) & 0x20) {
        	p->pSavetimeData->Bypass_RDDQC[_MappingFreqArray[LP4_DDR3200]]=0;
        	p->pSavetimeData->Bypass_RXWINDOW[_MappingFreqArray[LP4_DDR3200]]=0;
        	p->pSavetimeData->Bypass_TXWINDOW[_MappingFreqArray[LP4_DDR3200]]=0;
        } else {
...
#ifdef FOR_HQA_TEST_USED
    GetVcoreDelayCellTimeFromTable(p);
#endif
...
#if SUPPORT_SAVE_TIME_FOR_CALIBRATION
        p->pSavetimeData->u2MR5VendorID = p->vendor_id;
#endif
...
#if SUPPORT_SAVE_TIME_FOR_CALIBRATION
		p->pSavetimeData->u8MR8Density[u1ChannelIdx][u1RankIdx] = u8Size;
#endif
         }
...
{   
    ...
    p->freq_sel = LP4_HIGHEST_FREQSEL;
