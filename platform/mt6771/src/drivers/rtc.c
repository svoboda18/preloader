...
static void rtc_switch_mode(bool XOSC, bool recovery)
{
	...
	if (XOSC)
	{
		...
		rtc_xosc_write(osc32con | 0x7);  /* assume crystal exist mode + XOSCCALI = 0x7 */
		if (recovery)
			mdelay(1500);
	} else
	{
...
static bool rtc_frequency_meter_check(void)
{
	...
	{
		RTC_INFO("OSC32CON_ANALOG_SETTING = 0x%x\n", OSC32CON_ANALOG_SETTING);
		osc32con = OSC32CON_ANALOG_SETTING;
		rtc_xosc_write(osc32con | 0x7);	/* crystal exist mode + XOSCCALI = 0x7 */
		return true;
	}
	else
	{
...
static bool rtc_hw_init(void)
{
	...
	do {
		RTC_Write(RTC_BBPU, RTC_Read(RTC_BBPU) | RTC_BBPU_KEY | RTC_BBPU_RELOAD);
		Write_trigger();
	} while ((RTC_Read(RTC_BBPU) & RTC_BBPU_INIT) && get_timer(begin) < 500);

	if (RTC_Read(RTC_BBPU) & RTC_BBPU_INIT) {
		RTC_ERR("hw_init timeout\n");
		return false;
	}
...
void rtc_enable_dcxo(void)
{
	...
	RTC_INFO("enable_dcxo first con = 0x%x, osc32con = 0x%x, sec = 0x%x\n", RTC_Read(RTC_CON), RTC_Read(RTC_OSC32CON), RTC_Read(RTC_AL_SEC));
}
...
static void rtc_bbpu_power_down(void)
{
	...
	if (is_pmic_rtc_alarm() || is_pmic_spar()) {
		RTC_INFO("clearing alarm/spar, %d, %d\n", is_pmic_rtc_alarm(), is_pmic_spar());
		bbpu |= RTC_BBPU_CLR;
	}
	RTC_Write(RTC_BBPU, bbpu);
	RTC_Write(RTC_AL_MASK, RTC_AL_MASK_DOW);	/* lpsd; mask DOW */
	Write_trigger();
...
static void rtc_lpsd_solution(void)
{
	...
	RTC_INFO("1st RTC_AL_MASK= 0x%x \n", RTC_Read(RTC_AL_MASK));
	RTC_Write(RTC_AL_MASK, (RTC_AL_MASK_SEC | RTC_AL_MASK_MIN | RTC_AL_MASK_HOU | RTC_AL_MASK_DOM | RTC_AL_MASK_DOW | RTC_AL_MASK_MTH | RTC_AL_MASK_YEA));
...
void rtc_init(void)
{
	...
	RTC_INFO("%s#1 powerkey1 = 0x%x, powerkey2 = 0x%x, %s LPD\n",
		__func__, RTC_Read(RTC_POWERKEY1), RTC_Read(RTC_POWERKEY2),(RTC_Read(RTC_CON) & RTC_CON_LPSTA_RAW) ? "with" : "without" );
	...
	pmic_config_interface(PMIC_RG_OCTL_RTC_32K1V8_0_ADDR, 0x8, PMIC_RG_OCTL_RTC_32K1V8_0_MASK, PMIC_RG_OCTL_RTC_32K1V8_0_SHIFT);
	rtc_clean_mark();
	...
	Write_trigger();
	if (!rtc_lpd_init()) {
		RTC_ERR("[RTC] rtc_lpd_init fail1\n");
		return;
	}
...
bool rtc_boot_check(void)
{
	...
	if (irqsta & RTC_IRQ_STA_AL) {
		...
		if (pdn1 & RTC_PDN1_PWRON_TIME) {	/* power-on time is available */
			...
			//RTC_INFO("now =%u time_upper = %u time_lower = %u\n", now_time, time_upper, time_lower);
...
void rtc_save_2sec_stat(void)
{
	...
	RTC_INFO("rtc_2sec_reboot_check 0x%x, %s 2sec reboot, type 0x%x\n", reboot,
		(reboot & RTC_BBPU_2SEC_STAT_STA) ? "with" : "without",
		...
		switch((reboot & RTC_BBPU_2SEC_MODE_MSK) >> RTC_BBPU_2SEC_MODE_SHIFT) {
			...
			case 2:
				if(reboot & RTC_BBPU_2SEC_STAT_STA) {
					...
					{
						RTC_INFO("rtc IPO shutdown disable auto reboot\n");
...
void secure_rtc_set_ck(void)
{
	...
	pmic_read_interface(MT6358_SCK_TOP_CKPDN_CON0, &val, 0xFF, 0);
	...
	RTC_INFO("SCK_TOP_CKPDN_CON0[0x%x]=0x%x, RTC_SEC_CK_PDN[0x%x]=0x%x\n", MT6358_SCK_TOP_CKPDN_CON0, val, RTC_SEC_CK_PDN, RTC_Read(RTC_SEC_CK_PDN));
}
