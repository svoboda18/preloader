...
static bool uart_listen(struct bldr_comport *comport, uint8 *data, uint32 size, int retry, uint32 tmo_ms)
{
    ...
    for (; retry > 0; retry--) {
        ...
        while(1) {
            ...
            if (rx_cnt == size){
                pal_log_info("%s <UART> listen  success!\n",MOD);
                return TRUE;
            }
        }
    }

      pal_log_info("%s <UART> listen  ended, receive size:%d!\n",MOD,rx_cnt);
...
static bool uart_handshake_handler(struct bldr_command_handler *handler)
{
    ...
    if (g_boot_mode == META_BOOT || g_boot_mode == ADVMETA_BOOT) {
        pal_log_info("%s aleady detected meta tool!\n", MOD);
        result =FALSE;
        goto exit;
    }
...
exit:
    ...
    *(unsigned int *)0x00100004 = 0xAABBCCDD;
    ...
    pal_log_info("\n");
    pal_log_info("%s <UART> wait sync time %dms->%dms\n", MOD, UART_SYNC_TIME, sync_time);
    pal_log_info("%s <UART> receieved data: (%s)\n", MOD, buf);
...
bool uart_handshake(struct bldr_command_handler *handler)
{
    ...
    if (result == TRUE) {
        pal_log_info("%s <UART> handshake pass!\n",MOD);
    }
    return result;
}
