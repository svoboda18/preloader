...
void init_dram_buffer(){
    ...
    if ((u64)CFG_DRAM_ADDR + platform_memory_size() > (u64)DRAM_4GB_SIZE)  // DRAM size > 4GB
        g_dram_buf = (dram_buf_t *)(DRAM_4GB_SIZE - (5*1024*1024));
    else
        g_dram_buf = (dram_buf_t *)(CFG_DRAM_ADDR + platform_memory_size() - (5*1024*1024)); // Max DRAM size - 5MB
#elif defined(DUMMY_AP)
    g_dram_buf = (dram_buf_t *)CFG_DRAM_ADDR;
#else
    #if CFG_BYPASS_EMI
    g_dram_buf = (dram_buf_t *)CFG_DRAM_ADDR;
    #else
