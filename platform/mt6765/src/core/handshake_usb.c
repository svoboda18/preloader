...
extern int mt_usb_phy_is_on(void);
    ...
    do {
        ...
        if (tmo) {
            ...
            if (get_timer(start_time) > i * 1000) {
                if (!usb_accessory_in() && !pmic_detect_powerkey() && !platform_is_boot_reason_valid())
                    pl_power_off();

		#if !CFG_EVB_PLATFORM
                /* check bypass power key from the 2nd second */
                if (i > 1 && pmic_detect_powerkey()) {
                        pal_log_info("%s Enumeration(Skip): powerkey pressed\n", MOD);
                    break;
                }
                i++;
		#endif
...
static int usb_recv(u8 *buf, u32 size, u32 tmo_ms)
{
    ...
    while (1) {
        if (tmo_en && (get_timer(start_time) > tmo_ms)){
            pal_log_warn("%s : usb receive timeout\n", MOD);
            return -1;
        }
...
static bool usb_listen(struct bldr_comport *comport, uint8 *data, uint32 size, uint32 tmo_ms)
{
    ...
    while (1) {
        ...
        if (tmo_en && (get_timer(start_time) > tmo_ms)){
            pal_log_warn("%s : usb listen timeout\n", MOD);
            return FALSE;
         }
...
static bool usb_port_down(uint32 tmo_ms)
{
    ...
    pal_log_info("%s usb port down: %dms\n", MOD, get_timer(start_time));
...
bool usb_handshake(struct bldr_command_handler *handler)
{
    ...
    if (mt_usb_phy_is_on()) {
        handshake_tmo += 3500;
        pal_log_info("%s increase handshake_tmo %d\n", MOD, handshake_tmo);
    }
    ...
    if (FALSE == usb_port_down(USB_PORT_DOWN_TIME)) {
        pal_log_warn("%s USB port down timeout!\n", MOD);
    }
