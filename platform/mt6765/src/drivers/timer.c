...
#define GPT3_CON                    ((P_U32)(GPT_BASE+0x0030))
#define GPT3_CLK                    ((P_U32)(GPT_BASE+0x0034))
#define GPT3_DAT                    ((P_U32)(GPT_BASE+0x0038))

#define GPT3_EN                     0x0001
#define GPT3_FREERUN                0x0030
#define GPT3_RTC_CLK                0x0010
#define GPT3_CLK_DIV1               0X0000

#define GPT3_CLK_SETTING    (GPT3_RTC_CLK | GPT3_CLK_DIV1)
...
void gpt_busy_wait_ns(U32 timeout_ns)
{
    U32 start_tick, timeout_tick;

    // get timeout tick
    timeout_tick = (timeout_ns * GPT4_1US_TICK + (1000 - 1)) / 1000;
    start_tick = gpt4_get_current_tick();
...
/* delay nsec useconds */
/* Can only delay multiple of 76.92ns */
void ndelay(unsigned long nsec)
{
   gpt_busy_wait_ns(nsec);
}

static void gpt3_start(void)
{
    *GPT3_CLK = (GPT3_CLK_SETTING);
    *GPT3_CON = (GPT3_EN | GPT3_FREERUN);
}

static void gpt3_stop(void)
{
    *GPT3_CON = 0x0; // disable
    *GPT3_CON = 0x2; // clear counter
}

static void gpt3_init(bool bStart)
{
    // clear GPT3 first
    gpt3_stop();

    // enable GPT3 without lock
    if (bStart)
        gpt3_start();
}
...
void mtk_timer_init (void)
{
    ...
    gpt3_init(TRUE); /* 32k freerun timer */
}
