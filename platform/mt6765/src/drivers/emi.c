...
unsigned int dramc_get_vddq_voltage(unsigned int ddr_type)
{
	...
	if (ddr_type == TYPE_LPDDR4)
		sum = dramc_get_vdd2_voltage(ddr_type);
	else
...
static void update_default_emi_setting(DRAM_INFO_BY_MRR_T *DramInfo)
{
	unsigned int i;
	unsigned int ddr_type;
	unsigned long long die_size;
	unsigned int col, row, row_ext, rank_size;

	ddr_type = mt_get_dram_type_from_hw_trap();

	// EMI conf init value
	default_emi_setting.EMI_CONH_VAL = 0x00000003;
	if (u1IsLP4Family(ddr_type)) {
		default_emi_setting.EMI_CONA_VAL = 0x00000104;
		default_emi_setting.CHN0_EMI_CONA_VAL = 0x04000000;
	} else {
		default_emi_setting.EMI_CONA_VAL = 0x00000006;
		default_emi_setting.CHN0_EMI_CONA_VAL = 0x00000002;
	}

	for (i = 0; i < DramInfo->u4RankNum; i++) {
		die_size = DramInfo->u8MR8Density[0][i] / DramInfo->u1DieNum[0][i];
		switch (die_size | (DramInfo->u1DieNum[0][i] << 4) | u1IsLP4Family(ddr_type)) {
		case 0x20000010LL: // 4Gb, x32 LP3
			col = 1;
			row = 1;
			break;
		case 0x20000020LL: // 4Gb, x16 LP3
			col = 2;
			row = 1;
			break;
		case 0x30000010LL: // 6Gb, x32 LP3
		case 0x40000010LL: // 8Gb, x32 LP3
			col = 1;
			row = 2;
			break;
		case 0x30000020LL: // 6Gb, x16 LP3
		case 0x40000020LL: // 8Gb, x16 LP3
		case 0x60000010LL: // 12Gb, x32 LP3
		case 0x80000010LL: // 16Gb, x32 LP3
			col = 2;
			row = 2;
			break;
		case 0x60000020LL: // 12Gb, x16 LP3
		case 0x80000020LL: // 16Gb, x16 LP3
			col = 3;
			row = 2;
			break;
		case 0x20000011LL: // 4Gb, x16 LP4
			col = 1;
			row = 1;
			break;
		case 0x20000021LL: // 4Gb, x8 LP4
		case 0x30000011LL: // 6Gb, x16 LP4
		case 0x40000011LL: // 8Gb, x16 LP4
			col = 1;
			row = 2;
			break;
		case 0x30000021LL: // 6Gb, x8 LP4
		case 0x40000021LL: // 8Gb, x8 LP4
		case 0x60000011LL: // 12Gb, x16 LP4
		case 0x80000011LL: // 16Gb, x16 LP4
			col = 1;
			row = 3;
			break;
		case 0x60000021LL: // 12Gb, x8 LP4
		case 0x80000021LL: // 16Gb, x8 LP4
			col = 1;
			row = 4;
			break;
		default:
			dramc_crit("[DRAMC] map EMI setting fail\n");
			ASSERT(0);
		}

		rank_size = DramInfo->u8MR8Density[0][i] >> 28;
		if (u1IsLP4Family(ddr_type))
		...
		if (i == 1) {
			...
			row_ext <<= 1;
			rank_size <<= 4;
			default_emi_setting.EMI_CONA_VAL |= 0x30000;
			default_emi_setting.CHN0_EMI_CONA_VAL |= 0x1;
		}

		default_emi_setting.EMI_CONA_VAL |=
			(col << 4) | (row << 12) | (col << 20) | (row_ext << 24) | (row << 28);
		default_emi_setting.EMI_CONH_VAL |=
			(row_ext << 4) | (rank_size << 16) | (rank_size << 24);
		default_emi_setting.CHN0_EMI_CONA_VAL |=
			(row_ext << 2) | (col << 4) | (row << 12) | (rank_size << 16);
		default_emi_setting.CHN1_EMI_CONA_VAL = default_emi_setting.CHN0_EMI_CONA_VAL;
		default_emi_setting.DRAM_RANK_SIZE[i] = DramInfo->u8MR8Density[0][i];
	}

	default_emi_setting.EMI_CONF_VAL = 0x0;
	default_emi_setting.type = mt_get_dram_type_from_hw_trap();

	switch ((DramInfo->u1DieNum[0][0] << 4) | DramInfo->u1DieNum[0][1]) {
	case 0x12:
	...
	default_emi_setting.iLPDDR3_MODE_REG_5 = DramInfo->u2MR5VendorID & 0xFF;
}
...
static int get_mdl_mr_only(void)
{
	...
	update_default_emi_setting(&DramInfo);
	init_done = 1;

	return -1; // use default_emi_setting
}

static int mt_get_mdl_number(void)
{
#ifdef COMBO_MCP_MR_ONLY
	return get_mdl_mr_only();
#else
	return get_mdl();
#endif
}
...
void get_dram_rank_size (u64 dram_rank_size[])
{
  ...
  #endif  	
    {
        ...
        for(i = 0; i < rank_nr; i++){
  #ifdef COMBO_MCP_MR_ONLY
            dram_rank_size[i] = default_emi_setting.DRAM_RANK_SIZE[i];
  #else
...
#ifdef MTK_LP3_HIGHEST_DDR1600
    dramc_crit("[DRAMC] detect MTK_LP3_HIGHEST_DDR1600\n");
#endif

#ifdef MTK_LP4_HIGHEST_DDR2400
    if (mt_get_dram_type_from_hw_trap() == TYPE_LPDDR4X) {
        dramc_crit("[DRAMC] detect MTK_LP4_HIGHEST_DDR2400 for LP4X\n");
#ifndef MTK_LP4X_HIGHEST_DDR2400
        ASSERT(0);
#endif
    }
#else
    if (mt_get_dram_type_from_hw_trap() == TYPE_LPDDR4) {
        dramc_crit("[DRAMC] no MTK_LP4_HIGHEST_DDR2400 for LP4\n");
        ASSERT(0);
    }
#endif
