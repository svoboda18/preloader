...
static signed int _pwrap_init_sistrobe(int dual_si_sample_settings)
{
	...
	/* TINFO = "[DrvPWRAP_InitSiStrobe] SI Strobe Calibration For ULPOSC Clock For PMIC 0 Done, *PMIC_WRAP_RESERVED = %x", si_sample_ctrl */
#ifdef DUAL_PMICS
	if(dual_si_sample_settings == 1)
	{
		si_sample_ctrl = si_sample_ctrl | (0x1 << 18);
		/* TINFO = "[DrvPWRAP_InitSiStrobe] SI Strobe Calibration For ULPOSC Clock For PMIC 1............" */
		si_en_sel = (WRAP_RD32(PMIC_WRAP_SI_SAMPLE_CTRL) << 14) >> 29;
		si_ck_sel = (WRAP_RD32(PMIC_WRAP_SI_SAMPLE_CTRL) << 17) >> 31;
		clk_edge_no = (((si_en_sel * 2 + si_ck_sel) * 100) * CLK_26M_PRD / CLK_ULPOSC_PRD + 50) / 100;
		/* TINFO = "[DrvPWRAP_InitSiStrobe] The Sampling Clock Edge For ULPOSC Clock For PMIC 1 Is Chosen As Clock Edge %d.", clk_edge_no */
		si_en_sel = clk_edge_no / 2;
		si_ck_sel = clk_edge_no % 2;
		si_sample_ctrl = (si_sample_ctrl & 0xfffc01ff) | (si_en_sel << 15) | (si_ck_sel << 14);
		WRAP_WR32(PMIC_WRAP_SI_SAMPLE_CTRL_ULPOSC, si_sample_ctrl);
		/* TINFO = "[DrvPWRAP_InitSiStrobe] SI Strobe Calibration For ULPOSC Clock For PMIC 1 Done, *PMIC_WRAP_RESERVED = %x", si_sample_ctrl */
	}
#endif
	...
	pwrap_read_nochk(EXT_DEW_READ_TEST, &rdata);
	if (rdata != DEFAULT_VALUE_READ_TEST) {
		PWRAPERR("_pwrap_init_sistrobe Read Test Failed, EXT rdata = %x, exp = 0x5aa5\n",
		         rdata);
		return 0x11;
	}
#endif
