...
#if 0 //to reduce preloader code size
...
#endif //to reduce preloader code size
  ...
  if (!(tmo == 0 )&& TRANSFER_ERROR==FALSE )
  {
    ...
    { /*only read mode or write_read mode and fifo mode need to get data*/
      //data_size = (i2c_readl(i2c, OFFSET_FIFO_STAT) >> 4) & 0x000F;
         /* we use user's read_len for the FIFO length */
...
int i2c_hw_init(void)
{
	...
	I2C_SET_REG32((I2C2_BASE + OFFSET_MULTI_DMA), (0x01<<1), (0x01<<1));
	I2C_SET_REG32((I2C3_BASE + OFFSET_MULTI_DMA), (0x01<<1), (0x01<<1));
	I2C_SET_REG32((I2C4_BASE + OFFSET_MULTI_DMA), (0x01<<1), (0x01<<1));
	I2C_SET_REG32((I2C6_BASE + OFFSET_MULTI_DMA), (0x01<<1), (0x01<<1));

	I2CLOG("[i2c2]ADDR:0x%x,multi_DMA:0x%x\n", I2C2_BASE + OFFSET_MULTI_DMA,
		DRV_Reg32(I2C2_BASE + OFFSET_MULTI_DMA));
	I2CLOG("[i2c3]ADDR:0x%x,multi_DMA:0x%x\n", I2C3_BASE + OFFSET_MULTI_DMA,
		DRV_Reg32(I2C4_BASE + OFFSET_MULTI_DMA));
	I2CLOG("[i2c4]ADDR:0x%x,multi_DMA:0x%x\n", I2C4_BASE + OFFSET_MULTI_DMA,
		DRV_Reg32(I2C4_BASE + OFFSET_MULTI_DMA));
	I2CLOG("[i2c6]ADDR:0x%x,multi_DMA:0x%x\n", I2C6_BASE + OFFSET_MULTI_DMA,
		DRV_Reg32(I2C6_BASE + OFFSET_MULTI_DMA));
#endif
