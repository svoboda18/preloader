...
/* 11 - 15*/	52000, 109200, 273000, 384000, 200000,
/* 16 - 20*/	26000, 136500, 196608, 24576, 26000,
/* 21 - 25*/	273000, 136500, 62400, 68250, 124800,
/* 26 - 30*/	68250, 312000, 384000, 15625, 208000,
/* 31 - 35*/	0, 0, 0, 0, 0,
/* 36 - 40*/	0, 0, 0, 0, 0,
/* 41 - 45*/	0, 0, 0, 0, 0,
/* 46 - 50*/	0, 0, 0, 0, 0,
...
/* 16 - 20*/	26000, 0, 0, 0, 0,
/* 21 - 25*/	1500000, 920000, 1092000, 1248000, 460000,
/* 26 - 30*/	384000, 457000, 196608, 0, 0,
/* 31 - 35*/	192000, 0, 0, 0, 0,
/* 36 - 40*/	0, 0, 0, 0, 0,
/* 41 - 45*/	0, 0, 0, 0, 0,
/* 46 - 50*/	0, 0, 0, 600000, 208000,
/* 51 - 55*/	0, 0, 0, 0, 48000,
/* 56 - 57*/	104000, 52000,
};
...
void mt_pll_init(void)
{
	...
	//step 0
	project_id = seclib_get_devinfo_with_index(M_SEGMENT);
	//step 1
	...
	if (project_id != 0x8 && project_id != 0xF) {
		...
		temp = DRV_Reg32(ARMPLL_CON3);
		DRV_WriteReg32(ARMPLL_CON3, temp & 0xFFFFFFFE); // [0]=1 (ARMPLL_PWR_ON)
	}
	...
	if (project_id != 0x8 && project_id != 0xF) {
		...
		temp = DRV_Reg32(ARMPLL_CON3);
		DRV_WriteReg32(ARMPLL_CON3, temp | 0x2); // [1]=0 (ARMPLL_ISO_EN)
	}
	...
	*********************/
	//step 26
	if (project_id != 0x8 && project_id != 0xF)
		DRV_WriteReg32(ARMPLL_CON1, 0x8111B13B); // [25:0] (ARMPLL_N_INFO, 920MHz)

	//step 27
	DRV_WriteReg32(ARMPLL_L_CON1, 0x811CD89D); // [25:0] (ARMPLL_LL_N_INFO, 1500MHz)

	//step 28
	DRV_WriteReg32(CCIPLL_CON1, 0x821713B1); // [25:0] (CCIPLL_N_INFO, 600MHz)

	//step 29
	DRV_WriteReg32(MFGPLL_CON1, 0x8211B13B); // [21:0] (GPUPLL_N_INFO, 460MHz)
	...
	//step 35
	DRV_WriteReg32(APLL1_CON1, 0x83000000); // [21:0] = LSB of APLL1_N_INFO, 196.608MHz
	DRV_WriteReg32(APLL1_CON2, 0x3C7EA932); // [23:14] = MSB of APLL1_N_INFO, 196.608MHz
	...
	************************/
	...
	if (project_id != 0x8 && project_id != 0xF) {
		...
		temp = DRV_Reg32(ARMPLL_CON0);
		DRV_WriteReg32(ARMPLL_CON0, temp & 0xFFFFFFFE); // [0]=1 (ARMPLL_EN)
	}
	//step 37
	...
	****************/
	//step 46
	temp = DRV_Reg32(MAINPLL_CON0);
	DRV_WriteReg32(MAINPLL_CON0, temp | 0xD4000000); // [31:24] (MAINPLL_DIV_EN)

	//step 47
	temp = DRV_Reg32(UNIVPLL_CON0);
	DRV_WriteReg32(UNIVPLL_CON0, temp | 0xD4000000); // [31:24] (UNIVPLL_DIV_EN)
	...
	*/
	...
	/* config other clocks */
	/* scp_sel=top_ap_clk_ctrl_f26m_ck(0), mm_sel=mmpll_d2(2), ddrphycfg_ck=don't touch,  mem_sel=DMPLL_CK(1),axi=syspll1_d7(1) */
	DRV_WriteReg32(CLK_CFG_0, 0x00020101);
	...
	/* camtm_sel=univpll2_d2(3),pwrap_ulposc_sel=clk26m(0) */
	DRV_WriteReg32(CLK_CFG_7, 0x00000300);
	...
	DRV_WriteReg32(MODULE_SW_CG_1_SET, 0x18080b00);
	DRV_WriteReg32(MODULE_SW_CG_2_CLR, 0x2ffc87dd);
	DRV_WriteReg32(MODULE_SW_CG_3_CLR, 0x00038fff);
