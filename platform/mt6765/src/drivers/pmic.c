...
static U32 g_sts_cp_off;
static U32 g_sts_xcpt_off;
...
/* check if PMIC critical power is turned off during system on */
U32 is_pmic_cp_off(void)
{
	return g_sts_cp_off;
}

/* check if PMIC PG/OC shutdown during system on */
U32 is_pmic_xcpt_off(void)
{
	return g_sts_xcpt_off;
}
...
void pmic_dbg_status(unsigned char option)
{
	...
	unsigned int oc_sts = 0;
	unsigned int pg_sts = 0;
	...
	/*--power not good--*/
		pg_sts = upmu_get_reg_value(MT6357_PG_SDN_STS0);
		print("[PMIC]PGSTATUS0[0x%x]=0x%x\n",
			MT6357_PG_SDN_STS0, pg_sts);
		if (((pg_sts >> 14) & 1) == 0) {
			print("[%s] VCORE PG Off\n", __func__);
			g_sts_xcpt_off = 1;
		}
	/*--buck oc--*/
		oc_sts = upmu_get_reg_value(MT6357_OC_SDN_STS0);
		print("[PMIC]PSOCSTATUS[0x%x]=0x%x\n",
			MT6357_OC_SDN_STS0, oc_sts);
