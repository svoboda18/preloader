...
#if ENABLE_TMRRI_NEW_MODE
    //[fcCervino DVT](1)dram auto refersh rate by hardware mr4 for rank0 and rank1; (2)After suspend resume, HW MR4 will be fire autoly (Berson)
...
#if (fcFOR_CHIP_ID == fcCervino) //ChA-B1, ChB-B0/B1/CA are used as RX
...
#if (fcFOR_CHIP_ID == fcCervino)                                        //before switch clock from 26M to PHY, need to init PHY clock first
...
#if IMPEDANCE_HW_SAVING_OLD_MODE
...
#if IMPEDANCE_HW_SAVING_OLD_MODE
    vIO32WriteFldAlign(DRAMC_REG_REFCTRL0+SHIFT_TO_CHB_ADDR, 1, REFCTRL0_DRVCGWREF);
    vIO32WriteFldAlign_All(DRAMC_REG_REFCTRL0, 1, REFCTRL0_DQDRVSWUPD);
#endif
}
#endif
