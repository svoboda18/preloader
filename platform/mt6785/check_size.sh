...
# 345 KB Preloader      : 0x0020_1000 ~ 0x0025_77FF
# 80 KB Preloader BSS2  : 0x0026_7000 ~ 0x0027_B000
PL_START_ADDR=0x00201000
PL_END_ADDR=0x002577FF
...
# Since secure buffer allocate 48 KB, thus we evaluate
# bss2 size up to 80 KB on L3C.
PL_MAX_RAM_SIZE=81920

# Keep ROM size smaller than 345 KB
# Keep ROM size smaller than 0x0025_77FF - 0x0020_1000 - Signature
...
PL_RAM_SIZE=$(mawk '{if($3=="_bss2_start") {BSS_START=("0x" $1)+0} ;
