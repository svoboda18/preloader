...
void CheckRxPinMux(DRAMC_CTX_T *p)
{
    ...
    for (i=0; i<DQ_DATA_WIDTH_LP4; i+=1)
    {
        ...
        DramcRxWindowPerbitCal(p, 0, NULL);//RDDQC
...
{
    U8 u1Txdelay[DQ_DATA_WIDTH_LP4] = {0};
    U8 u1TxIndex[DQ_DATA_WIDTH_LP4] = {0};
    int iOriFirstPass[DQ_DATA_WIDTH_LP4] = {0};
    int i = 0; 
    int j = 0;
    int iMin = 0xff;
    ...
    for(j = 0; j<DQ_DATA_WIDTH_LP4; j++)
    {
       iOriFirstPass[j] = gFinalTXPerbitFirstPass[p->channel][j];
    ...
    for(i = 0; i<DQ_DATA_WIDTH_LP4; i++)
    {
        ...
        for(j = 0; j<DQ_DATA_WIDTH_LP4; j++)
        {
            if((gFinalTXPerbitFirstPass[p->channel][j] - iOriFirstPass[j]) <= iMin)
            {
                iMin = gFinalTXPerbitFirstPass[p->channel][j] - iOriFirstPass[j];
                u1MinIdx = j;
            }
        }
...
void DramcPostPackageRepair(DRAMC_CTX_T *p)
{
    ...
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_RK0_DUMMY_RD_ADR), 0, RK0_DUMMY_RD_ADR_DMY_RD_RK0_LEN);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_RK0_DUMMY_RD_BK), u2FailRow, RK0_DUMMY_RD_BK_DMY_RD_RK0_ROW_ADR);
