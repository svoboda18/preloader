...
DRAM_STATUS_T DramcStartDQSOSC(DRAMC_CTX_T *p)
{
    ...
    if(u4TimeCnt==0)//time out
    {
        ...
        mcDUMP_REG_MSG(("Start fail (time out)\n"));
...
DRAM_STATUS_T DramcDQSOSCAuto(DRAMC_CTX_T *p)
{
    ...
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_RKCFG), 0, RKCFG_DQSOSC2RK); //@Jouling, for sw tx calib. hw tx is ignored.
    ...
    mcDUMP_REG_MSG(("[DQSOSCAuto] RK%d, (LSB)MR18= 0x%x, (MSB)MR19= 0x%x, tDQSOscB0 = %d ps tDQSOscB1 = %d ps\n", u1GetRank(p), u2MR18, u2MR19, u2DQSOsc[0], u2DQSOsc[1]));
#endif
...
DRAM_STATUS_T DramcDQSOSCShuSettings(DRAMC_CTX_T *p)
{
    ...
    U8 u1FILT_PITHRD = 0;
    U8 u1W2R_SEL = 0; // @Jouling, SHU_WODT_TXUPD_W2R_SEL for HW TX tracking PI update timing
    ...
    if ((p->frequency%100) >= 50) // @Darren,  round off >= 50
        u1DQSOSCRCNT++;
    ...
    //@Jouling, DQSOSC_PRDCNT for CKE pull high period after DQSOSC start command
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_SHU_DQSOSC_PRD), u2PRDCNT, SHU_DQSOSC_PRD_DQSOSC_PRDCNT); //@Darren, unit: 16MCK and tOSCO=40ns/MCK
...
#if __ETT__
void CheckTxTrackingHwLimit(DRAMC_CTX_T *p)
{
    U8 u1ShuLevel=0;
    U8 u1DQSOSCENDIS=0, u1RWOFOEN=0;

    u1ShuLevel = u4IO32ReadFldAlign(DRAMC_REG_SHUSTATUS, SHUSTATUS_SHUFFLE_LEVEL);
    u1DQSOSCENDIS = u4IO32ReadFldAlign(DRAMC_REG_SHU_SCINTV+(SHU_GRP_DRAMC_OFFSET*u1ShuLevel), SHU_SCINTV_DQSOSCENDIS);
    u1RWOFOEN = u4IO32ReadFldAlign(DRAMC_REG_PERFCTL0, PERFCTL0_RWOFOEN);

    // u1DQSOSCENDIS = 0 is Enable tx tracking
    if ((u1RWOFOEN==DISABLE) && (u1DQSOSCENDIS==DISABLE))
    {
        mcSHOW_ERR_MSG(("[CheckTxTrackingHwLimit][Error] Dramc Hw Limitation for RWOFOEN=0 & Enable Tx tracking !!!\n"));
        while(1);
    }
}
#endif
...
void Enable_TX_Tracking(DRAMC_CTX_T *p, U32 u4DramcShuOffset, U16 u2Frequency)
{
    //DDR800 do not enable tracking
    if (u2Frequency == 400) //add if(u1ShuffleIdx==DRAM_DFS_SRAM_MAX) to avoid enable tx-tracking when running DDR800 as RG-SHU0
    {
...
void Enable_RDSEL_Tracking(DRAMC_CTX_T *p, U32 u4DramcShuOffset, U16 u2Frequency)
{
    //DDR800 do not enable tracking
    //if (p->pDFSTable->shuffleIdx == SRAM_SHU9) //add if(u1ShuffleIdx==DRAM_DFS_SRAM_MAX) to avoid enable tx-tracking when running DDR800 as RG-SHU0
    if (u2Frequency >= 1866) // @Darren, It only enable for SHU0 (DDR3733/DDR4266) after DVT verify
    {
    ...
    {
        vIO32WriteFldMulti_All(DRAMC_REG_SHU_RDSEL_TRACK+u4DramcShuOffset, P_Fld(0x0, SHU_RDSEL_TRACK_R_RDSEL_TRACK_EN) 
                                                                         | P_Fld(0x1, SHU_RDSEL_TRACK_R_RDSEL_HWSAVE_MSK));
    }
}
#endif
...
void DramcRxDlyTrackDebug(DRAMC_CTX_T *p)
{
    ...
    vIO32WriteFldMulti_All(DRAMC_REG_ADDR(DRAMC_REG_RK0_DUMMY_RD_ADR), P_Fld(0, RK0_DUMMY_RD_ADR_DMY_RD_RK0_COL_ADR)
                        | P_Fld(0, RK0_DUMMY_RD_ADR_DMY_RD_RK0_LEN));
    vIO32WriteFldAlign_All(DRAMC_REG_RK0_DUMMY_RD_BK, 2, RK0_DUMMY_RD_BK_DMY_RD_RK0_ROW_ADR);
    vIO32WriteFldMulti_All(DRAMC_REG_ADDR(DRAMC_REG_RK1_DUMMY_RD_ADR), P_Fld(0, RK1_DUMMY_RD_ADR_DMY_RD_RK1_COL_ADR)
                        | P_Fld(0, RK1_DUMMY_RD_ADR_DMY_RD_RK1_LEN));
    vIO32WriteFldAlign_All(DRAMC_REG_RK1_DUMMY_RD_BK, 2, RK1_DUMMY_RD_BK_DMY_RD_RK1_ROW_ADR);
...
void DramcDummyReadAddressSetting(DRAMC_CTX_T *p)
{
    ...
    for (channelIdx=CHANNEL_A; channelIdx<CHANNEL_NUM; channelIdx++)
    {
        ...
        for (rankIdx=RANK_0; rankIdx<RANK_MAX; rankIdx++)
        {
            ...
            vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_RK0_DUMMY_RD_ADR), P_Fld(dram_addr.col, RK0_DUMMY_RD_ADR_DMY_RD_RK0_COL_ADR)
                                | P_Fld(0, RK0_DUMMY_RD_ADR_DMY_RD_RK0_LEN));
            vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_RK0_DUMMY_RD_BK), dram_addr.row, RK0_DUMMY_RD_BK_DMY_RD_RK0_ROW_ADR);
...
void DramcImpedanceTrackingEnable(DRAMC_CTX_T *p)
{
    ...
    vIO32WriteFldAlign_All(DRAMC_REG_REFCTRL0, 1, REFCTRL0_DRVCGWREF); //@Maoauo, Wait AB refresh to avoid IO drive via logic design
