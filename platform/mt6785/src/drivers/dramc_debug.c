...
extern const U16 gRXVref_Voltage_Table_LP4[EYESCAN_RX_VREF_RANGE_END+1];
...
void DramcRegDumpRange(U32 u4StartAddr, U32 u4EndAddr)
{
    ...
    for(ii = u4StartAddr; ii <= u4EndAddr; ii += 4)
    {
        ...
        mcDUMP_REG_MSG(("Reg(0x%xh) Address 0x%X = 0x%X\n", (ii&0xfff)>>2, ii, (*(volatile unsigned int *)(ii))));
...
void DumpAoShuReg(U32 u4DramcShuOffset, U32 u4DDRPhyShuOffset)
{
...
DRAM_STATUS_T DramcRegDump(DRAMC_CTX_T *p, DRAM_DFS_SRAM_SHU_T sram_shu_level)
{
    ...
    DumpAoNonShuReg();
    DumpAoShuReg(u4DramcShuOffset, u4DDRPhyShuOffset);
    DumpNaoReg();
...
void DumpAoReg(DRAMC_CTX_T *p)
{
    ...
    {
        DramcDFSDirectJump_SPMMode(p, u1ShuffleIdx); //fill conf SHU0 && SHU1 from SRAM SHU(0~9) while DVFS twice
        DramcDFSDirectJump_SPMMode(p, u1ShuffleIdx);
        mcSHOW_DBG_MSG(("\n\tSRAM SHU%d", u1ShuffleIdx));
        DumpAoShuReg(DRAMC_AO_SHU_BASE, DDRPHY_AO_SHU_BASE);    
    }
    DramcDFSDirectJump_SPMMode(p, u1BkShuffleIdx); //Restore to original freq
    DramcDFSDirectJump_SPMMode(p, u1BkShuffleIdx); 
...
void DumpShuRG(DRAMC_CTX_T *p)
{
    ...
    vIO32WriteFldAlign_All(DDRPHY_SRAM_DMA0, 0x1, SRAM_DMA0_R_APB_SLV_SEL);
    vIO32WriteFldAlign_All(DDRPHY_SRAM_DMA2, 0x1, SRAM_DMA2_R_APB_DMA_DBG_ACCESS);
    //mcDUMP_REG_MSG(("\n[dumpRG]AONonSHURG-start\n"));
    //DumpAoNonShuReg();
    //mcDUMP_REG_MSG(("\n[dumpRG]AONonSHURG-end\n"));
...
{
    {"REFP_ARBMASK_PBR2PBR_PA_DIS", 0x0, 0x0},
    {"COMBCLKCTRL", 0x1, 0x1},
    ...
    {"DVFS_CG_OPT", 0x0, 0x0},
    {"RX_DQ_EYE_SEL_B3", 0x0, 0x0},
    {"RX_DQ_EYE_SEL_B2", 0x0, 0x0},
    {"RX_DQ_EYE_SEL_B1", 0x0, 0x0},
    {"RX_DQ_EYE_SEL", 0x0, 0x0},
    {"DCBLNCINS", 0x0, 0x0},
    {"DCBLNCEN", 0x0, 0x0},
    {"EYESCAN_DQS_OPT", 0x0, 0x0},
    {"EYESCAN_NEW_DQ_SYNC_EN", 0x0, 0x0},
    {"EYESCAN_DQ_SYNC_EN", 0x0, 0x0},
    {"EYESCAN_TOG_OPT", 0x0, 0x0},
    {"EYESCAN_CHK_OPT", 0x0, 0x0},
    {"EYESCAN_RD_SEL_OPT", 0x0, 0x0},
    {"RX_MIOCK_JIT_LIMIT", 0xFFFFFFFF, 0xFFFFFFFF},
...
GOLDEN_FIELD_T golden_setting_anwer[] =
{
    {"REFP_ARBMASK_PBR2PBR_PA_DIS", 0x1, 0x1},
    {"COMBCLKCTRL", 0x0, 0x0},
...
DRAM_STATUS_T CheckGoldenSettingSHU(DRAMC_CTX_T *p)
{
    DRAM_STATUS_T eStatus = DRAM_OK;
    ...
    eStatus |= CheckGoldenField(p, DDRPHY_SHU_B1_DQ7, SHU_B1_DQ7_R_DMRANKRXDVS_B1, "R_DMRANKRXDVS_B1");
...
DRAM_STATUS_T CheckGoldenSetting(DRAMC_CTX_T *p)
{
    ...
    //Non shuffle part
    eStatus |= CheckGoldenField(p, DRAMC_REG_MISCTL0, MISCTL0_REFP_ARBMASK_PBR2PBR_PA_DIS, "REFP_ARBMASK_PBR2PBR_PA_DIS");
    ...
    eStatus |= CheckGoldenField(p, DRAMC_REG_EYESCAN, EYESCAN_RX_DQ_EYE_SEL_B3, "RX_DQ_EYE_SEL_B3");
    eStatus |= CheckGoldenField(p, DRAMC_REG_EYESCAN, EYESCAN_RX_DQ_EYE_SEL_B2, "RX_DQ_EYE_SEL_B2");
    eStatus |= CheckGoldenField(p, DRAMC_REG_EYESCAN, EYESCAN_RX_DQ_EYE_SEL_B1, "RX_DQ_EYE_SEL_B1");
    eStatus |= CheckGoldenField(p, DRAMC_REG_EYESCAN, EYESCAN_RX_DQ_EYE_SEL, "RX_DQ_EYE_SEL");
    eStatus |= CheckGoldenField(p, DRAMC_REG_EYESCAN, EYESCAN_DCBLNCINS, "DCBLNCINS");
    eStatus |= CheckGoldenField(p, DRAMC_REG_EYESCAN, EYESCAN_DCBLNCEN, "DCBLNCEN");
    eStatus |= CheckGoldenField(p, DRAMC_REG_EYESCAN, EYESCAN_EYESCAN_DQS_OPT, "EYESCAN_DQS_OPT");
    eStatus |= CheckGoldenField(p, DRAMC_REG_EYESCAN, EYESCAN_EYESCAN_NEW_DQ_SYNC_EN, "EYESCAN_NEW_DQ_SYNC_EN");
    eStatus |= CheckGoldenField(p, DRAMC_REG_EYESCAN, EYESCAN_EYESCAN_DQ_SYNC_EN, "EYESCAN_DQ_SYNC_EN");
    eStatus |= CheckGoldenField(p, DRAMC_REG_EYESCAN, EYESCAN_EYESCAN_TOG_OPT, "EYESCAN_TOG_OPT");
    eStatus |= CheckGoldenField(p, DRAMC_REG_EYESCAN, EYESCAN_EYESCAN_CHK_OPT, "EYESCAN_CHK_OPT");
    eStatus |= CheckGoldenField(p, DRAMC_REG_EYESCAN, EYESCAN_EYESCAN_RD_SEL_OPT, "EYESCAN_RD_SEL_OPT");
    eStatus |= CheckGoldenField(p, DRAMC_REG_MIOCK_JIT_MTR, MIOCK_JIT_MTR_RX_MIOCK_JIT_LIMIT, "RX_MIOCK_JIT_LIMIT");
    ...
    if(gAndroid_DVFS_en)
    {
        for(u1ShuffleIdx = 0; u1ShuffleIdx <= DRAM_DFS_SRAM_MAX; u1ShuffleIdx++) 
        {
            ...
            DramcDFSDirectJump_SPMMode(p, u1SramShuffleIdx); //fill conf SHU0 && SHU1 from SRAM SHU(0~9) while DVFS twice
            DramcDFSDirectJump_SPMMode(p, u1SramShuffleIdx);
            eStatus |= CheckGoldenSettingSHU(p);   
        }
    }
    else
    {
