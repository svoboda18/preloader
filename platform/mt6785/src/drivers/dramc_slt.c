...
void UpdateSltDFSTbltoDDR2400(DRAMC_CTX_T *p, U8 u1final_shu, U8 u1slt_shu)
{
    U16 u2Freq = 0;
    DRAM_CHANNEL_T channel_idx = CHANNEL_A, backup_ch;
    S8 u1ShuIdx;

    if (p->frequency != 1200)
    {
        mcSHOW_ERR_MSG(("[UpdateSltDFSTbltoDDR2400] Error p->frequency(%d) != 1200\n", p->frequency));
        while(1);
    }

    // Update SRAM shuffle from >=DDR3200 to DDR2400 for SLT only
    for (u1ShuIdx = DRAM_DFS_SHUFFLE_1; u1ShuIdx < DRAM_DFS_SHUFFLE_MAX; u1ShuIdx++)
    {
        u2Freq = GetFreqBySel(p, gFreqTbl[u1ShuIdx].freq_sel);
        if (u2Freq >= 1600) // freq>=DDR3200 change to DDR2400_shu3
        {
            gFreqTbl[u1ShuIdx].freq_sel = LP4_DDR2400;
            #if TDQSCK_PRECALCULATION_FOR_DVFS
            backup_ch = vGetPHY2ChannelMapping(p);
            vSet_Current_ShuLevel(p, gFreqTbl[u1ShuIdx].shuffleIdx);
            for(channel_idx=CHANNEL_A; channel_idx<p->support_channel_num; channel_idx++)
            {
                vSetPHY2ChannelMapping(p, channel_idx);
    ...
    vSet_Current_ShuLevel(p, u1final_shu); // rollback to SRAM_SHU4
    for (u1ShuIdx = DRAM_DFS_SHUFFLE_1; u1ShuIdx < DRAM_DFS_SHUFFLE_MAX; u1ShuIdx++)
    {
        if (gFreqTbl[u1ShuIdx].shuffleIdx <= SRAM_SHU2) // update all DDR2400 shuffle RG jump ratio for SHU0/1/2
            DramcSaveToShuffleSRAM(p, DRAM_DFS_SHUFFLE_1, gFreqTbl[u1ShuIdx].shuffleIdx);                
    }

    vSetDFSFreqSelByTable(p, get_FreqTbl_by_shuffleIndex(p, u1slt_shu));
    DramcDFSDirectJump_SPMMode(p, SRAM_SHU2);
    DramcDFSDirectJump_SPMMode(p, u1slt_shu);  // DDR2400 0.65V for SLT
}
#endif
