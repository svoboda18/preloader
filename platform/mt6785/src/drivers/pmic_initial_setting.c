...
/* MD DVFS request that vmodem=0.875V, vsram_md=0.99375V */
/* GPU request that VGPU11=0.7V, VPU(sram)=0.85V */
...
void pmic_default_voltage(void)
{
	...
	if (mtk_regulator_set_voltage(&reg_vsram_others, vsram_others_vol, vsram_others_max_uV))
		ret |= (1 << MTK_REGULATOR_VSRAM_OTHERS);
	if (mtk_regulator_set_voltage(&reg_vpu, vpu_vol, vpu_max_uV))
		ret |= (1 << MTK_REGULATOR_VPU);
...
#endif
	pal_log_info("RG_STRUP_EXT_PMIC_PG_ENB=1\n");
	pmic_config_interface(PMIC_RG_STRUP_EXT_PMIC_PG_ENB_ADDR, 1,
