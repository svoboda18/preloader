...
U32 u4RegBaseAddrTraslate(DRAM_RANK_T eRank, U32 u4reg_addr)
{
    ...
    if (eRank == RANK_1)
    {
        ...
        else if (u4RegType >=2 && u4RegType <=3)
        {
            ...
            else if (u4Offset >= (DRAMC_REG_RK0_PRE_TDQSCK25-DRAMC_AO_BASE_ADDRESS) && u4Offset < (DRAMC_REG_RK1_PRE_TDQSCK25-DRAMC_AO_BASE_ADDRESS))
            {
                u4Offset += 0x18;
            }
        }
        ...
        else
        {
            ...
            else if (u4Offset >= (DDRPHY_SHU_R0_B0_DQ0-DDRPHY_AO_BASE_ADDR) && u4Offset < (DDRPHY_SHU_R1_B0_DQ0-DDRPHY_AO_BASE_ADDR))
            {
                u4Offset += 0x100;
            }      
...
U8 ucDram_Register_Write(DRAMC_CTX_T *p, U32 u4reg_addr, U32 u4reg_mask, U32 u4reg_value)
#else
...

{
    ...
    if(RegLogEnable)
    {
        ...
        mcDUMP_REG_MSG(("\n[REG_ACCESS_PORTING_DBG]  ucDramC_Register_Write Reg(0x%X) Mask 0x%X= 0x%X\n", u4reg_addr, u4reg_mask, u4reg_value));
    }
#endif
#if REG_ACCESS_NAO_DGB
    if(RegNotAO)
...
void vIO32Write4BMsk2(DRAMC_CTX_T *p, U32 reg32, U32 val32, U32 msk32)
{
    ...
    #if REG_ACCESS_PORTING_DGB
    ucDram_Register_Write(p, reg32, msk32, u4Val);
    #else
...
void vIO32Write4BMsk_All2(DRAMC_CTX_T *p, U32 reg32, U32 val32, U32 msk32)
{
    ...
    for(ii=0; ii< u1AllCount; ii++)
    {
        ...
        u4Val = ((u4Val &~msk32)|val32);    
    #if REG_ACCESS_PORTING_DGB
        ucDram_Register_Write(p, reg32+((U32)ii<<POS_BANK_NUM), msk32, u4Val);
    #else	
...
U32 u4IO32ReadFldAlign_Phy_Byte2(DRAMC_CTX_T *p, U8 ucByteIdx, U32 reg32, U32 fld)
{
    ...
    if (reg32 >= (DDRPHY_SHU_R0_B0_DQ0-DDRPHY_AO_BASE_ADDR) && reg32 < (DDRPHY_SHU_R0_B1_DQ0-DDRPHY_AO_BASE_ADDR))
    {
        offset = 0x50;
        ///TODO: BE CAREFUL. CLK (PHY) -> DQS (DRAM)
        if (reg32 >= (DDRPHY_SHU_R0_B0_DQ0-DDRPHY_AO_BASE_ADDR) && reg32 < (DDRPHY_SHU_R0_B0_DQ6-DDRPHY_AO_BASE_ADDR))
        {
            except_offset = 0x0;
        }
        else
    ...
    else if (reg32 >= (DDRPHY_SHU_B0_DQ0-DDRPHY_AO_BASE_ADDR) && reg32 < (DDRPHY_SHU_B1_DQ0-DDRPHY_AO_BASE_ADDR))
    {
        offset = 0x80;
        except_offset = 0;
    }
    ...
    switch(ucByteIdx)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            if(reg32==(DDRPHY_SHU_R0_B0_DQ7&0xffff) && fld==SHU_R0_B0_DQ7_RK0_ARPI_DQM_B0)
            {
...
void vIO32Write4BAPBMsk2(DRAMC_CTX_T *p, U32 reg32, U32 val32, U32 start32, U32 len32)
{
    U32 u4Val = 0;

    u4Val |= val32; // DATA[15:0]
    u4Val |= len32 << 22; //LEN[25:22]
    u4Val |= start32 << 27; //START[31:27]
    //mcSHOW_ERR_MSG(("[vIO32Write4BAPBMsk2] val32 = 0x%x, len32 = 0x%x, start32 = 0x%x\n", val32, len32, start32));
    ucDram_Register_Write(p, reg32, u4Val);
}

void vIO32WriteAPB4BMsk_All2(DRAMC_CTX_T *p, U32 reg32, U32 val32, U32 start32, U32 len32)
{
    U32 u4Val = 0;
    U8 ii, u1AllCount;
    U32 u4RegType = (reg32 & (0xf <<POS_BANK_NUM));
    ...
    u4Val |= (val32 & 0xffff); // DATA[15:0]
    u4Val |= (len32 & 0xf) << 22; //LEN[25:22]
    u4Val |= (start32 & 0x1f) << 27; //START[31:27]
    //mcSHOW_ERR_MSG(("[vIO32WriteAPB4BMsk_All2] val32 = 0x%x, len32 = 0x%x, start32 = 0x%x\n", val32, len32, start32));
...
#endif // end of ENABLE_APB_MASK_WRITE
