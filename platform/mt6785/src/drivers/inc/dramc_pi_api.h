...
//SW option
//#define DUAL_FREQ_K 1 // It's removed. The DVFS to control by ENABLE_DRAM_SINGLE_FREQ_SELECT macro (@dramc_pi_main.c)
#define ENABLE_RANK_NUMBER_AUTO_DETECTION 1 //only can enable when DUAL_RANK_ENABLE is 1
#if APOLLO_SPECIFIC
#define ENABLE_EYESCAN_GRAPH 0 //Lafit* default enable //__ETT__ //draw eye diagram after calibration, if enable, need to fix code size problem.
#else
#define ENABLE_EYESCAN_GRAPH 1 //Lafit* default enable //__ETT__ //draw eye diagram after calibration, if enable, need to fix code size problem.
#endif
#define EYESCAN_GRAPH_SAVE_SPACE_VREF_DIV 1  // 1 (origin) or 2 (div 2)(save 9K size)
#define EYESCAN_RX_VREF_RANGE_END          63   //field is 6 bit, but can only use 0~63
...
#define RX_VREF_RANGE_BEGIN_ODT_ON      12
#define RX_VREF_RANGE_END          63   //field is 6 bit, but can only use 0~63
#define RX_VREF_RANGE_STEP         2
#endif
...
#define DRAMC_SHU_OLD_MODE 0 //it must DRAMC_DFS_MODE=2
...
#define MR13_RRO_VALUE 1 //MR4 refresh rate option, 0:Disable codes 001 and 010 in MR4 OP[2:0]   1:Enable all codes in MR4 OP[2:0]

//Lewis@20170508: Undefine DUMMY_READ_FOR_DQS_GATING_RETRY since SPM not use it.
//Lewis@20180323: DDR4266 will use DUMMY_READ_FOR_DQS_GATING_RETRY(PIC: YT)
 ...
 */
#if !APOLLO_SPECIFIC    //Lafit* default enable (__ETT__)
...
#define MRFSP_TERM_FREQ 1333
#define ENABLE_DVFS_CDC_SYNCHRONIZER_OPTION 1
#define ENABLE_DVFS_BYPASS_MR13_FSP 0 //TODO: Darren
#define ENABLE_FIX_PRE8_DVFS_AT_CKEPRD 1
#define ENABLE_APB_MASK_WRITE 0 //APB Mask Write for DVT
#define ENABLE_FIX_SHORT_PLUSE 0
...
#if (ENABLE_REMOVE_MCK8X_UNCERT_DFS_OPTION || ENABLE_REMOVE_MCK8X_UNCERT_LOWPOWER_OPTION)
#define ENABLE_MCK8X_MODE 1 //PIC: KaiHsin
#endif

#define ENABLE_APHY_DLL_IDLE_MODE_OPTION 1
...
#endif
#define CMD_CKE_WORKAROUND_FIX 1 //Eig_er E1: have to use workaround, E2: remove workaround because bug is fix
#define ENABLE_DDR800_OPEN_LOOP_MODE_OPTION 1
#define ENABLE_DDR800_OPEN_LOOP_PHASE_PREDICTION 0
#define ENABLE_DDR800_PHASE_MODE_INTERSECTION 0 //@Darren, test only
#define RX_PIPE_BYPASS_EN 1
...
#undef CBT_MOVE_CA_INSTEAD_OF_CLK
#define CBT_MOVE_CA_INSTEAD_OF_CLK 0  //FT Pattern no need to do this
#endif
#define ENABLE_PICG_FREERUN_TO_SHORT_PULSE 0
...
#define ENABLE_TPBR2PBR_REFRESH_TIMING 1 //@Darren, for 10GB

#define ENABLE_PINMUX_FOR_RANK_SWAP 0 //@Darren, test only
#define ENABLE_SAMSUNG_NT_ODT 0
//========================== For 1:4 mode workaround ===================================
#define ENABLE_CBT_DIV4_WA 0
#define ENABLE_RDDQC_DIV4_WA 0
...
#endif
#define ENABLE_MPLL_SSC 1 // only for Ev_erest ETT
#else // not ETT
...
//========== FT DDRPHY INTERNAL LPBK & RXTX Margin TEST Definition ============
...
#undef CMD_CKE_WORKAROUND_FIX
#define CMD_CKE_WORKAROUND_FIX 0

#define VCORE_SHMOO
#ifdef VCORE_SHMOO
#define VCORE_SHMOO_RX_TX_SCAN 1    //for Rx_Tx_scan pattern
#endif
...
#define EMI_USE_TA2 1   // 0:CPU write, 1:TA2,  DVsim/Dsim use TA2, but 1:4 mode must use cpu write(because TA2 not support 1:4 mode)
#else
#define EMI_USE_TA2 0
#endif
#define EMI_LPBK_K_RK1    1
...
#define RUNTIME_SHMOO_RX_VREF_RANGE_END          63   
#define RUNTIME_SHMOO_RX_TEST_MARGIN        2 //8
...
#define TOTAL_AC_TIMING_NUMBER  (AC_TIMING_NUMBER_LP4)
...
typedef struct _SAVE_TIME_FOR_CALIBRATION_T
{
    ...
    // RX perbit
    U8 u1RxWinPerbitVref_Save[CHANNEL_NUM][DQS_NUMBER_LP4];
...
#endif
extern int doe_get_config(char* feature);
...
#if APOLLO_SPECIFIC
    #if PRINT_CALIBRATION_SUMMARY
    U32 aru4CalResultFlag[2][RANK_MAX];// record the calibration is fail or success,  0:success, 1: fail
    U32 aru4CalExecuteFlag[2][RANK_MAX]; // record the calibration is execute or not,  0:no operate, 1: done
    U32 SWImpCalResult;
    U32 SWImpCalExecute;
    #endif
    #if WRITE_LEVELING_MOVE_DQS_INSTEAD_OF_CLK
    BOOL arfgWriteLevelingInitShif[2][RANK_MAX];
    #endif
#else
    #if PRINT_CALIBRATION_SUMMARY
...
typedef struct _REG_FLD_DQS_PRE_K
{
    U32 u4UI_Fld;
    U32 u4PI_Fld;
    U32 u4UI_Fld_P1[2];
...
// Fld( width, shift, AC), and "AC" will NOT be used
#define MR13_FSP_OP     (Fld(1, 7, AC_FULLB0))
#define MR13_FSP_WR     (Fld(1, 6, AC_FULLB0))
#define MR13_DMD        (Fld(1, 5, AC_FULLB0))
#define MR13_PRO        (Fld(1, 4, AC_FULLB0))
#define MR13_VRCG       (Fld(1, 3, AC_FULLB0))
#define MR13_CBT        (Fld(1, 0, AC_FULLB0))

void DramcMRWriteFldMsk(DRAMC_CTX_T *p, U8 mr_idx, U8 listValue, U8 msk, U8 UpdateMode);
...
DRAM_STATUS_T DramcRxWindowPerbitCal(DRAMC_CTX_T *p, U8 u1UseTestEngine, U8 *u1AssginedVref);
...
extern U16 gFinalRXPerbitFirstPass[CHANNEL_NUM][DQ_DATA_WIDTH];
#endif

#if PINMUX_AUTO_TEST_PER_BIT_TX
extern U8 gTX_check_per_bit_flag;
extern U16 gFinalTXPerbitFirstPass[CHANNEL_NUM][DQ_DATA_WIDTH];
#endif
...
extern S8 gDramcSwImpedanceAdjust[IMP_VREF_MAX][IMP_DRV_MAX];//ODT_ON/OFF x DRVP/DRVN/ODTP/ODTN
