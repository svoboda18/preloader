...
# If L2C SRAM Size limitation is 128KB (Denali-2)
# Keep ROM size smaller than 0x0022_0000 - 0x0020_1000 =0x1F000 (126976) Bytes
if [ "${MACH_TYPE}" == "mt6735m" ]; then
    PL_MAX_ROM_SIZE=$(( 126976 - 292 ))
fi

if [ "${MACH_TYPE}" == "mt6737m" ]; then
    PL_MAX_ROM_SIZE=$(( 126976 - 292 ))
fi
...
PL_RAM_SIZE=$(awk '{if($3=="_bss_start") {BSS_START=("0x" $1)+0} ;
