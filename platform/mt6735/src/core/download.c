...
static u16 usbdl_verify_da(u8* da_addr, u32 da_len, u32 sig_len)
{
    ...
    if ((FALSE == seclib_sec_usbdl_enabled(TRUE)) || (FALSE == seclib_sec_boot_enabled(TRUE))) {
        printf("%s usbdl_verify_da: DA validation disabled\n", MOD);
    } else {
        /* init download agent authentication key */
        if (da_auth_init() != 0) {
            printf("%s usbdl_verify_da: DA init key fail\n", MOD);
        ...
        if (da_len > DA_RAM_LENGTH) {
            printf("%s usbdl_verify_da: DA (0x%x) relocate size (0x%x) fail\n", MOD, da_len, DA_RAM_LENGTH);
        ...
        if (sec_auth(da_addr, (da_len - sig_len), sig_addr, sig_len) != 0) {
            printf("%s usbdl_verify_da: DA validation fail\n", MOD);
			status = E_DAA_SIG_VERIFY_FAILED;
        }
    }
#else
    if (FALSE == seclib_sec_usbdl_enabled(TRUE)) {
        ...
        printf("%s usbdl_verify_da: DA download is forbidden\n", MOD);
		status = E_DAA_SIG_VERIFY_FAILED;
    }
#endif
#endif
...
static void usbdl_send_image(void)
{
	...
	print("%s SEND: addr = 0x%x\n", MOD, img_addr);
	if (!img_addr) {
		print("%s Unknown image\n", MOD);
		//return;
	}
	...
	print("%s SEND: PL Checksum = 0x%x\n", MOD, my_checksum32);

	usbdl_get_dword(&checksum32);
	print("%s SEND: XFLASH checksum = 0x%x\n", MOD, checksum32);
	...
	//usbdl_put_word(status);
      ...
      if(id == IMAGE_ATF_ID) {
          ...
          int ret = tee_verify_image(&maddr, dsize);
          if (ret)
...
static void usbdl_send_da(void)
{
...
#ifdef DBG_PRELOADER
    printf("%s usbdl_send_da: status2 is 0x%x\n", MOD, status);
#endif
    ...
    if (SEC_OK != status) {
        printf("%s usbdl_send_da: clean invalid da\n", MOD);
...
int usbdl_handler(struct bldr_comport *comport, u32 hshk_tmo_ms)
{
    ...
    while (1) {
		...
		if(cnt > 20) {
            printf("%s Unhandled CMD Over 20 times! Exit.\n", MOD);
            break;
        }
    }
    return 0;
