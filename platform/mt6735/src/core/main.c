...
static int set_sw_segment_code(void)
{
    u32 segment_code = seclib_get_devinfo_with_index(24) >> 25;
    print("OLD seclib_get_devinfo_with_index=0x%x\n", segment_code);
    switch(segment_code){
        case 0x52:
    ...
    print("NEW seclib_get_devinfo_with_index=0x%x\n", (seclib_get_devinfo_with_index(24)>>25));
    return 0;
}
#endif
...
if (0 == aarch64_slt_done())
{
    ...
    *(unsigned int*) AARCH64_SLT_DONE_ADDRESS = AARCH64_SLT_DONE_MAGIC;
    ...
    printf("%s Aarch64 Kernel SLT , jump to 64 bit kernel, address: 0x%x\n", MOD,jump_addr);
...
print("EMI MPUS=0x%x; MPUT=0x%x\n", *(unsigned int*) 0x102031F0, *(unsigned int*) 0x102031F8);
