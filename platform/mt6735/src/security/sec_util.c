...
{
	return (U8 *)(g_sec_buf.sram_sec_img_buf);
}

U32 sec_util_get_img_buf_sz(void)
{
...
{
	return (U8 *)(g_sec_buf.sram_sec_working_buf);
}

U32 sec_util_get_working_buf_sz(void)
{
...
{
	return (U8 *)(g_dram_buf->sec_lib_heap_buf);
}
...
{
	return (U8 *)(g_dram_buf->sec_secro_buf);
}

U32 sec_util_get_secro_buf_sz(void)
{
...
#define SEC_PL_ERASE_SIZE 2048
	...
	if (SEC_OK != sec_util_write_image(SBOOT_PART_PL, sec_buf, 0,
					   SEC_PL_ERASE_SIZE)) {
...
error:
	DMSG("error on jumping to fastboot mode\n");
	while (1);
}
