        ...
        switch (request->bRequest)
        {
            ...
            case STDREQ_CLEAR_FEATURE:
                ...
                case USB_RECIP_ENDPOINT:
                    ep_num = request->wIndex & 0x0f;
                    dir =  request->wIndex & USB_DIR_MASK;

                    print ("STDREQ_CLEAR_FEATURE,ep_num=%d, dir=%d\n", ep_num, dir);
                    #if DIAG_COMPOSITE_PRELOADER
                    /* workaround for hangshake with PC tool*/
                    if(dir == USB_DIR_OUT && ep_num == USBD_DIAG_OUT_ENDPOINT) {
                        tool_state_update (1);
                    }
                    #endif
                    return 0;
...
void mt_udc_reset (void)
{
    ...
    */
    ...
    {
        printf ("[USBD] USB High Speed\n");
        enable_highspeed ();
    }
    else
    {
        printf ("[USBD] USB Full Speed\n");
    }
...
static void mt_udc_ep0_setup (void)
{
    ...
    if (((request->bmRequestType) & USB_TYPE_MASK) == USB_TYPE_STANDARD)
    {
        ...
        if (stall)
        {
            printf ("[USB] STANDARD REQUEST NOT SUPPORTED\n");
    ...
    else if (((request->bmRequestType) & USB_TYPE_MASK) == USB_TYPE_CLASS)
    {
        ...
        if (stall)
        {
            printf ("[USB] CLASS REQUEST NOT SUPPORTED\n");
            printf ("       bmRequestType = %x\n",
                request->bmRequestType);
            printf ("       bRequest = %x\n", request->bRequest);
            printf ("       wValue = %x\n", request->wValue);
