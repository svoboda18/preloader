...
#define VMAX_VAL		MV_TO_VAL(1250) 
#define VMIN_VAL		MV_TO_VAL(1150) // XXX: for VCORE_AO & VCORE_PDN
...
#define for_each_det(det) for (det = &ptp_detectors[PTP_DET_SOC]; det < (ptp_detectors + ARRAY_SIZE(ptp_detectors)); det++)
...
static struct ptp_det ptp_detectors[NR_PTP_DET] = {
	[PTP_DET_SOC] = {
		...
		.VBOOT		= MV_TO_VAL(1250), /* 1.0v: 0x30 */
	},
};
...
static void get_devinfo(struct ptp_devinfo *p)
{
	...
	val[6] = seclib_get_devinfo_with_index(25); // ptp_read(0x10206270);	/* M_HW_RES6 */
	val[7] = seclib_get_devinfo_with_index(24); // ptp_read(0x102061B0);	/* M_HW_RES7 */
#if 0
	if (0 == p->PTPINITEN) {
...
static void ptp_init_det(struct ptp_det *det, struct ptp_devinfo *devinfo)
{
	...
	case PTP_DET_SOC:
		...
		det->VBOOT	= upmu_get_reg_value(0x618) - PTPOD_PMIC_OFFSET; //This should be 1.15V 0x48
...
void ptp_init(void)
{
...

	{
		...
		vcore0 = ptp_detectors[PTP_DET_SOC].volt_tbl_init2[0];
		vcore1 =  MV_TO_VAL(1150) + PTPOD_PMIC_OFFSET;
