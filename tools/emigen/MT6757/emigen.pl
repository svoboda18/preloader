...
#=cut                 
    ...
    unless($ENV{PROJECT} eq "mt6757_evb"){
    ...
    if ($DIS_LPDDR3 > 0 && (($DIS_LPDDR2 > 0) || ($MCP_LPDDR2 > 0) || ($DIS_PCDDR3 > 0) || ($MCP_LPDDR4X > 0) || ($DIS_LPDDR4X > 0) || ($MCP_LPDDR4 > 0) || ($DIS_LPDDR4 > 0)))
    {
	    die "[Error] LPDDR2 and LPDDR3 are not allowed to be mixed in the Combo Discrete DRAM list.\n" ;
    }
    if ($DIS_LPDDR4 > 0 && (($DIS_LPDDR2 > 0) || ($DIS_LPDDR3 > 0) || ($MCP_LPDDR2 > 0) || ($MCP_LPDDR3 > 0) || ($DIS_PCDDR3 > 0)))
    {
	    die "[Error] LPDDR2, LPDDR3 and LPDDR4/4X are not allowed to be mixed in the Combo Discrete DRAM list.\n" ;
    }
    if ($DIS_LPDDR4X > 0 && (($DIS_LPDDR2 > 0) || ($DIS_LPDDR3 > 0) || ($MCP_LPDDR2 > 0) || ($MCP_LPDDR3 > 0) || ($DIS_PCDDR3 > 0)))
    {
	    die "[Error] LPDDR2, LPDDR3 and LPDDR4/4X are not allowed to be mixed in the Combo Discrete DRAM list.\n" ;
    }
    if ($DIS_LPDDR4P > 0 && (($DIS_LPDDR2 > 0) || ($DIS_LPDDR3 > 0) || ($MCP_LPDDR2 > 0) || ($MCP_LPDDR3 > 0) || ($DIS_PCDDR3 > 0)))
    {
...
#        #  openoffice saved file workaround END
	...
	else
	{
        ...
        if ($DEV_MODE[$id] eq "CBT_NORMAL_MODE")
	{
            $MODE_VAL[$id] = '0x0'; 
	}
	elsif ($DEV_MODE[$id] eq "CBT_BYTE_MODE1")
	{
            $MODE_VAL[$id] = '0x1'; 
	}
	elsif ($DEV_MODE[$id] eq "CBT_R0_NORMAL_R1_BYTE")
	{
            $MODE_VAL[$id] = '0x2'; 
	}
	elsif ($DEV_MODE[$id] eq "CBT_R0_BYTE_R1_NORMAL")
	{
            $MODE_VAL[$id] = '0x3'; 
	}
	else
...
sub write_tag()
{
    ...
    print FILE pack("a24", "MTK_BLOADER_INFO_v25"); #MT6757
...
sub write_tag_one_element_default_LPDDR3()
{
    ...
    print FILE pack ("L", hex (lc("0x0"))) ;     # MODE_VAL
    $fs = $fs + 4 ;
...
#    print FILE pack ("L", hex (lc("0x0400f050"))) ;     # CHN1_EMI_CONA_VAL
    ...
    print FILE pack ("L", hex (lc("0x1"))) ;     # MODE_VAL
    $fs = $fs + 4 ;
...
sub write_tag_one_element()
{
    ...
    print FILE pack ("L", hex (lc($MODE_VAL[$id]))) ;     # MODE_VAL
    $fs = $fs + 4 ;
