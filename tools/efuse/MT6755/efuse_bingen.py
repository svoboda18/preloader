...
def checkLessThan32BitsHexStringLength(tag_name, hexString, min_index, max_index) :
...
def main():
    ...
    printAndLog("***************************************************************************")
    printAndLog("************** MediaTek 6755 EFUSE XML Parser ([MTK_XML2BIN]) **************")
    printAndLog("****************************** version 1.4.2 ******************************")
    printAndLog("***************************************************************************")
    ...
    EFUSE_c_data_0 = parseXmlTagInnerValue(xml_file, "c_data_0", 16, True)
    EFUSE_c_data_1 = parseXmlTagInnerValue(xml_file, "c_data_1", 16, True)
    EFUSE_c_ctrl1 = parseXmlTagInnerValue(xml_file, "c_ctrl1", 4, True)
    ...
    EFUSE_c2k_sbc_en = parseXmlTagAndAttribute(xml_file, "c_ctrl_0", "c2k_sbc_en")
    EFUSE_md1_sbc_en = parseXmlTagAndAttribute(xml_file, "c_ctrl_0", "md1_sbc_en")
    EFUSE_disable_self_blow = parseXmlTagAndAttribute(xml_file, "c_ctrl_0", "disable_self_blow")
    EFUSE_c_ctrl1_lock = parseXmlTagAndAttribute(xml_file, "c_lock", "c_ctrl1_lock")
    EFUSE_c_ctrl0_lock = parseXmlTagAndAttribute(xml_file, "c_lock", "c_ctrl0_lock")
    EFUSE_c_data1_lock = parseXmlTagAndAttribute(xml_file, "c_lock", "c_data1_lock")
    EFUSE_c_data0_lock = parseXmlTagAndAttribute(xml_file, "c_lock", "c_data0_lock")
    ...
    checkLessThan32BitsHexStringLength("c_ctrl1", EFUSE_c_ctrl1, 0, 13);
    ...
    printAndLog("EFUSE_c_data_0 = " + str(EFUSE_c_data_0))
    printAndLog("EFUSE_c_data_1 = " + str(EFUSE_c_data_1))
    printAndLog("EFUSE_c_ctrl1 = " + str(EFUSE_c_ctrl1))
    printAndLog("EFUSE_c2k_sbc_en = " + str(EFUSE_c2k_sbc_en))
    printAndLog("EFUSE_md1_sbc_en = " + str(EFUSE_md1_sbc_en))
    printAndLog("EFUSE_disable_self_blow = " + str(EFUSE_disable_self_blow))
    printAndLog("EFUSE_c_ctrl1_lock = " + str(EFUSE_c_ctrl1_lock))
    printAndLog("EFUSE_c_ctrl0_lock = " + str(EFUSE_c_ctrl0_lock))
    printAndLog("EFUSE_c_data1_lock = " + str(EFUSE_c_data1_lock))
    printAndLog("EFUSE_c_data0_lock = " + str(EFUSE_c_data0_lock))
    ...
    with open(args.output_bin_name, "wb") as f :
        ...
        writeHexStringToFile(f, EFUSE_c_data_0[0:8])  #0x44
        writeHexStringToFile(f, EFUSE_c_data_0[8:16]) #0x48
        writeHexStringToFile(f, EFUSE_c_data_1[0:8])  #0x4C
        writeHexStringToFile(f, EFUSE_c_data_1[8:16]) #0x50

        writeBitValueToFile(f, [(0, EFUSE_Disable_EMMC_boot), (1, EFUSE_Disable_NAND_boot), (2, EFUSE_USB_download_type), (4, EFUSE_Disable_NAND_boot_speedup)])  #0x54
        writeBitValueToFile(f, [(0, EFUSE_Disable_JTAG), (1, EFUSE_Enable_SBC), (2, EFUSE_Enable_DAA), (3, EFUSE_Enable_SLA),(4, EFUSE_Enable_ACK), (5, EFUSE_Enable_ACC), (6, EFUSE_Enable_SW_JTAG_CON), (7, EFUSE_Enable_Root_Cert), (9, EFUSE_Disable_DBGPORT_LOCK)])  #0x58
        writeHexStringToFile(f, EFUSE_c_ctrl1)  #0x5C

        writeBitValueToFile(f, [(0, EFUSE_c2k_sbc_en), (1, EFUSE_md1_sbc_en), (2, EFUSE_disable_self_blow)])  #0x60
        writeBitValueToFile(f, [(0, EFUSE_c_data0_lock), (1, EFUSE_c_data1_lock), (4, EFUSE_c_ctrl0_lock), (5, EFUSE_c_ctrl1_lock)])  #0x64
        ...
        writeBitValueToFile(f, [(0, EFUSE_sbc_pubk_hash_lock), (1, EFUSE_ackey_lock), (2, EFUSE_sec_attr_lock)])  #0x68
        writeBitValueToFile(f, [(1, EFUSE_usb_id_lock), (2, EFUSE_com_ctrl_lock)])  #0x6C

        #extends to 512 bytes file siz
        for i in range(92) : #0x5C
